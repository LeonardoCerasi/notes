...
\end{equation}
Computing the spatial momentum from \eref{eq:4-mom-def} as $ P^i = \mathfrak{N} \int \dd^3x\, \theta^{0i} = \int \dd^3x\, \mathfrak{N} \pa_0 \phi \pa^i \phi $:
\begin{equation}
...
\begin{proofbox}
  \begin{proof}
    ...
    Using \lref{lemma:haml-lem}, \eref{eq:uv-spin-id} and the antisymmetry $ \mathfrak{N} b_{\ve{p},s} b_{\ve{p},s}^\dagger = - b_{\ve{p},s}^\dagger b_{\ve{p},s} $:
    \begin{equation*}
...
According to Wigner's theorem (\tref{th:wigner}), this symmetry can be represented by a unitary operator, i.e. $ \mathcal{P}^\dagger = \mathcal{P}^{-1} $, but $ \mathcal{P}^2 = \id $, therefore $ \mathcal{P}^\dagger = \mathcal{P} $. Then, \eref{eq:parity-op} is valid for $ a_{\ve{p},s} , b_{\ve{p},s} $ too, thus parity acts as:
\begin{equation}
