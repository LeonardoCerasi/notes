...
A real scalar field is promoted to a real hermitian operator. In particular, by Eq. \ref{eq:scalar-field-exp}:
\begin{equation}
...
\end{equation}
In terms of creation and annihilation operators, the commutator Eq. \ref{eq:scalar-quant-comm} reads:
\begin{equation}
...
\begin{proposition}{Normalization}{}
  ...
  \begin{proof}
    By Eq. \ref{eq:scalar-quant-op-comm}:
    \begin{equation*}
...
\begin{proposition}{KG Hamiltonian}{}
  ...
  \begin{proof}
    First of all, from Eq. \ref{eq:scalar-quant-exp}:
    \begin{equation*}
      \begin{split}
    ...
    \end{equation*}
    Inserting these expressions in Eq. \ref{eq:scalar-field-haml}:
    \begin{equation*}
      \begin{split}
        \mathcal{H}
...
The second term in the Hamiltonian Eq. \ref{eq:kg-haml} is the sum of the zero-point energy of all oscillators and is proportional to $ (2\pi)^3 \delta^{(3)}(0) \rightarrow V $, thus:
\begin{equation*}
...
\end{equation}
Computing the spatial momentum from Eq. \ref{eq:4-mom-def} as $ P^i = \mathfrak{N} \int \dd^3x\, \theta^{0i} = \int \dd^3x\, \mathfrak{N} \pa_0 \phi \pa^i \phi $:
\begin{equation}
...
When considering a complex scalar field, Eq. \ref{eq:compl-scal-field-exp} becomes:
\begin{equation}
...
\begin{proposition}{$ \Un{1} $ charge}{}
  ...
  \begin{proof}
    By Eq. \ref{eq:comp-scal-field-curr}:
    \begin{equation*}
      \begin{split}
        Q_{\Un{1}}
...
From the Dirac Lagrangian Eq. \ref{eq:dirac-lagrangian}, the conjugate momentum to the Dirac field $ \Psi $ is computed as:
\begin{equation}
...
\end{equation}
where the spinor wave functions $ u^s(p) , v^s(p) $ are given by Eqq. \ref{eq:u-spinor-def}-\ref{eq:v-spinor-def}. Translating Eq. \ref{eq:dirac-anticomm} in terms of creation/annihilation operators:
\begin{equation}
...
\begin{proposition}{Dirac Hamiltonian}{}
  ...
  \begin{proof}
    By Eqq. \ref{eq:dirac-conj-mom}, the Hamiltonian density is:
    \begin{equation*}
    ...
    \end{equation*}
    Therefore, using Eqq. \ref{eq:dirac-exp}-\ref{eq:dirac-bar-exp} and Eq. \ref{eq:fourier-delta}:
    \begin{equation*}
      \begin{split}
        H
    ...
    \end{equation*}
    Therefore, omitting the constraint $ p^0 = E_\ve{p} $ in the spinors' arguments and using Eq. \ref{eq:fourier-delta}:
    \begin{equation*}
      \begin{split}
        H
    ...
    Using Lemma \ref{lemma:haml-lem}, Eq. \ref{eq:uv-spin-id} and the antisymmetry $ \mathfrak{N} b_{\ve{p},s} b_{\ve{p},s}^\dagger = - b_{\ve{p},s}^\dagger b_{\ve{p},s} $:
    \begin{equation*}
...
\footnotetext{This hints to a more profound meaning of the spin-statistic theorem: as shown by Pauli in \cite{pauli}, this theorem is implied by the conditions of Lorentz invariance, positive energies and causality.}
...
\begin{proposition}{Momentum operator}{}
  ...
  \begin{proof}
    By Eq. \ref{eq:en-mom-tensor}, the $ 0i $-component of energy-momentum tensor of the Dirac Lagrangian is:
    \begin{equation*}
      \theta^{0i} = \frac{\pa \mathcal{L}_\text{D}}{\pa (\pa_0 \Psi)} \pa^i \Psi = \bar{\Psi} i \gamma^0 \pa^i \Psi = - \Psi^\dagger i \pa_i \Psi
    \end{equation*}
    Thus, according to Eq. \ref{eq:4-mom-def}:
    \begin{equation*}
      \begin{split}
        \ve{P}
...
Under a generic Lorentz transformation, the Dirac field $ \Psi $ transforms according to Eq. \ref{eq:dir-field-change}:
\begin{equation}
  \Psi'(x) = e^{-\frac{i}{2} \omega_{\mu \nu} J^{\mu \nu}} \Psi(x) \simeq \left[ \tens{I}_4 - \frac{i}{2} \omega_{\mu \nu} J^{\mu \nu} \right] \Psi(x)
  \label{eq:dirac-inf-rot}
\end{equation}
where the Lorentz generator $ J^{\mu \nu} $ is defined in Eq. \ref{eq:dir-field-tot-ang-mom}.

\begin{proposition}{Angular momentum}{}
  The conserved charge associated to the infinitesimal rotation Eq. \ref{eq:dirac-inf-rot} is:
  \begin{equation}
  ...
  \begin{proof}
    Consider a rotation of $ \theta $ around the $ z $-axis: it is described by $ \omega_{12} = - \omega_{21} = \theta $, so, by Eq. \ref{eq:dir-field-change}:
    \begin{equation*}
    ...
    \end{equation*}
    Using the same notation of Def. \ref{def:inf-trans}, $ \epsilon \equiv \theta $ and $ F = \Delta \Psi $, therefore, by Eq. \ref{eq:noether-current}, the temporal component of the conserved Noether current is (without negative sign, as it is mathematically equivalent):
    \begin{equation*}
...
\end{equation*}
The $ S_z $ operator thus acts as (recall Eq. \ref{eq:u-spinor-def} with $ p^\mu = (m,0,0,0) $):
\begin{equation*}
  \begin{split}
...
Another important conserved Noether charge of Dirac theory is that associated to the vector current $ j^\mu_\text{V} = \bar{\Psi} \gamma^\mu \Psi $ (recall Eq. \ref{eq:dirac-u1-symm}), which is (using $ j^0_\text{V} = \Psi^\dagger \Psi $):
\begin{equation}
...
\begin{equation*}
  ...
  \end{pmatrix}
\end{equation*}
Consider $ \Psi_\text{L} $. As for Eq. \ref{eq:dirac-exp}:
\begin{equation*}
...
According to Wigner's theorem (Th. \ref{th:wigner}), this symmetry can be represented by a unitary operator, i.e. $ \mathcal{P}^\dagger = \mathcal{P}^{-1} $, but $ \mathcal{P}^2 = \id $, therefore $ \mathcal{P}^\dagger = \mathcal{P} $. Then, Eq. \ref{eq:parity-op} is valid for $ a_{\ve{p},s} , b_{\ve{p},s} $ too, thus parity acts as:
\begin{equation}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Setting $ \ve{p}' \equiv - \ve{p} , \ve{x}' \equiv -\ve{x} $ and noting that, by Eqq. \ref{eq:u-spinor-p3-frame}-\ref{eq:v-spinor-def} $ \ve{p} \mapsto -\ve{p} $ exchanges the left-handed and the right-handed compontents of the spinors, i.e. $ u^s(p) \mapsto \gamma^0 u^s(p) $ and $ v^s(p) \mapsto - \gamma^0 v^s(p) $:
\begin{equation*}
  \begin{split}
    \mathcal{P} \Psi(x) \mathcal{P}
...
Recall Eq. \ref{eq:dir-spin-charge-conj}: charge conjugation acts on the classical Dirac field as $ \Psi \mapsto -i \gamma^2 \Psi^* $.
...
\begin{proposition}{Charge conjugation on Dirac fields}{}
  ...
  \begin{proof}
    Using Eqq. \ref{eq:charge-conj-op}-\ref{eq:charge-conj-spin}:
    \begin{equation*}
      \begin{split}
