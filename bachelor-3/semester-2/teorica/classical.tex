...
\begin{definition}{Lie algebras}{}
  ...
  \begin{equation}
    [T^a, T^b] = i f^{abc} T^c
  \end{equation}
  where $ f^{abc} $ are called the \textit{structure constants}.
\end{definition}
...
A Lie algebra $ \mathfrak{g} $ is \textit{commutative} if $ [X,Y] = 0 \,\,\forall X,Y \in \mathfrak{g} $.
...
\begin{proposition}{Structure constants}{}
  ...
  \begin{proof}
    ...
    \end{equation*}
    \begin{equation*}
      \iff f^{ade} f^{bcd} + f^{bde} f^{cad} + f^{cde} f^{abd} = 0
    \end{equation*}
    The condition $ ([T^b_\text{ad},T^c_\text{ad}])_{ae} = i f^{bcd} (T^d_\text{ad})_{ae} $ then gives:
    \begin{equation*}
      f^{abd} f^{dce} - f^{acd} f^{dbe} = f^{bcd} f^{ade}
    \end{equation*}
    \begin{equation*}
      \iff f^{abd} f^{cde} + f^{cad} f^{bde} + f^{bcd} f^{ade} = 0
    \end{equation*}
...
\begin{equation}
  ...
  \label{eq:quad-cas}
\end{equation}
This is called the \textit{quadratic Casimir operator}, as it is associated to $ T^2 \equiv T^a T^a $ (a Casimir operator since $ [T^b, T^2] = i f^{bac} \{T^c,T^a\} = 0 $ by antisymmetry).
...
\begin{proposition}{Quadratic Casimir operator}{}
  ...
  \begin{proof}
    ...
    \end{equation*}
    Consider $ \mathtt{n} \otimes \mathtt{n}^* $, where $ \mathtt{n}^* $ is the complex conjugate of the fundamental representation (for complex representations, $ \mathtt{r} $ and $ \mathtt{r}^* $ are generally inequivalent representations): then $ \Xi_{pq} $ contains a term proportional to the invariant $ \delta_{pq} $, while the other $ n^2 - 1 $  independent components transform as a general $ n \times n $ traceless tensor, i.e. under the adjoint representation of $ \SUn{n} $ (as of Eqq. \ref{eq:sun-herm}-\ref{eq:sun-trace}), thus $ \mathtt{n} \otimes \mathtt{n}^* = \mathtt{1} \oplus \mathtt{g} $ and the above identity becomes:
    \begin{equation*}
      (C_2(\mathtt{1}) + C_2(\mathtt{g})) (n^2 - 1) = (C_2(\mathtt{n}) + C_2(\mathtt{n}^*)) n^2
    \end{equation*}
    Using $ C_2(\mathtt{1}) = 0 $ (as all generators are trivially zero) and $ C_2(\mathtt{n}^*) = C_2(\mathtt{n}) $:
    \begin{equation*}
...
The fundamental representation of $ \SUn{2} $ is $ T^a_\mathtt{2} = \frac{\sigma^a}{2} $, while $ \mathfrak{su}(2) $ is defined by commutators $ [T^a_\mathtt{2},T^b_\mathtt{2}] = i \epsilon^{abc} T^c_\mathtt{2} $ (as $ \sigma^a \sigma^b = \delta^{ab} \tens{I}_2 + i \epsilon^{abc} \sigma^c $). The adjoint representation then is:
\begin{equation}
...
\begin{proposition}{}{lorentz-algebra}
  ...
  \begin{proof}
    ...
    \end{equation*}
    These are two commuting $ \mathfrak{su}(2) $ algebras, thus proving the thesis.
  \end{proof}
\end{proposition}
