...
\footnotetext{Given an index set $ I $ and a $ \K $-vector space, the latter is $ I $-\textit{graded} if there exists a family of subspaces $ \{V_i\}_{i \in I} $ of $ V $ such that:
\begin{equation*}
...
\footnotetext{Note that this is not in the basis of $ \dira $. It has the following properties:
\begin{equation*}
  (\gamma^5)\dg = \gamma^5
  \qquad
  (\gamma^5)^2 = \tens{I}_4
...
This result means that, although they are matrices, the $ \mu $ in $ \gamma^\mu $ is effectively a Lorentz index, thus allowing to construct dot products with 4-vectors: given a 4-vector $ A^\mu $, its dot product with the gamma matrices is denoted by $ \slashed{A} \equiv \gamma^\mu A_\mu $.
