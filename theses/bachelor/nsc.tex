...
\end{equation}
where $ X $ is a colour-neutral system. The hadron-scale physics can be treated separately from the parton-scale physics (see Section 1.1 of \cite{Collins-2011} and \secref{sec:coll-phys}): this makes it possible for us to directly manipulate partonic cross-sections according to \eref{eq:fact-th}, where now the sum runs over all initial-state massless partons $ a $ and $ b $ which contribute to the production of the final state under consideration. Moreover, for the rest of this work we set $ \ren = \fac = \mu $, where $ \mu $ is the typical energy scale of the considered process.
...
For NLO real emissions we consider an additional parton, i.e. a final state $ \fs{n}{m+1} $ with $ n \in \en_{m+1} $. To precisely identify potentially-unresolved partons, we first consider a partition of unity such that:
\begin{equation}
  ...
  \label{eq:delta-part}
\end{equation}
with $ z_{k,j} \equiv E_k / (E_k + E_j) $. An explicit construction of these damping factors is given in \secref{sec:unit-part}. It is clear that a term multiplied by $ \Delta^i $ vanishes if any parton other than $ i $ becomes unresolved, thus this partition allows for the extraction of single potentially-unresolved partons. We can thus write the real-emission contribution as:
\begin{equation*}
...
\footnotetext{Massive partons cannot become unresolved, as they do not determine soft or collinear singularities. Moreover, with a slight notation abuse, we consider the gluon as a ``partonic flavour".}
...
\begin{equation}
  ...
  \label{eq:omega-part}
\end{equation}
It now remains to understand how the operators in \eref{eq:nest-sub} act on the $ \flm $ functions and how the partonic set $ \fs{n}{m}(\ur) $ changes when $ \ur $ effectively becomes unresolved.
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
where $ \hyp(a,b,c,z) $ is the Gauss hypergeometric function and we made use of the short-hands defined in \eref{eq:short-hand} and the angular integral from Appendix G.3 of \cite{Asteriadis-2020}. It is thus possible to write the integrated soft-counterterm as:
\begin{equation}
...
Collinear limits are more delicate to analyze, due to the fact that each potentially-unresolved parton can become collinear to any other parton in $ \prt{n}{m}(\ur) $. To consider a single collinear limit at a time, i.e. a single parton pair becoming collinear, we use the partition \eref{eq:omega-part}. In particular, we are interested in the extraction of hard-collinear singularities stemming from terms of the form $ \oso_\ur \co_{i\ur} $, which in the case of unresolved quarks coincides with $ \co_{i\ur} $.
...
\begin{equation*}
  ...
  \end{tikzpicture}
\end{equation*}
Note that we only consider amplitudes corresponding to a singular limit, and these must proceed through an intermediate state $ [i\ur] $: for this reason, we ignore the case of $ i $ and $ \ur $ being produced directly from the hard process. The factorization of the amplitude reads:
\begin{equation}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
where $ \so_z \equiv \lim_{z \rightarrow 1} $ is the $ \so_\ur $ operator expressed in terms of the $ z $ variable. Since $ \so_z $ only extracts the singular part, and by \eeref{eq:spl-1}{eq:spl-4}, it is clear that:
\begin{equation}
  \so_z \spl_{f_{[i\ur]}f_i}(z) = \frac{2}{1 - z} \tco^2_{[i\ur]} \delta_{f_{[i\ur]},f_i}
\end{equation}
We then write $ [\zm,1] = [\zm,0) \cup [0,1] $, and note that the region $ [\zm,0) $ gives a non-zero contribution in the soft limit. It only remains to evaluate the following integral:
\begin{equation*}
  \begin{split}
...
\end{equation}
The process-dependence of this generalized anomalous dimension, as opposed to the initial-state one discussed below, is due to the $ z $ factor coming from the damping factor $ \Delta^\ur $ (\eref{eq:delta-part}).
...
The only difficulty left is to combine the generalized anomalous dimensions arising from the various terms in \eref{eq:part-chan}.
...
We now move to the case of an unresolved quark, i.e. we consider the hard-collinear operator $ \oso_{\ur_{q_\rho}} \co_{i\ur_{q_\rho}} \equiv \co_{i\ur_{q_\rho}} $. In this case, IR singularities arise when $ \ur_{q_\rho} $ becomes collinear to:
\begin{itemize}
...
\begin{multline}
  \sum_{n \in \en_{m+1}(q_\rho)} \sum_{i \in \prt{n}{m}(\ur_{q_\rho})} \ips{\oso_\ur \co_{i\ur} \Delta^\ur \flm_{a,b}[\fs{n}{m}(\ur_{q_\rho})]} = \left( a \leftrightarrow b \right) + \sum_{n \in \en_m} \frac{[\rc]}{\de} \ips{\delta_{f_a , g} \gspl_{\bar{q} g} \otimes \flm_{\bar{q}_\rho , b}[\fs{n}{m}]} + \\
...
\begin{multline}
  \sum_{n \in \en_{m+1}(\bar{q}_\rho)} \sum_{i \in \prt{n}{m}(\ur_{\bar{q}_\rho})} \ips{\oso_\ur \co_{i\ur} \Delta^\ur \flm_{a,b}[\fs{n}{m}(\ur_{\bar{q}_\rho})]} = \left( a \leftrightarrow b \right) + \sum_{n \in \en_m} \frac{[\rc]}{\de} \ips{\delta_{f_a , g} \gspl_{q g} \otimes \flm_{q_\rho , b}[\fs{n}{m}]} + \\
...
\begin{equation*}
  \begin{split}
    ...
    & \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad + \ips{\delta_{f_a , \bar{q}_\rho} \gspl_{g \bar{q}} \otimes \flm_{g,b}[\fs{n}{m}]} + \left( a \leftrightarrow b\right) \big] \\
  ...
  \end{split}
\end{equation*}
where we used the fact that generalized splitting functions and anomalous dimensions do not depend on quark/antiquark flavours and we have included terms arising from collinear limits with respect to the initial-state parton $ b $. Collectively, Mellin convolutions can be grouped as:
\begin{equation*}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
where we used \eref{eq:fin-gen-an-dim} to combine generalized process-dependent final-state anomalous dimensions into generalized final-state anomalous dimensions, which can be summed with initial-state ones to define the \bctxt{integrated collinear operator}:
\begin{equation}
  \ico(\de) \defeq \sum_{i \in \prt{n}{m}} \frac{\Gamma_{i,f_i}}{\de}
\end{equation}
This operator is $ \ico(\de) \sim \de^{-1} $, as opposed to $ \iso(\de) \sim \de^{-2} $. The $ \iso(\de) $ and $ \ico(\de) $ operators allow us to rewrite the expression of the real contribution in \eref{eq:part-chan} as:
\begin{multline}
  2\hat{s}\, \dd\pcs_{a,b}^\text{R} = 2\hat{s}\, \dd\pcs_{a,b}^\text{NLO,reg} + [\rc] \sum_{n \in \en_m} \ips{[\iso(\de) + \ico(\de)] \flm_{a,b}[\fs{n}{m}]} + \\
  + \frac{[\rc]}{\de} \sum_c \sum_{n \in \en_m} \ips{\gspl_{f_c f_a} \otimes \flm_{c,b}[\fs{n}{m}] + \gspl_{f_c f_b} \otimes \flm_{a,c}[\fs{n}{m}]}
  \label{eq:real-fin}
\end{multline}
where the finite reminder is (recall \eref{eq:nest-sub}):
\begin{equation}
...
The counterterms resulting from the extraction of IR singularities from real corrections, which have an explicit expression in \eref{eq:real-fin}, are not $ \de $-finite: their poles must cancel against those resulting from virtual corrections and from the collinear renormalization of PDFs.

\subsection{Virtual corrections}

An expression with clear $ \de $-poles can be derived from \eref{eq:virt-cs} by expressing the one-loop amplitude in \eref{eq:mat-exp} using Catani's formula:
\begin{equation}
...
\end{equation}
Then, making the finite reminder of the one-loop amplitude explicit, \eref{eq:virt-cs} can be rewritten as:
\begin{equation}
...
As already stated, the partonic cross-section is a non-observable quantity and it is linked to the observable hadronic cross-section by the factorization theorem \eref{eq:fact-th}. With this theorem, non-perturbative effects are encapsulated in PDFs, which are divergent: hence, they need to be renormalized.
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
where we used \eref{eq:gspl-def}. This expression is manifestly $ \de $-finite.
...
\begin{equation}
  ...
  \label{eq:co-alg}
\end{equation}
where the sum over $ j \in \prt{n}{m} $ was removed due to the colour-conservation condition \eref{eq:col-cons}. Then, the above sum can be rewritten as:
\begin{equation}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Therefore, they have the same expression as \eref{eq:gen-ad}. Hence, we can write the integrated collinear operator as:
\begin{equation}
  \ico(\de) = \frac{1}{\de} \sum_{i \in \prt{n}{m}} \left( \gamma_i + 2 \tco_i^2 \leg_i \right) + \smo(\de^0)
  \label{eq:ic-exp}
\end{equation}
The poles in \ceref{eq:isvo-exp-l}{eq:ic-exp} manifestly cancel, leaving then $ \ito(\de) \sim \smo(\de^0) $. Denoting $ \ito(\de) \equiv \ito^{(0)} + \smo(\de) $, the NLO correction in \eref{eq:part-ser-exp} is written in an $ \de $-finite form as:
\begin{multline}
...
\end{equation}
The various terms of \eref{eq:nlo-finite-massless} have no IR singularity, thus they can be numerically integrated. We note that this equation correctly reproduces the results in \cite{rontsch-2509}.
