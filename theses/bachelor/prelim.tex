...
\end{equation}
where $ \mt{U}(y,x) $ is defined through \eref{eq:fac-cov-der}. To make this definition explicit, it is necessary to get an expression of $ \mt{U}(y,x) $ at infinitesimally-separted points. Given the unitarity of $ \mt{U}(y,x) $, it can be expressed through the generators $ \{T^a\}_{a = 1, \dots, d} $ as:
\begin{equation}
