...
\footnotetext{Note that, if $ \ell \bar{\ell} \rightarrow \ell \bar{\ell} $, with $ \ell = e^-, \mu^- $, was considered instead, there would be a second Feynman diagram to compute:
\begin{equation*}
  \begin{tikzpicture}[baseline=(r.base)]
    \begin{feynman}[inline=(r.base)]
...
\begin{lemma}[before upper = {\tcbtitle}]{Bi-spinor products}{}
  \begin{equation}
    (\bar{u} \gamma^\mu v)\dg = \bar{v} \gamma^\mu u
  \end{equation}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    $ (\bar{u} \gamma^\mu v)\dg = v\dg (\gamma^\mu)\dg \bar{u}\dg = v\dg (\gamma^\mu)\dg (u\dg \gamma^0)\dg = v\dg (\gamma^\mu)\dg \gamma^0 u = v\dg \gamma^0 \gamma^\mu u = \bar{v} \gamma^\mu u $.
  \end{proof}
\end{proofbox}
...
Note that this is the most general expression for $ 2 \rightarrow 2 $ scattering with $ m_{i_1} = m_{i_2} $ and $ m_{f_1} = m_{f_2} $. In this frame, the Mandelstam variables are:
\begin{equation*}
...
\begin{lemma}{Scaling property}{del-scal}
  Given a differentiable $ f : I \subset \R \rightarrow \R $, then:
  \begin{equation}
    \delta(f(x)) = \sum_{i \in \mathcal{I}} \frac{\delta(x_i)}{\abs{f'(x_i)}}
  \end{equation}
  where $ \{x_i\}_{i \in \mathcal{I}} \subset I : f(x_i) = 0 $ are the roots of $ f $.
\end{lemma}
...
\begin{proofbox}
  \begin{proof}
    By \eref{eq:dirac-eq-spinors}, in the high-energy limit $ \slashed{p} u = \slashed{p} v = 0 $, therefore (as $ \gamma^0 \gamma^0 = \tens{I}_4 $):
    \begin{equation*}
...
\end{equation*}
As $ \gamma^5 u_\pm = \pm u_\pm $ and $ \gamma^5 v_\pm = \mp v_\pm $, these are clearly chirality eigenstates\footnote{Recall \lref{lemma:spinor-charge-conj}: as $ \{\gamma^5,\gamma^\mu\} = 0 $, chirality anti-commutes with charge conjugation, therefore antifermions have flipped chirality.}, hence they are helicity eigenstates too:
\begin{equation}
