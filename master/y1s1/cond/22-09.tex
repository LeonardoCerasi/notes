...
The Stern-Gerlach experiment has proved the existence of the electron's spin. Consider the $ \ch{^{47}Ag} $ atom, which has the ground state configuration $ [\ch{Kr}] 4\text{d}^{10} 5\text{s}^1 $: full shells do not contribute to the orbital angular momentum, hence the ground state of $ \ch{^{47}Ag} $ has $ L = 0 $. As we can associate a magnetic moment\footnotemark to an electron orbiting around a nucleus, we can study the angular momentum of $ \ch{^{47}Ag} $ using a magnetic field: classically $ E = - \bs{\mu} \cdot \ve{B} $, but quantistically $ \bs{\mu} $ is an operator, so we have to consider its expectation value.
