...
\begin{proposition}{Interaction correction}{}
  The correction due to $ \ham_{1,2} $ can be written as:
  \begin{equation}
    \Delta E^\pm(\sigma , \sigma') = \mathcal{J} \pm \mathcal{K}
  \end{equation}
  where the \bcprop{Coulomb integral} is defined as:
  \begin{equation}
    \mathcal{J} \equiv \int_{\R^6} \dd^3 r_1 \dd^3 r_2 \, \abs{\psi_1(\sigma)}^2 \abs{\psi_2(\sigma')}^2 \frac{e^2}{\norm{\ve{r}_1 - \ve{r}_2}}
  \end{equation}
  and the \bcprop{exchange integral} is defined as:
  \begin{equation}
    \mathcal{K} \equiv \int_{\R^6} \dd^3 r_1 \dd^3 r_2 \, \psi_1(\sigma)^* \psi_1(\sigma') \psi_2(\sigma')^* \psi_2(\sigma) \frac{e^2}{\norm{\ve{r}_1 - \ve{r}_2}}
  \end{equation}
\end{proposition}
...
\begin{example}{Helium again}{}
  ...
  \begin{itemize}
    ...
    \item $ 1\text{s}2\text{p} $ is split into a triplet $ \ch{^3P_{0,1,2}} $, which can be further split by the spin-orbit interaction, and a singlet $ \ch{^1P_1} $.
  \end{itemize}
