...
\begin{proposition}{}{}
  Given $ f \in \hmk(V,W) $ and its representative matrix $ \mt{M}_f \in \K^{m \times n} $, then:
  \begin{enumerate}[label = {\alph*.}]
...
\begin{proofbox}
  \begin{proof}
    ...
    Now, consider $ n = m $, i.e. $ \dk V = \dk W $. If $ \rk \mt{M}_f = n $, then $ \rk \ran{f} = \dk W $ and $ \dk \ker{f} = 0 $, so $ f $ is both an surjection and an injection, i.e. a bijection.
  \end{proof}
\end{proofbox}
