...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \dd s^2 = g_{\tau \tau}(\tau, \rho) \dd \tau^2 + 2 g_{\tau \rho}(\tau, \rho) \dd \tau\,\dd \rho + g_{\rho \rho}(\tau, \rho) \dd \rho^2 + r^2(\tau, \rho) \dd \Omega_2^2
    \end{equation*}
    These coordinates make the $ \SOn{3} $ isometry manifest, as it acts on $ \Sn{2} $ leaving $ \tau $ and $ \rho $ unchanged: this is a \textit{foliation} of the space by $ \Sn{2} $. Being $ r(\tau,\rho) $ the size of the sphere, it is useful to change coordinates to $ \tau $ and $ r $:
    \begin{equation*}
      \dd s^2 = g_{\tau \tau}(\tau,r) \dd \tau^2 + 2g_{\tau r}(\tau,r) \dd \tau\,\dd r + g_{rr}(\tau,r) \dd r^2 + r^2 \dd \Omega_2^2
    \end{equation*}
    ...
    \begin{equation*}
      \dd \tilde{t}^2 = \left( \frac{\pa \tilde{t}}{\pa \tau} \right)^2 \dd \tau^2 + 2 \frac{\pa \tilde{t}}{\pa \tau} \frac{\pa \tilde{t}}{\pa r} \dd \tau\,\dd r + \left( \frac{\pa \tilde{t}}{\pa r} \right)^2 \dd r^2
    \end{equation*}
    It's always possible to pick $ \tilde{t}(\tau,r) $ such that the cross-term vanishes, so that:
    \begin{equation*}
      \dd s^2 = - f(\tilde{t},r) \dd \tilde{t}^2 + g(\tilde{t},r) \dd r^2 + r^2 \dd \Omega_2^2
    \end{equation*}
    Solving the Einstein field equations requires that $ f(\tilde{t},r) = h(\tilde{t}) f(r) $ and $ g(\tilde{t},r) = g(r) $, thus redefining $ h(\tilde{t}) \dd \tilde{t}^2 = \dd t^2 $ yields:
    \begin{equation*}
      \dd s^2 = - f(r) \dd t^2 + g(r) \dd r^2 + r^2 \dd \Omega_2^2
    \end{equation*}
    Note that, even though time-independence was not assumed, it was derived from the field equations: this metric has the timelike Killing vector $ K = \pa_t $, besides those from the $ \SOn{3} $ isometry. At this point, the field equations require that $ f(r) = g(r)^{-1} $, thus this metric reduces to that considered in \secref{ssec:geo-dev}: the Schwarzschild metric is the most general solution with $ \Lambda = 0 $.
  \end{proof}
\end{proofbox}

Therefore, the Schwarzschild metric does not only describe a black hole, but it describes the spacetime outside any non-rotating spherically-symmetric object, even in the presence of time-dependence (e.g. a collapsing star).

\begin{definition}{Stationary spacetime}{def-t-ind}
  A spacetime is said to be \bcdef{stationary} if it admits a globally-timelike Killing vector field $ K $. In addition, being $ t $ the coordinate along the integral curves of $ K $, if it is invariant under $ t \rightarrow -t $, then it is said to be \bcdef{static}.
\end{definition}

The definition of static spacetime rules out $ \dd t \, \dd x $ cross-terms in the metric. Moreover, by Birkhoff theorem, a spherically-symmetric metric must be static.
...
\end{equation*}
%These are precisely the coordinates Eqq. \ref{eq:1.33}-\ref{eq:1.34} experienced by an observer undergoing constant acceleration $ a = 1 / (4GM) $: this makes sense, as an observer sitting at constant $ \rho $, i.e. constant $ r $, must accelerate in order not to fall into the black hole.\\ TODO write first chapter
These are precisely the coordinates experienced by an observer undergoing constant acceleration $ a = 1 / (4GM) $: this makes sense, as an observer sitting at constant $ \rho $, i.e. constant $ r $, must accelerate in order not to fall into the black hole.
...
\begin{equation*}
  \dd t = \dd v - \left( 1 - \frac{2GM}{r} \right) \dd r
\end{equation*}
...
\end{equation}
This is the Schwarzschild black hole in \bctxt{ingoing Eddington--Finkelstein coordinates}. Note that there's no more singularity at $ r = 2GM $; however, the $ \dd v^2 $ term vanishes at the horizon and becomes negative for $ r < 2GM $. Nonetheless, the metric is still non-degenerate thanks to the cross-term:
\begin{equation*}
  \det g = \det
  \begin{bmatrix}
    - (1 - \frac{2GM}{r}) & 1 & 0 & 0 \\
...
\end{equation*}
Ouside of the horizon $ K_\mu K^\mu < 0 $ as expected. But inside, with $ r < 2GM $, the Killing vector field is spacelike: therefore, by \dref{def:def-t-ind}, the full Schwarzschild spacetime is not time-independent, but it becomes clear only after crossing the event horizon.
