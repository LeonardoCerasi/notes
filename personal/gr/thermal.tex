...
\begin{itemize}
  \item since $ s \propto a^{-3} $, if the number of particles in a comoving volume $ N_i $ is conserved (i.e. $ n_i \propto a^{-3} $), then $ n_i \propto s $. This is the case, for example, for the net baryon number after baryogenesis, which is conserved, hence $ (n_\text{B} - n_{\bar{\text{B}}}) / s $ is constant;
