...
\begin{equation}
  \eta_\text{b} \equiv \frac{n_\text{b}}{n_\gamma} \sim 10^{-9}
\end{equation}
...
\begin{itemize}
  \item since $ s \propto a^{-3} $, if the number of particles in a comoving volume $ N_i $ is conserved (i.e. $ n_i \propto a^{-3} $), then $ n_i \propto s $, with $ N_i = n_i / s $. This is the case, for example, for the net baryon number after baryogenesis, which is conserved, hence $ N_\text{net} \equiv (n_\text{B} - n_{\bar{\text{B}}}) / s $ is constant;
...
\end{equation*}
as the number of WIMPs is conserved after freeze-out, i.e. $ N_{X,0} = N_X^\infty $. Inserting \ceref{eq:s-def}{eq:dm-relic-estimate}:
\begin{equation*}
...
\end{equation*}
A better estimate would be imposing $ (n_{\ch{D}} / n_p)_\text{eq} \sim 10^{-3} $, resulting in $ t_\text{nuc} \sim 250 \,\text{sec} $: in both cases $ t_\text{nuc} \ll \tau_n $, so \eref{eq:neutron-decay} is not very sensitive to a precise estimate of $ t_\text{nuc} $. In both cases:
\begin{equation}
...
\end{equation}
This prediction is consistent with the observed helium in the universe, as shown in \figref{fig:he-abundance}. The only atomic species which deviates from theoretical predictions is litium, and in fact this is known as the \emph{Li problem}.

\begin{figure}
  \centering
  \includegraphics[width = 0.40 \textwidth]{he-abundance.png}
