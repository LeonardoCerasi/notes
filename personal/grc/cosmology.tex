...
\begin{equation}
  \chi(z) = \int_t^{t_0} \frac{\dd t'}{a(t')} = \int_0^z \frac{\dd z'}{H(z')}
\end{equation}
which depends on the matter content of the universe through $ H(z) $. It must be emphasized that neither the metric distance nor the comoving distance are physically observable.

\paragraph{Luminosity distance}

Type IA supernovae are called ``standard candles", since they are believed to be objects of known absolute luminosity $ L $, i.e. energy emitted per second: therefore, the observed flux $ F $, i.e. energy per second per receiving area, from a supernova explosion can be used to infer its \emph{luminosity distance}. Considering a source at fixed comoving distance $ \chi $, in a static Euclidean space the relation between $ F $ and $ L $ would be:
\begin{equation*}
  F = \frac{L}{4\pi \chi^2}
\end{equation*}
...
Since the number of particles has to be conserved, in Minkowski spacetime $ \dot{n} = - \pa_i N^i $, i.e. $ \pa_\mu N^\mu = 0 $, which generalizes to a curved spacetime as:
\begin{equation}
  \na_\mu N^\mu = 0
\end{equation}
...
\begin{equation*}
  T_{ij}(\ve{x} = \ve{0}) \propto \delta_{ij} \propto g_{ij}(\ve{x} = \ve{0})
\end{equation*}
...
\begin{proposition}{Ricci tensor}{}
  ...
  \begin{equation}
    \rr_{00} = - 3 \frac{\ddot{a}}{a}
    \qquad
    \rr_{ij} = - \left[ \frac{\ddot{a}}{a} + 2 \left( \frac{\dot{a}}{a} \right)^2 + 2 \frac{k}{a^2} \right] g_{ij}
  \end{equation}
\end{proposition}

\begin{proofbox}
  \begin{proof}
    First, $ \rr_{0i} = 0 $ due to the isotropy of the FLRW metric. Then, contracting \eref{eq:riemann-comp} and recalling the only non-vanishing Christoffel symbols:
    \begin{equation*}
      \rr_{00} = - \pa_0 \Gamma^i_{i0} - \Gamma^j_{i0} \Gamma^i_{j0} = - 3 \frac{\dd}{\dd t} \left( \frac{\dot{a}}{a} \right) - 3 \left( \frac{\dot{a}}{a} \right)^2 = - 3 \frac{\ddot{a}}{a}
    \end{equation*}
    ...
    \begin{equation*}
      ...
      \end{split}
    \end{equation*}
    Spatial homogeneity and isotropy imply that $ \rr_{ij} \propto \gamma_{ij} $, thus the general result is:
    \begin{equation*}
      \rr_{ij} = \left( a\ddot{a} + 2\dot{a}^2 + 2k \right) \gamma_{ij} = - \frac{1}{a^2} \left( a\ddot{a} + 2\dot{a}^2 + 2k \right) g_{ij}
    \end{equation*}
    which is the thesis.
  \end{proof}
\end{proofbox}
...
\begin{equation}
  \rr = - 6 \left[ \frac{\ddot{a}}{a} + \left( \frac{\dot{a}}{a} \right)^2 + \frac{k}{a^2} \right]
\end{equation}
...
With \eref{eq:flrw-einstein}, the Einstein field equations \eref{eq:ef-eq} with energy-momentum tensor \eref{eq:flrw-en-mom-tens} become (respectively the temporal and spatial components):
\begin{equation}
  \left( \frac{\dot{a}}{a} \right)^2 = \frac{8\pi G}{3} \rho - \frac{k}{a^2}
  \qquad \qquad
  \frac{\ddot{a}}{a} = - \frac{4\pi G}{3} \left( \rho + 3 P \right)
\end{equation}
where the cosmological constant has been suppressed since it is accounted for by dark energy as a form of matter content. Note that these equations are not independent, as the latter is the time derivative of the former: nonetheless, these are called the \bctxt{Freedman equations}. The energy density $ \rho $ and the pressure $ P $ are to be understood as the sums of the contributions from the various forms of matter: the contribution from radiation $ \rho_\text{r} = \rho_\gamma + \rho_\nu $ (photons and neutrinos), the one from matter $ \rho_\text{m} = \rho_\text{b} + \rho_\text{c} $ (baryons and cold dark matter) and the vacuum energy contribution $ \rho_\Lambda $. Recalling the definition of the Hubble parameter \eref{eq:hubble-param}, the first Friedmann equation becomes:
\begin{equation}
...
\end{equation*}
where $ H_0 \equiv 100 h \,\text{km} \,\text{s}^{-1} \text{Mpc}^{-1} $. Then, the dimensionless density parameters are defined as:
\begin{equation}
  \Omega_a \equiv \frac{\rho_{a,0}}{\rho_{\text{crit},0}}
  \label{eq:omega-def}
\end{equation}
with $ a = \text{r},\text{m},\Lambda,\dots $ running over all possible matter species. \eref{eq:fried-hubble} can thus be rewritten as:
\begin{equation}
  H^2(a) = H_0^2 \left[ \Omega_\text{r} \left( \frac{a_0}{a} \right)^4 + \Omega_\text{m} \left( \frac{a_0}{a} \right)^3 + \Omega_k \left( \frac{a_0}{a} \right)^2 + \Omega_\Lambda \right]
\end{equation}
...
\begin{equation}
  \frac{H^2}{H_0^2} = \Omega_\text{r} a^{-4} + \Omega_\text{m} a^{-3} + \Omega_k a^{-2} + \Omega_\Lambda
  \label{eq:h-omega}
\end{equation}
...
\begin{equation*}
  ...
  \qquad
  \Omega_\text{r} \simeq 9.4 \cdot 10^{-5}
  \qquad
  \Omega_\text{m} \simeq 0.32
  \qquad
  \Omega_\Lambda \simeq 0.68
\end{equation*}
In particular, matter splits into $ 5\% $ baryonic matter, i.e. with $ \Omega_\text{b} \simeq 0.05 $, and $ 27\% $ cold dark matter, i.e. with $ \Omega_\text{c} \simeq 0.27 $. Moreover, note that even today curvature only contributes for less than $ 1\% $ of cosmic energy, and, since $ \Omega_k \propto a^{-2} $ while $ \Omega_\text{m} \propto a^{-3} $ and $ \Omega_\text{r} \propto a^{-4} $, in the past it was completely negligible: hence, setting $ \Omega_k \equiv 0 $ is justified, i.e. $ k \equiv 0 $.

\subsubsection{Single-component universe}

Given the clearly distinct scalings of $ \Omega_\text{r} \propto a^{-4} $, $ \Omega_\text{m} \propto a^{-3} $ and $ \Omega_\Lambda \propto a^0 $, for most of its history the universe was dominated by a single component: first radiation, then matter and finally vacuum energy. This is illustrated in \figref{fig:scalings}.
...
\end{equation}
where $ w_a $ characterizes the component. Note that $ \Omega_a \equiv 1 $, since this single-component universe is flat. This equation can be integrated, obtaining the time-dependence of the scale factor:
\begin{equation}
  a(t) \propto
  \begin{cases}
