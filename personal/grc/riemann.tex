...
On a non-coordinate basis $ \{e_a\} $, connection components are computed in the usual way:
\begin{equation}
  \na_{e_c} e_b \equiv \omega^a_{cb} e_a
  \label{eq:connection-vielbeins}
\end{equation}
where the $ n^3 $ functions $ \omega^a_{cb} $ are just the connection coefficients $ \Gamma^a_{cb} $ on the non-coordinate basis.
...
Then, the connection in the non-coordinate basis is easily defined:
\begin{equation*}
...
As for the connection coefficients, it is possible to compute the torsion tensor on a non-coordinate basis as $ \tensor{T}{^a_{bc}} = \tors(\theta^a , e_b , e_c) $, which retains the antisimmetry $ \tensor{T}{^a_{bc}} = - \tensor{T}{^a_{cb}} $.
...
This is a vector-valued 2-form, i.e. $ T^a $ are truly $ n $ 2-forms. Like \eref{eq:torsion-comp}, the torsion 2-form is linked to the connection 1-form by the first Cartan structure equation.

\begin{theorem}[before upper = {\tcbtitle}]{First Cartan structure equation}{}
  \begin{equation}
    \dd\theta^a + \tensor{\omega}{^a_b} \wedge \theta^b = T^a
  \end{equation}
\end{theorem}
...
\begin{lemma}{Lorentz invariance}{}
  The covariant exterior derivative is invariant under local Lorentz transformations:
  \begin{equation}
...
The same construction can be applied to form-valued 1-forms simply contracting with $ \eta_{ab} $:
\begin{equation}
  \dcov \zeta_a = \dd \zeta_a + \tensor{\omega}{_a^b} \wedge \zeta_b
\end{equation}
...
The Levi--Civita connection is the unique connection such that \eeref{eq:first-cartan-stru-eq}{eq:connection-form-symm} both hold. Moreover, for a Levi--Civita connection:
\begin{equation}
  \dcov (\zeta^a \xi_a) = \dd (\zeta^a \xi_a)
\end{equation}
for general vector/form-valued 1-forms, since $ \tensor{\omega}{_a^b} = - \tensor{\omega}{^b_a} $.
