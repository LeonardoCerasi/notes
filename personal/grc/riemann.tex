...
\begin{equation}
  \na_{\hat{e}_c} \hat{e}_b = \omega^a_{cb} \hat{e}_a
...
\begin{definition}{Connection 1-form}{}
  ...
  \begin{equation}
    \tensor{\omega}{^a_b} \defeq \omega^a_{cb} \hat{\theta}^c
...
\begin{equation*}
  \na_X Y = X(Y^a) \hat{e}_a + Y^a X^b \na_{\hat{e}_b} \hat{e}_a = X^b [\hat{e}_b(Y^a) + \omega^a_{bc} Y^c] \hat{e}_a
\end{equation*}
Hence, setting the notation $ \na_b Y^a \equiv (\na_{\hat{e}_b} Y)^a $, it is clear that the connection takes the form:
\begin{equation}
  \na_b Y^a = \hat{e}_b(Y^a) + \omega^a_{bc} Y^c
\end{equation}
In general, $ \hat{e}_b(Y^a) = \tensor{e}{_b^\mu} \pa_\mu Y^a $. Using \eref{eq:cov-der-form-aux}, the covariant derivative of forms is found:
\begin{equation}
  \na_b \zeta_a = \hat{e}_b(\zeta_a) - \omega^c_{ba} \zeta_c
\end{equation}
Finally, for tensors of general rank:
\begin{equation}
  \na_c \tensor{T}{^{a_1 \dots a_r}_{b_1 \dots b_s}} = \hat{e}_c(\tensor{T}{^{a_1 \dots a_r}_{b_1 \dots b_s}}) + \sum_{j = 1}^r \omega^{a_j}_{c d_j} \tensor{T}{^{a_1 \dots d_j \dots a_n}_{b_1 \dots b_s}} - \sum_{j = 1}^s \omega^{d_j}_{c b_j} \tensor{T}{^{a_1 \dots a_n}_{b_1 \dots d_j \dots b_n}}
\end{equation}

\begin{proposition}{Transformation law}{}
  Given a local Lorentzian transformation $ \Lambda $ acting as in \eref{eq:vielbeins-trans}:
  \begin{equation}
    \tensor{\tilde{\omega}}{^a_b} = \tensor{\Lambda}{^a_c} \tensor{\omega}{^c_d} \tensor{(\Lambda^{-1})}{^d_b} + \tensor{\Lambda}{^a_c} \tensor{(\dd\Lambda^{-1})}{^c_b}
  \end{equation}
\end{proposition}
...
\begin{theorem}{First Cartan structure equation}{}
  ...
  \begin{equation}
    \dd\hat{\theta}^a + \tensor{\omega}{^a_b} \wedge \hat{\theta}^b = 0
  ...
  \begin{proof}
    ...
    \begin{equation*}
      \begin{split}
        ...
        &= -\eta_{cf} \tensor{e}{^f_\rho} \tensor{e}{_b^\mu} \na_\mu \tensor{e}{_a^\rho} = -\omega_{cba}
      \end{split}
    \end{equation*}
    From \eref{eq:connection-1-form} $ \omega_{ab} = \omega_{acb} \hat{\theta}^c $, thus completing the proof.
  \end{proof}
\end{proofbox}
...
\begin{definition}{Curvature 2-form}{}
  ...
  \begin{equation}
    \tensor{\Omega}{^a_b} \defeq \frac{1}{2} \tensor{R}{^a_{bcd}} \hat{\theta}^c \wedge \hat{\theta}^d
    \label{eq:curvature-2-def}
  \end{equation}
\end{definition}

Again, these are truly $ n^2 $ 2-forms. A second Cartan structure relation holds.

\begin{theorem}{Second Cartan structure equation}{}
  On a metric manifold with a Levi--Civita connection:
  \begin{equation}
    \tensor{\Omega}{^a_b} = \tensor{\dd\omega}{^a_b} + \tensor{\omega}{^a_c} \wedge \tensor{\omega}{^c_b}
