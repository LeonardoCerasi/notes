...
\begin{proofbox}
  \begin{proof}
    Recall \eref{eq:conformal-ricci}. $ \rr = 0 $ in an open neighbourhood of $ \scri $, by \dref{def:asymptotically-flat}, so the limit $ \Omega \ra 0 $ results into $ \braket{n , n}_g = 0 $: by the same argument in the proof of \pref{prop:causal-conformal-boundary}, $ \scri $ is a null hypersurface.
  \end{proof}
\end{proofbox}
