...
Each smooth coordinate map $ \varphi_{\alpha} $ provides a coordinate system on the smooth coordinate domain $ U_{\alpha} $: $ \varphi_{\alpha}(p) = \left( x^1(p), \dots, x^n(p) \right) $. The transition functions $ \varphi_{\beta} \circ \varphi_{\alpha}^{-1} $ are therefore coordinate transformations on overlapping regions.
