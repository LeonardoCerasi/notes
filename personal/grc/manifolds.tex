...
The empty set satisfies the definition of $ n $-dimensional topological manifold for all $ n \in \N $, hence it is excluded.

\begin{definition}{Smooth manifold}{smooth-manifold}
  An $ n $-dimensional topological manifold is a \bcdef{smooth manifold}
\end{definition}
...
\begin{definition}{Smooth maps}{smooth}
  A function $ f : \mathcal{M} \rightarrow \R^n $ on a differentiable manifold $ (\mathcal{M},\mathcal{A}) $ is \bcdef{smooth} if $ f \circ \varphi_{\alpha}^{-1} : U_{\alpha} \rightarrow \R^n $ is smooth for all charts $ (U_{\alpha},\varphi_{\alpha}) \in \mathcal{A} $.
...
Note that transition maps between overlapping charts are indeed diffeomorphisms, since they are smooth homeomorphisms (composition of homeomorphisms).
...
Since coordinate changes are just transition maps between overlapping charts on the manifold, they are diffeomorphisms from a patch of $ \man $ on itself (as noted in \secref{ssec:maps}). In general, a diffeomorphism $ f : \man \ra \man $ defines a change of coordinates on the whole $ \man $: indeed, given two charts $ (\varphi_1 , U) , (\varphi_2 , V) $ such that $ f(U) = V $, then:
\begin{equation*}
  \begin{tikzcd}
