...
Free particles moving on an $ n $-dimensional metric manifold $ (\man,g) $ have geodesics as trajectories, which are found as extrema of the action functional.
...
\begin{equation*}
  p_{\mu} = \frac{\pa L}{\pa \dot x^{\mu}} = - m c \frac{\pa}{\pa \dot x^{\mu}} \sqrt{- g_{\nu \rho} \dot{x}^{\nu} \dot{x}^{\rho}} = - \frac{m^2 c^2}{L} \dot x_\mu
\end{equation*}
which results into the \bctxt{mass-shell condition}:
\begin{equation}
  p^2 = - m^2 c^2
\end{equation}
Therefore, the time component of $ p^\mu $ is found to be $ (p^0)^2 = m^2 c^2 + \ve{p}^2 $, which has two important consequences: first of all, $ p^0 \neq 0 $, which means that the particle is never at rest in the temporal direction (which was to be expected); secondly, $ p^\mu $ only has $ n - 1 $ independent components, which is a consequence of the reparametrization invariance of the action\footnotemark.
...
\begin{equation}
  ...
  \label{eq:em-act}
\end{equation}
where $ A_\mu(x) $ is the potential of the field and $ q \in \R $ is the charge of the coupling. To determine the potential $ A_\mu $ for the electromagnetic interaction, compare the classical limit of \eref{eq:em-act} to the action of a non-relativistic particle in an external electromagnetic field:
\begin{equation*}
...
\end{equation*}
Comparing the individual terms, the electromagnetic interaction is described by the vector potential $ A_\mu(x) = (\phi(x) / c , \ve{A}(x)) $.
...
\begin{equation*}
  \act = - m c^2 \int_{t_1}^{t_2} \dd t \sqrt{- g_{00}(x) - \frac{1}{c} \left( 2 g_{0i} \dot{x}^i + \frac{1}{c} g_{ij} \dot{x}^i \dot{x}^j \right)}
\end{equation*}
