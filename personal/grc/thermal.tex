...
\end{equation}
where $ n $ is the number density of particles, $ v $ is their average velocity and $ \sigma $ is the interaction cross-section. For $ T \gtrsim 100 \gev $, all known particles are ultra-relativistic, so $ v \sim 1 $, and their masses can be ignored, leaving the temperature $ T $ as the only dimensionfull scale: by dimensional analysis $ n \sim T^3 $. Moreover, interactions are mediated by gauge bosons, which are massless above the scale of electroweak symmetry breaking, leaving a similar dependence for the cross-section of strong and electroweak processes:
\begin{equation*}
...
\end{equation*}
The strength of weak interactions now decreases as the temperature of the universe drops, and the ratio $ \Gamma / H $ becomes:
\begin{equation}
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \dd N_A = \frac{\abs{\hat{\ve{v}} \cdot \hat{\ve{n}}} \dd A}{4 \pi R^2} \dd N = \frac{g}{(2\pi)^3} f(E,T) v \frac{\abs{\hat{\ve{v}} \cdot \hat{\ve{n}}}}{4 \pi} \dd A \dd t \dd \Omega
    \end{equation*}
...
\begin{proofbox}
  \begin{proof}
    ...
    \end{equation*}
    Since the entropy function must exist, this differential form must be exact. Since $ \dd^2 = 0 $, an exact form is also closed, hence:
    \begin{equation}
    ...
    \end{equation*}
    Tho show that entropy is conserved at equilibrium, consider:
    \begin{equation*}
      \begin{split}
...
It is convenient to define the \bctxt{entropy density}, which ignoring an integration constant is (by the above proof):
\begin{equation}
  s = \frac{\rho + P}{T}
\end{equation}
...
\end{equation}
which is $ \dofs^\text{dec}(T) \neq \dof^\text{dec}(T) $. It follows that $ \dof(T) = \dofs(T) $ only when \emph{all} relativistic species are in thermal equilibrium with the photon bath: in the real universe, this is the case until $ t \approx 1 \,\text{sec} $ (see \figref{fig:rel-dof}).
...
\begin{center}
  \begin{tabular}{cccc}
    ...
    Neutrino decoupling & $ 1 \,\text{s} $ & $ 6 \cdot 10^9 $ & $ 0.8 \mev $ \\ [1ex]
...
\end{equation*}
as the number of WIMPs is conserved after freeze-out: $ N_{X,0} = N_X^\infty $. Inserting \eref{eq:s-def} and \eref{eq:dm-relic-estimate}:
\begin{equation*}
...
\subsubsection{Equilibrium abundances}
 ...
 To show the validity of these assumptions, recall that at $ T \gtrsim 0.8 \mev $ protons and neutrons are coupled via weak interactions ($ \beta $-decay and inverse $ \beta $-decay):
\begin{equation*}
  n + \nu_e \lra p^+ + e^-
  \quad \qquad
  n + e^+ \lra p^+ + \bar\nu_e
...
\begin{equation*}
  n + p^+ \lra D + \gamma
\end{equation*}
Since $ \mu_\gamma = 0 $, then $ \mu_{\ch{D}} = \mu_n + \mu_p $, recalling that $ g_{\ch{D}} = 4 $ and $ g_n = g_p = 2 $:
\begin{equation}
...
The primordial ratio of neutrons is particularly important for BBN, since essentially all neutrons became incorporated into $ \ch{^4He} $. Defining the neutron fraction as $ X_n \equiv n_n / (n_n + n_p) $, with \eref{eq:n-to-p}:
\begin{equation}
...
At $ T \lesssim 0.2 \mev $, i.e. $ t \gtrsim 100 \,\text{sec} $, the finite lifetime of the neutron becomes important, and indeed neutron decay must be accounted for:
\begin{equation}
