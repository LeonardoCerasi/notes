...
\end{equation}
Recalling \eref{eq:riemann-comp}, the $ \Gamma \Gamma \sim h^2 $ terms of the Riemann tensor are negligible at first order, so:
\begin{equation}
...
\end{equation}
where $ \dal \defeq \pa^\mu \pa_\mu $ and $ h \equiv \tensor{h}{^\mu_\mu} $ is the trace of $ h_{\mu \nu} $. The Ricci scalar is:
\begin{equation}
  R = \pa^\mu \pa^\nu h_{\mu \nu} - \Box h
\end{equation}
...
As in Electromagnetism, it is useful to impose a gauge fixing condition.
...
\begin{equation}
  \dd s^2 = - \left( 1 + 2\Phi(\ve{x}) \right) \dd t^2 + \left( 1 - 2\Phi(\ve{x}) \right) \dd\ve{x}^2
  \label{eq:linear-newtonian}
\end{equation}
This is exactly the condition \eref{eq:newtonian-limit}.
...
\end{equation}
where $ H_{\mu \nu} \in \C^{4 \times 4} $ is a symmetric polarization matrix and the wave-vector $ k^\mu $ is a real 4-vector. For simplicity, the $ \Re $ is made implicit in the following calculations. This plane-wave ansatz solves \eref{eq:wave} if the wave-vector is null: $ k_\mu k^\mu = 0 $: therefore, gravitational waves propagate at the speed of light. Writing $ k^\mu = (\omega, \ve{k}) $, this condition becomes $ \omega = \pm \abs{\ve{k}} $. Moreover, being the vacuum wave equation linear, the general solution is just a linear combination of plane waves.
...
\begin{equation*}
  \dal \xi_\mu = 0 \quad \Rightarrow \quad \xi_\mu(x) = \lambda_\mu e^{\img k_\rho x^\rho}
\end{equation*}
...
\begin{equation}
  ...
  \label{eq:traceless}
\end{equation}
These condition, together with \eref{eq:transverse}, are called \bctxt{transverse traceless gauge}. Being $ H_{\mu \nu} $ traceless, in this gauge $ \bar{h}_{\mu \nu} = h_{\mu \nu} $, as it is traceless too. Of the 10 components of the polarization matrix, only 2 are independent: de Donder condition \eref{eq:transverse} poses 4 constraints and the gauge transformation \eref{eq:gw-gauge-tr} poses 4 more of them (since one of the $ H_{0\mu} = 0 $ conditions is made redundant by $ k^\mu H_{\mu \nu} = 0 $). Therefore, there are only two independent polarizations in $ H_{\mu \nu} $.
...
\end{equation*}
Suppose that, in the absence of gravitational waves, the geodesics are in a rest-frame such that $ u^\mu = (1,0,0,0) $: as the gravitational wave passes $ u^\mu = (1,0,0,0) + \smo(h) $, so the aim is to compute the geodesic deviation to leading order in $ h $. The Riemann tensor is already $ \smo(h) $, thus other correction terms can be neglected; similarly, proper time $ \tau $ can be replaced with coordinate time $ t $, so that the geodesic deviation equation becomes $ \frac{\dd^2}{\dd t^2} S^\mu = \tensor{R}{^\mu_{00 \nu}} S^\nu $.
...
\begin{equation}
  \frac{\dd^2 S^\mu}{\dd t^2} = \frac{1}{2} \frac{\dd^2 \tensor{h}{^\mu_\nu}}{\dd t^2} S^\nu
...
\subsubsection{\texorpdfstring{$ + $}{+} polarization}
...
\begin{equation}
  ...
  \qquad
  S^2(t) = S^2(0) \left[ 1 - \frac{1}{2} H_+ e^{\img \omega t} + \dots \right]
...
\subsubsection{\texorpdfstring{$ \times $}{x} polarization}
...
\begin{equation}
  ...
  \qquad
  S^2(t) = S^2(0) + \frac{1}{2} S^1(0) H_\times e^{\img \omega t} + \dots
\end{equation}
...
\begin{equation*}
  \bar{h}_{00}(\ve{x}) \approx \frac{4G}{r} E
  \qquad \qquad
...
\end{equation*}
where $ \pa_j r = x_j / r \equiv \hat{x}_j $. Note that, although the first term is $ \sim 1/r^2 $ and the second $ \sim 1/r $, the latter has an additional time derivative, i.e. an extra factor of the characteristic source frequency $ \omega $: therefore, the second term dominates for $ r \gg 1/\omega $, i.e. $ r \gg \lambda $ (emitted gravitational waves' wavelength). This is the so-called \textit{far-field zone} or \bctxt{radiation zone}, and in this regime:
\begin{equation}
  \bar{h}_{0i} \approx - \frac{4G}{r} P_i - \frac{2G \hat{x}_j}{r} \ddot{I}_{ij} (t - r)
\end{equation}
This expression, found integrating the preceding equation, contains an integration constant given by the aforementioned $ P_i $, but it can be set to zero choosing coordinates in which the center of mass of the system is stationary. With analogous reasoning:
\begin{equation}
...
\begin{example}{Gravitational waves in binary system}{}
  Consider two objects of mass $ M $ and separated by a distance $ R $, orbiting in the $ (x,y) $ plane: by Newtonian gravity, the orbit frequency is $ \omega^2 = 2GM / R^3 $. Treating these objects as point particles, their energy density is:
  \begin{equation*}
...
In the context of Electromagnetism, the power radiated as electromagnetic waves are emitted is easily found defining the Poynting vector $ \ve{S} \defeq \frac{1}{\mu_0} \ve{E} \times \ve{B} $ and, using the dipole approximation, finding the \bctxt{Larmor formula}:
\begin{equation*}
  \mathcal{P} = \int_{\Sn{2}} \dd^2\ve{r} \cdot \ve{S} = \frac{\mu_0}{6\pi c} \abs{\ddot{p}}^2
\end{equation*}
Analogously, a source emitting gravitational waves loses energy which is carried by them. The problem of finding an equivalent to Larmor formula is that there is no local energy-momentum tensor for gravitational fields, so there is no analogue to the Poynting vector. A possible way forward is to define an energy-momentum tensor $ t_{\mu \nu} $ for gravitatonal waves which, in the linearized theory, obeys $ \pa^\mu t_{\mu \nu} = 0 $: \eref{eq:no-div-th} does not allow to do so in a diffeomorphism-invariant way, i.e. in the full non-linear theory $ t_{\mu \nu} $ is not a tensor and in the linearized theory it is not invariant under gauge transformations \eref{eq:weak-gauge-tr}. It turns out that there are a number of different ways to define such a tensor.
...
\begin{equation*}
  0 = \na_\mu \tensor{T}{^\mu_\nu} = \frac{1}{\sqg} \pa_\mu (\sqg\, \tensor{T}{^\mu_\nu}) - \Gamma^\rho_{\mu \nu} \tensor{T}{^\mu_\rho} = \frac{1}{\sqg} \pa_\mu (\sqg\, \tensor{T}{^\mu_\nu}) - \frac{1}{2} \pa_\nu g_{\mu \rho} T^{\mu \rho}
\end{equation*}
...
\begin{equation*}
  \pa_\mu (\sqg\, \tensor{T}{^\mu_\nu}) = \frac{1}{16\pi G} \sqg\, \pa_\nu g_{\mu \rho} \left( R^{\mu \rho} - \frac{1}{2} R g^{\mu \rho} \right) = \frac{1}{16\pi G} \sqg\, \pa_\nu g_{\mu \rho} R^{\mu \rho}
\end{equation*}
The idea is to express this equation as $ \pa_\mu (\sqg\, \tensor{T}{^\mu_\nu}) = - \pa_\mu (\sqg\, \tensor{t}{^\mu_\rho}) $, for some $ \tensor{t}{^\mu_\nu} $ referred to as the \bctxt{Landau-Lifshitz pseudotensor}: this suggests that the sum of matter energy $ \tensor{T}{^\mu_\nu} $ and gravitational energy $ \tensor{t}{^\mu_\nu} $ is conserved, however it would be coordinate-dependent as $ \tensor{t}{^\mu_\nu} $ is not a real tensor.\\
