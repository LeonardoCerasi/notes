...
\begin{example}{Ignorable degrees of freedom}{}
  Given a metric $ g_{\mu \nu}(x) $, if it does not depend on $ x^1 \equiv y $, then $ \xi = \pa_y $ is a Killing vector field, for $ (\ld_{\pa_y} g)_{\mu \nu} = \pa_y g_{\mu \nu} = 0 $. This is the case of ignorable degrees of freedom, as \eeref{eq:dS-energy}{eq:dS-ang-mom}.
\end{example}

It is possible to reframe Noether's theorem in the context of Killing vectors. Indeed, the presence of a Killing vector field gives rise to a conserved quantity.
...
In general, the Noether charge of a timelike Killing vector field is interpreted as an energy, while that of a spacelike Killing vector field as a momentum (either linear or angular).
...
Consider now $ \Lambda > 0 $. A possible ansatz is:
\begin{equation*}
...
The main problem of the C-EMT, however, is that it is not symmetric; moreover, its generalization to a generic curved spacetime is not obvious. For this reason, it is convenient to define another energy-momentum tensor starting from the action $ \act[\Phi] $:
\begin{equation}
  T_{\mu \nu} \defeq - \frac{2}{\sqg} \frac{\delta \act}{\delta g^{\mu \nu}}
  \label{eq:em-tens-def}
\end{equation}
This is the so-called \bctxt{Belinfante--Rosenfeld energy-momentum tensor} (BR-EMT), which is manifestly a symmetric $ (0,2) $-tensor, as it inherits the symmetries of the metric. It can be shown that, in general, the C-EMT and the BR-EMT only differ by a total derivative:
\begin{equation}
...
\begin{proposition}{Einstein field equations}{}
  The equations of motion determined by the action \eref{eq:ei-matter-action} are:
  \begin{equation}
    G_{\mu \nu} + \Lambda g_{\mu \nu} = 8\pi \gd T_{\mu \nu}
    \qquad \qquad
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \delta \act = \frac{1}{16\pi \gd} \int_\man \dd^d x \sqg \left[ G_{\mu \nu} + \Lambda g_{\mu \nu} \right] \delta g^{\mu \nu} - \frac{1}{2} \int_\man \dd^d x \sqg T_{\mu \nu} \delta g^{\mu \nu} + \int_\man \dd^d x \frac{\delta \act_\text{mat}}{\delta \phi} \delta \phi = 0
    \end{equation*}
    which is the thesis.
  \end{proof}
\end{proofbox}
...
There is a difference between the energy-momentum tensor and the current which arises from a global symmetry. Consider a conserved current $ J^\mu : \na_\mu J^\mu = 0 $. Invoking the divergence theorem \eref{eq:gauss-theorem}:
\begin{equation*}
...
However, the same argument does not apply to the energy-momentum tensor: the problem arises from generalizing $ \na_\mu J^\mu = \frac{1}{\sqg} \pa_\mu (\sqg J^\mu) $ to a higher-order tensor field, which is necessary in order to have a divergence theorem like \eref{eq:gauss-theorem}. Indeed:
\begin{equation*}
