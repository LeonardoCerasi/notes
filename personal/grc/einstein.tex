...
The force of gravity is mediated by a gravitational field, which is identified with a metric $ g(x) $ on a $ n $-dimensional Lorentzian manifold. This metric is a dynamical object, as all other fields in Nature, thus the laws governing its dynamics must be studied.

\section{Geometrodynamics in vacuum}

Differential Geometry poses strict limits to the possible actions that can be formulated, ensuring it is something intrinsic to the metric and independent on the choice of coordinates.

\begin{definition}{Spacetime}{}
  An orientable Lorentzian manifold $ (\man , g) $ equipped with a torsion-free Levi--Civita connection is called a \bcdef{spacetime}.
\end{definition}

To define the action for the metric of a spacetime $ (\man , g) $, recalling the canonical volume form in \eref{eq:volume-form}, there needs to be a metric-dependent scalar function on $ \mathcal{M} $: the obvious non-trivial choice is the Ricci scalar $ \rr $. The resulting action is the \bctxt{Einstein--Hilbert action}:
\begin{equation}
  \act_\text{EH}[g] = \int_\man \dd^d x \sqg \, \rr
  \label{eq:einstein-hilbert-action}
\end{equation}
This is the simplest non-trivial action for the gravitational field allowed by Differential Geometry.
Schematically, $ \rr \sim \pa \Gamma + \Gamma \Gamma $ and $ \Gamma \sim \pa g $, so the Einstein--Hilbert Lagrangian appears to be dependent on second derivatives of the metric: however, the terms proportional to $ \pa^2 g $ can be shown to form a total divergence term, thus making $ \lag_\text{EH} $ ``effectively" first order, like most other actions in physics. The total divergence term needs to be treated carefully on manifolds with boundary, since a well-posed variational problem with Dirichlet boundary conditions (fixing $ g \vert_{\pa \man} $) requires vanishing normal derivatives of $ \delta g $ at the boundary $ \pa \man $: this is not satisfied by $ \act_\text{EH} $ alone, and the standard fix is to add a boundary term (the so-called \emph{Gibbons--Hawking--York boundary term} $ \act_\text{GHY} $) which ensured $ \delta g \vert_{\pa \man} = 0 $. In the following, the inclusion of a GHY term in the action $ \act_\text{grav} = \act_\text{EH} + \act_\text{GHY} $ is made implicit by setting $ \delta g \vert_{\pa \man} = 0 $.

\begin{observation}{Dimensionale analysis}{dimensional-analysis}
  ...
  If $ x $ has dimension of length, then $ g $ is dimensionless and the Ricci scalar is $ [\rr] = L^{-2} $. Including the integration measure, $ [\act_\text{HE}] = L^{d - 2} $, but it must have the same dimension of $ [\hbar] =  M L^2 T^{-1} $ (i.e. energy $ \times $ time). Thus, the action with the right dimension is:
  \begin{equation}
    \act_\text{EH}[g] = \frac{c^3}{16 \pi \gd} \int_\man \dd^d x \sqg \, \rr
    \label{eq:einstein-hilbert}
  \end{equation}
  where the $ d $-dimensional Newton constant has dimension $ [\gd] = M^{-1} L^{d-1} T^{-2} $. In the following, natural units $ c = \hbar = k_\text{B} \equiv 1 $ are adopted, so that $ L \propto T \propto M^{-1} $: in particular, this means that the Newton constant has mass dimension $ d - 2 $, which makes any quantum gravitational theory non-renormalizable for $ d > 2 $.
\end{observation}

To determine the Euler--Lagrange equations of the Einstein--Hilbert action, consider a fixed initial metric $ g(x) $ and a perturbation $ g(x) \mapsto g(x) + \delta g(x) $.

\begin{lemma}{Metric variations}{metric-variations}
  Let $ (\man , g) $ be a metric manifold. Under a variation $ g(x) \mapsto g(x) + \delta g(x) $:
  \begin{equation}
    \delta g_{\mu \nu} \equiv (\delta g)_{\mu \nu}
    \qquad \quad
    \delta g^{\mu \nu} = - g^{\mu \alpha} g^{\nu \beta} \delta g_{\alpha \beta}
    \qquad \quad
    \delta \sqg = \frac{\sigma}{2} \sqg\, g_{\mu \nu} \delta g^{\mu \nu}
  \end{equation}
  \begin{equation}
    \delta {\Gamma^\rho_{\mu \nu}}\od1 = \frac{1}{2} g^{\rho \sigma} ( \na_\mu \delta g_{\sigma \nu} + \na_\nu \delta g_{\sigma \mu} - \na_\sigma \delta g_{\mu \nu} )
  \end{equation}
  \begin{equation}
    \delta \rr_{\mu \nu} = \na_\rho \delta \Gamma^\rho_{\mu \nu} - \na_\nu \delta \Gamma^\rho_{\rho \mu}
  \end{equation}
  where $ \sigma $ is the parity of the signature of $ g $.
\end{lemma}

\begin{proofbox}
  \begin{proof}
    $ \delta g_{\mu \nu} \equiv (\delta g)_{\mu \nu} $ is a definition. For the inverse metric:
    \begin{equation*}
      g_{\mu \nu} g^{\nu \rho} = \delta_\mu^\rho
      \quad \implies \quad
      \delta g_{\mu \nu} g^{\nu \rho} + g_{\mu \nu} \delta g^{\nu \rho} = 0
    ...
    \end{equation*}
    The variation of $ \sqg $ requires a classical result for inverticle matrices: $ \log \det \mt{A} = \tr \log \mt{A} $, i.e. $ (\det \mt{A})^{-1} \delta (\det \mt{A}) = \tr (\mt{A}^{-1} \delta \mt{A}) $. Applying this to the metric:
    \begin{equation*}
      \delta \sqg = \frac{1}{2} \frac{1}{\sqg} \delta \mt{g} = \frac{1}{2} \frac{1}{\sqg} (\sigma \mt{g}) g^{\mu \nu} \delta g_{\mu \nu} = \frac{\sigma}{2} \sqg\, g^{\mu \nu} \delta g_{\mu \nu} = \frac{\sigma}{2} \sqg\, g_{\mu \nu} \delta g^{\mu \nu}
    \end{equation*}

    Now, consider the Christoffel symbols, and in particular note that, although $ \Gamma^\rho_{\mu \nu} $ is not a tensor, $ \delta \Gamma^\rho_{\mu \nu} $ is, since it is the difference of two Christoffel symbols, one computed with $ g_{\mu \nu} $ and the other with $ g_{\mu \nu} + \delta g_{\mu \nu} $, and the extra term in the transformation law of $ \Gamma^\rho_{\mu \nu} $ is independent of the metric, thus it cancels out. This observation allows to work in normal coordinates at $ p \in \mathcal{M} $, so that $ \pa_\rho g_{\mu \nu} \vert_p = 0 $ and $ \Gamma^\rho_{\mu \nu} \vert_p = 0 $. Then, to linear order in the variation:
    \begin{equation*}
      {\delta \Gamma^\rho_{\mu \nu}}\od1 \vert_p = \frac{1}{2} g^{\rho \sigma} ( \pa_\mu \delta g_{\sigma \nu} + \pa_\nu \delta g_{\sigma \mu} - \pa_\sigma \delta g_{\mu \nu} ) \vert_p = \frac{1}{2} g^{\rho \sigma} ( \na_\mu \delta g_{\sigma \nu} + \na_\nu \delta g_{\sigma \mu} - \na_\sigma \delta g_{\mu \nu} )
    \end{equation*}
    This is a tensor equation, hence valid in all coordinate system and, by the arbitrariness of $ p $, on all $ \mathcal{M} $.
    In normal coordinates, the Riemann tensor becomes $ \tensor{\rr}{^\sigma_{\mu \rho \nu}} \vert_p = \pa_\rho \Gamma^\sigma_{\nu \mu} \vert_p - \pa_\nu \Gamma^\sigma_{\rho \mu} \vert_p $, so:
    \begin{equation*}
      \delta \tensor{\rr}{^\sigma_{\mu \rho \nu}} \vert_p = \pa_\rho \delta \Gamma^\sigma_{\nu \mu} \vert_p - \pa_\nu \delta \Gamma^\sigma_{\rho \mu} \vert_p = \na_\rho \delta \Gamma^\sigma_{\nu \mu} \vert_p - \na_\nu \delta \Gamma^\sigma_{\rho \mu} \vert_p
    \end{equation*}
    This is a tensor equation, hence again valid in all coordinates systems and on all $ \mathcal{M} $. Contracting indices $ \sigma,\rho $ yields the thesis.
  \end{proof}
\end{proofbox}

With these expressions it is now possible to compute the variations of the Einstein--Hilbert action to find the equations of motion of the gravitational field.
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \delta \act_\text{EH}[g] = \delta \int_\man \dd^d x \sqg\, g^{\mu \nu} \rr_{\mu \nu} = \int_\man \dd^d x \left[ (\delta \sqg) g^{\mu \nu} R_{\mu \nu} + \sqg (\delta g^{\mu \nu}) \rr_{\mu \nu} + \sqg\, g^{\mu \nu} (\delta \rr_{\mu \nu}) \right] \\
    \end{equation*}
    Inserting the expressions in \lref{lemma:metric-variations}:
    \begin{equation*}
      \delta \act_\text{EH}[g] = \int_\man \dd^d x \sqg \left[ \left( - \frac{1}{2} \rr g_{\mu \nu} + \rr_{\mu \nu} \right) \delta g^{\mu \nu} + g^{\mu \nu} \left( \na_\rho \delta \Gamma^\rho_{\mu \nu} - \na_\nu \Gamma^\rho_{\rho \mu} \right) \right]
    \end{equation*}
    Then, using the metric compatibility of the Levi--Civita connection:
    \begin{equation*}
      \act_\text{EH}[g] = \int_\man \dd^d x \sqg \left[ \left( \rr_{\mu \nu} - \frac{1}{2} \rr g_{\mu \nu} \right) \delta g^{\mu \nu} + \na_\mu \left( g^{\rho \nu} \delta \Gamma^\mu_{\rho \nu} - g^{\mu \nu} \delta \Gamma^\rho_{\rho \nu} \right) \right]
    \end{equation*}
    The last term is a total derivative, hence by the divergence theorem \eref{eq:gauss-theorem} it yields a boundary term which vanishes as $ \delta g \vert_{\pa \man} = 0 $. The Euler--Lagrange equations are then found imposing $ \delta \act_\text{HE} = 0 $ for arbitrary $ \delta g $, so recalling the definition of the Einstein tensor \eref{eq:einstein-def} the proof is completed.
  \end{proof}
\end{proofbox}

These equations are the Einstein field equations in vacuum, which govern the geometrodynamics of spacetime in the absence of any matter. For this reason, they can be further simplified with a contraction: in general, the trace of the Einstein tensor is:
\begin{equation}
  G = \frac{2 - d}{2} \rr
\end{equation}
This means that vacuum spacetimes of dimension $ d \neq 2 $ are Ricci-flat. Metric $ 2 $-manifolds are instead trivialized by the existence of isothermal coordinates (see Ex. 7.9 in \S 7.6.2 of \cite{nakahara}).

While a constant gravitational Lagrangian has no physical relevance, as it uniquely implies $ g \equiv 0 $, \eref{eq:einstein-hilbert} allows for the addition of a constant term to the Ricci scalar. An interesting expression is:
\begin{equation}
  \act_\text{grav}[g] = \frac{1}{16 \pi \gd} \int_\man \dd^d x \sqg \left[ \rr - (d - 2) \Lambda \right]
\end{equation}
$ \Lambda \in \R $ is known as a \emph{cosmological constant} and has dimension $ [\Lambda] = L^{-2} $. The resulting field equations are:
\begin{equation}
  \rr_{\mu \nu} - \frac{1}{2} \rr g_{\mu \nu} = - \frac{d-2}{2} \Lambda g_{\mu \nu}
\end{equation}
Contracting with $ g^{\mu \nu} $ yields $ \rr = \frac{2d}{d-2} \Lambda $, thus in the absence of matter:
\begin{equation}
  \rr_{\mu \nu} = \Lambda g_{\mu \nu}
\end{equation}
Manifolds on which $ \ricc = \kappa g $ for a constant $ \kappa \in \R $ are known as \bctxt{Einstein manifolds}: clearly, Ricci-flat manifolds are a special class of Einstein manifolds with $ \kappa = 0 $. Notice that Einstein manifolds have constant Ricci scalar curvature $ \rr = \kappa d $.

Note that the Einstein field equations are not linear in the metric, hence the superposition principle is not valid for gravitational fields, except at NLO in the approximation of weak gravity (see \secref{ssec:weak-gravity}).

\subsection{Symmetries}

The Einstein equations are highly non-linear, hence they are extremely hard to solve. In the study of their solutions, a fundamental role is played by symmetries. For example, consider the simplest solution, i.e. Minkowski spacetime: its symmetries are familiar, namely translations and rotations of spacetime (with the latter splitting between proper rotations and Lorentz boosts), whose existence allows to define conserved energy, momentum and angular momentum on a fixed Minkowski background. It is then important to characterize the symmetries of a general metric.

Being the metric a symmetric $ \R^{d \times d} $ matrix, it should have $ \frac{1}{2} d (d+1) $ degrees of freedom. However, not all the components of $ g_{\mu \nu} $ are physical. Indeed, two metrics related by a change of coordinates $ x^\mu \mapsto \tilde{x}^\mu(x) $ describe the same physical spacetime, thus there is a redundancy in any given representation of the metric which removes precisely $ d $ degrees of freedom, leaving only $ \frac{1}{2} d (d-1) $ physical degrees of freedom.

Formally, a diffeomorphism $ \phi : \mathcal{M} \rightarrow \mathcal{M} $ maps all fields on $ \mathcal{M} $ to a new set of fields on $ \mathcal{M} $: the result is physically indistinguishable from the original, describing the same spacetime but in different coordinates. Such diffeomorphisms are analogous to gauge symmetries in Yang-Mills theories.

Consider a diffeomorphism $ x^\mu \mapsto \tilde{x}^\mu = x^\mu + \delta x^\mu $: this can be viewed as an active diffeomorphism, where points in spacetime are mapped from one another, or as a passive diffeomorpshim, in which only the coordinates of each point are affected, but the two interpretations are equivalent.

\begin{lemma}{Transformation of tensors under infinitesimal diffeomorphisms}{tens-inf-diff}
  Let $ (\man,g) $ be a metric manifold, $ Z \in \xm $ and $ T \in \tns{r}{s}(\man) $. Given a smooth chart $ (U,(x^\mu)) $ on $ \man $, under a coordinate change $ x^\mu \mapsto \tilde{x}^\mu $, define the \bclemma{pointwise variation} of $ T $ as:
  \begin{equation}
    \delta T(x) \defeq \tilde{T}(x) - T(x)
  \end{equation}
  Then, if $ \tilde{x}^\mu(x) = x^\mu - Z^\mu(x) $, the pointwise variation of $ T $ is:
  \begin{equation}
     \delta T(x) = \ld_Z T(x) + \smo(Z^2)
  \end{equation}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    By \eref{eq:tensor-trans}:
    \begin{equation*}
      \tilde{T}^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(\tilde{x}) = \frac{\pa \tilde{x}^{\mu_1}}{\pa x^{\alpha_1}} \dots \frac{\pa x^{\mu_r}}{\pa x^{\alpha_r}} \frac{\pa x^{\beta_1}}{\pa \tilde{x}^{\nu_1}} \dots \frac{\pa x^{\beta_s}}{\pa \tilde{x}^{\nu_s}} T^{\alpha_1 \dots \alpha_r}_{\beta_1 \dots \beta_s}(x)
    \end{equation*}
    Now, note that\footnote{The second relation needs proof.}: % TODO: proof of the second relation
    \begin{equation*}
      \frac{\pa \tilde{x}^\mu}{\pa x^\alpha} = \delta^\mu_\alpha - \pa_\alpha Z^\mu
      \qquad \qquad
      \frac{\pa x^\beta}{\pa \tilde{x}^\nu} = \delta^\beta_\nu + \pa_\nu Z^\beta + \smo(Z^2)
    \end{equation*}
    so that:
    \begin{equation*}
      \tilde{T}^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(\tilde{x}) = T^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(x) - \sum_{i = 1}^r T^{\mu_1 \dots \rho \dots \mu_r}_{\nu_1 \dots \nu_l}(x) \pa_\rho Z^{\mu_i}(x) + \sum_{j = 1}^s T^{\mu_1 \dots \mu_r}_{\nu_1 \dots \rho \dots \nu_s}(x) \pa_{\nu_j} Z^\rho(x) + \smo(Z^2)
    \end{equation*}
    On the other hand:
    \begin{equation*}
      \tilde{T}^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(\tilde{x}) = \tilde{T}^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(x) - Z^\rho \pa_\rho \tilde{T}^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(x) = \tilde{T}^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(x) - Z^\rho \pa_\rho T^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(x) + \smo(Z^2)
    \end{equation*}
    Therefore, by definition:
    \begin{equation*}
      \begin{split}
        \delta T^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(x)
        & = Z^\rho \pa_\rho T^{\mu_1 \dots \mu_r}_{\nu_1 \dots \nu_s}(x) - \sum_{i = 1}^r T{\mu_1 \dots \rho \dots \mu_r}_{\nu_1 \dots \nu_l}(x) \pa_\rho Z^{\mu_i}(x) \\
        & \qquad \qquad \qquad \qquad \qquad \qquad \qquad + \sum_{j = 1}^s T^{\mu_1 \dots \mu_r}_{\nu_1 \dots \rho \dots \nu_s}(x) \pa_{\nu_j} Z^\rho(x) + \smo(Z^2)
      \end{split}
    \end{equation*}
    which is precisely \eref{eq:tensor-lie-der}.
  \end{proof}
\end{proofbox}

By \lref{lemma:tens-inf-diff}, at first order in $ Z $ the metric transforms as:
\begin{equation}
  \delta g_{\mu \nu} = (\ld_Z g)_{\mu \nu}
  \label{eq:lie-der-metric}
\end{equation}
Thus, an infinitesimal diffeomorphism along $ Z \in \xm $ makes the metric change by an infinitesimal amount given by its Lie derivative along $ Z $, which can be viewed as the leading term in the Taylor expansion along $ Z $. These equations can be rewritten with \eref{eq:tensor-lie-der}:
\begin{equation*}
  \delta g_{\mu \nu} = Z^\rho \pa_\rho g_{\mu \nu} + g_{\rho \nu} \pa_\mu Z^\rho + g_{\mu \rho} \pa_\nu Z^\rho
\end{equation*}
Using the metric compatibility $ \na_\rho g_{\mu \nu} = 0 $ of the Levi--Civita connection, from which it follows that $ \pa_\rho g_{\mu \nu} = \Gamma^\lambda_{\rho \mu} g_{\lambda \nu} + \Gamma^\lambda_{\rho \nu} g_{\mu \lambda} $, this expression becomes:
\begin{equation*}
  \delta g_{\mu \nu} = g_{\mu \rho} ( \pa_\nu Z^\rho + \Gamma^\rho_{\nu \lambda} Z^\lambda ) + g_{\rho \nu} ( \pa_\mu Z^\rho + \Gamma^\rho_{\lambda \nu} Z^\lambda ) = g_{\mu \rho} \na_\nu Z^\rho + g_{\rho \nu} \na_\mu Z^\rho
\end{equation*}
Using again the metric compatibility, the metric factors can be brought inside the covariant derivatives, yielding a simple expression:
\begin{equation}
  \delta g_{\mu \nu} = \na_\mu Z_\nu + \na_\nu Z_\mu
  \label{eq:metric-variation}
\end{equation}
This equation can be used in the path integral. In fact, insisting that $ \delta \act = 0 $ for any $ \delta g_{\mu \nu} $ gives the equations of motion; on the other hand, those variations $ \delta g_{\mu \nu} $ for which $ \delta \act = 0 $ for any metric are the \textit{symmetries} of the action. From \pref{prop:prop-einst-eq}:
\begin{equation*}
  \delta \act_\text{EH}[g] = \int_\man \dd^d x \sqg\, G^{\mu \nu} \delta g_{\mu \nu} = 2 \int_\man \dd^d x \sqg\, G^{\mu \nu} \na_\mu Z_\nu
\end{equation*}
Invariance for $ x^\mu \mapsto x^\mu - Z^\mu $ means that $ \delta \act = 0 $ for all $ Z \in \xm $, hence, integrating by parts:
\begin{equation}
  \na_\mu G^{\mu \nu} = 0
  \label{eq:bianchi-id}
\end{equation}
This is the Bianchi identity: although it is a result from Differential Geometry, is follows from diffeomorphism invariance of the Einstein--Hilbert action. This identity contains 4 equations, which make the 10 Einstein equations not completely independent: in fact, only 6 of them are independent, the same number of independent components of the metric, thus ensuring that the field equations are not over-determined.

\subsubsection{Isometries}
\label{ssec:isometries}

With \eref{eq:metric-variation}, it is possible to find those vector fields which identically have $ \delta g_{\mu \nu} \equiv 0 $, i.e. those under whose flow the metric tensor remains unchanged.

\begin{definition}{Isometry}{isometry}
  Let $ (\man , g) $ be a metric manifold. The flow associated to $ \xi \in \xm $ is an \bcdef{isometry} if:
  \begin{equation}
    \ld_\xi g = 0
  \end{equation}
\end{definition}

By \eeref{eq:lie-der-metric}{eq:metric-variation}, in a spacetime this condition can be specialized to a local frame as:
\begin{equation}
  \na_\mu \xi_\nu + \na_\nu \xi_\mu = 0
  \label{eq:killing}
\end{equation}
\dref{def:isometry} means that the metric does not change along flow lines: this is called \textit{Killing equation}, and a vector field which satisfied it is a \bctxt{Killing vector field}.
It can be shown that the Killing vector fields of a manifold form a Lie algebra: this is the Lie algebra of the isometry group of the manifold, which is thus generated by its Killing vector fields.

Given a Killing vector field $ \xi \in \xm $, it is always possible to find a set of coordinates $ (x^0 , x^1 , \dots , x^{d-2} , y) $ on $ \man $ such that $ \xi = \frac{\pa}{\pa y} $, known as \emph{adapted coordinates}, in which the Killing equation simply reads:
\begin{equation*}
  \frac{\pa}{\pa y} g_{\mu \nu} = 0
\end{equation*}
which is the case of an ignorable degree of freedom. It is possible to reframe Noether's theorem in the context of Killing vectors. Indeed, the presence of a Killing vector field gives rise to a conserved quantity.

\begin{theorem}{Noether's theorem}{noether-killing}
  Let $ (\man,g) $ be a spacetime, and suppose that it admits a Killing vector field $ \xi \in \xm $. Then, given an affinely-parametrized geodesic with tangent vector field $ U $, it is possible to define a \bcth{Noether charge} associated to $ \xi $ as:
  \begin{equation}
    Q_\xi \defeq \braket{U , \xi}_g
  \end{equation}
  which is conserved along the geodesic, i.e. $ \na_U Q_\xi = 0 $.
\end{theorem}

\begin{proofbox}
  \begin{proof}
    Since the geodesic is affinely-parametrized, $ \na_U U = 0 $, hence by direct computation:
    \begin{equation*}
      \begin{split}
        \na_U Q_\xi
        & = \na_U \braket{U , \xi}_g = \braket{\na_U U , \xi}_g + \braket{U , \na_U \xi}_g \\
        & = \braket{U , \na_U \xi}_g = U^\mu U^\nu \na_\nu \xi_\mu = U^\mu U^\nu \na_{(\nu} \xi_{\mu)} = 0
      \end{split}
    \end{equation*}
...
The interpretation of the Noether charge depends on the nature of the associated Killing vector field, e.g. if $ \xi $ is globally timelike then $ Q_\xi $ can be interpreted as an energy.

\begin{example}{Static spherically-symmetric spacetime}{}
  Consider a static spherically-symmetric spacetime with metric:
  \begin{equation*}
    \dd s^2 = - h(r) \dd t^2 + h(r)^{-1} \dd r^2 + r^2 \dd \Omega_2^2
  \end{equation*}
  There are $ 4 $ Killing vector fields: $ \frac{\pa}{\pa t} $, since $ t $ is a cyclic variable, and the $ 3 $ Killing vector fields, among which is $ \frac{\pa}{\pa \varphi} $, associated to the spatial isometry subgroup $ \SOn{3} $.

  Consider a generic geodesic $ \gamma $ with tangent vector field $ u $, with $ u^2 = -1 $ (timelike) or $ u^2 = 0 $ (null). By Noether's theorem \tref{th:noether-killing}, then, $ \braket{u , \frac{\pa}{\pa t}}_g $ is constant along $ \gamma $:
  \begin{equation*}
    \left\langle u , \frac{\pa}{\pa t} \right\rangle_g = u^\mu g_{\mu 0} = - u^0 h
  \end{equation*}
  This expression resembles that for the energy of a particle with $ 4 $-momentum $ p $ measured by an observer with $ 4 $-velocity $ v $:
  \begin{equation}
    E_\text{local} = - \braket{p , v}_g
  \end{equation}
  Indeed, the $ 4 $-velocity of a stationary ($ r , \vartheta , \varphi = \text{const.} $) observer at infinity is just $ v = \frac{\pa}{\pa t} $, so assuming a timelike geodesic with $ u^\mu = \frac{1}{m} p^\mu $:
  \begin{equation*}
    E_\infty = - \braket{p , v} = - m \left\langle u , \frac{\pa}{\pa t} \right\rangle_g
  \end{equation*}
  Therefore, the Noether charge associated to $ \frac{\pa}{\pa t} $ is the energy per unit mass measured at infinity, which is constant along timelike geodesics. Equivalently, $ \braket{u , \frac{\pa}{\pa \varphi}}_g $ turns out to be the particle's angular momentum per unit mass measured at infinity.
\end{example}

\subsubsection{Maximally symmetric spacetimes}

Starting from the Killing equation, it is possible to further characterize Killing vector fields with the curvature tensor.

\begin{lemma}{Killing vectors and Riemann tensor}{}
  Let $ (\man,g) $ be a metric manifold with a torsion-free Levi--Civita connection, and let $ \xi \in \xm $ be a Killing vector field. Then:
  \begin{equation}
    \na_\nu \na_\alpha \xi_\mu = - \tensor{\rr}{_{\alpha \mu \nu}^\beta} \xi_\beta
    \label{eq:killing-riemann}
  \end{equation}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    By the Ricci identity \eref{eq:ricci-id} for a torsion-free connection:
    \begin{equation*}
      \na_\mu \na_\nu \xi_\alpha - \na_\nu \na_\mu \xi_\alpha = \tensor{\rr}{_{\alpha \beta \mu \nu}} \xi^\beta = \tensor{\rr}{_{\mu \nu \alpha}^\beta} \xi_\beta
    \end{equation*}
    where the first symmetry of \eref{eq:riemann-symm} has been used. By the Killing equation $ \na_\nu \na_\mu \xi_\alpha = - \na_\nu \na_\alpha \xi_\mu $, so:
    \begin{equation*}
      \na_\mu \na_\nu \xi_\alpha + \na_\nu \na_\alpha \xi_\mu = \tensor{\rr}{_{\mu \nu \alpha}^\beta} \xi_\beta
    \end{equation*}
    Performing cyclic permutations of indices, a set of three equations is found (including the previous one):
    \begin{equation*}
      \na_\mu \na_\nu \xi_\alpha + \na_\nu \na_\alpha \xi_\mu = \tensor{\rr}{_{\mu \nu \alpha}^\beta} \xi_\beta
    \end{equation*}
    \begin{equation*}
      \na_\nu \na_\alpha \xi_\mu + \na_\alpha \na_\mu \xi_\nu = \tensor{\rr}{_{\nu \alpha \mu}^\beta} \xi_\beta
    \end{equation*}
    \begin{equation*}
      \na_\alpha \na_\mu \xi_\nu + \na_\mu \na_\nu \xi_\alpha = \tensor{\rr}{_{\alpha \mu \nu}^\beta} \xi_\beta
    \end{equation*}
    Summing the first two and subtracting the last one:
    \begin{equation*}
      2 \na_\nu \na_\alpha \xi_\mu = (\tensor{\rr}{_{\mu \nu \alpha}^\beta} + \tensor{\rr}{_{\nu \alpha \mu}^\beta} - \tensor{\rr}{_{\alpha \mu \nu}^\beta}) \xi_\beta
    \end{equation*}
    The Bianchi identity $ \tensor{\rr}{_{[\mu \nu \alpha]}^\beta} = 0 \implies \tensor{\rr}{_{\mu \nu \alpha}^\beta} + \tensor{\rr}{_{\nu \alpha \mu}^\beta} = - \tensor{\rr}{_{\alpha \mu \nu}^\beta} $ completes the proof.
  \end{proof}
\end{proofbox}

\ceref{eq:killing}{eq:killing-riemann} can be used to determine Killing vector fields: starting from a point $ p \in \man $ and moving along a curve, initial conditions are imposed on $ \xi^\mu \vert_p $ and $ \na_\mu \xi_\nu \vert_p $, however the Killing equation reduces the number of independent initial conditions, since only those on $ \na_{[\mu} \xi_{\nu]} \vert_p $ are relevant. On an $ n $-dimensional Lorentzian manifold $ \man $, then, the maximum number of independent Killing vector fields (i.e. of independent initial conditions) is:
\begin{equation}
  N_\text{max} = d + \frac{d (d-1)}{2} = \frac{d (d+1)}{2}
\end{equation}
A spacetime which admits $ N_\text{max} $ Killing vector fields is said to be \bctxt{maximally symmetric}. These $ N_\text{max} $ Killing vector fields split between generators of translations and rotations: indeed, in an $ d $-dimensional space there are $ d $ axes along which to define translations and $ \frac{1}{2} d (d-1) $ planes in which to define rotations, thus reproducing the above split between initial conditions.

\begin{proposition}{Riemann tensor of maximally symmetric spacetime}{}
  The curvature tensor of a maximally symmetric spacetime can be written as:
  \begin{equation}
    \rr_{\mu \nu \alpha \beta} = \lambda (g_{\mu \alpha} g_{\nu \beta} - g_{\nu \alpha} g_{\mu \beta})
    \qquad
    \lambda \in \R
  \end{equation}
\end{proposition}
...
\begin{proofbox}
  \begin{proof}
    In a maximally symmetric spacetime the curvature is independent under all possible translations and rotations, hence choose normal coordinates at an arbitrary $ p \in \man $, so that $ g_{\mu \nu} \vert_p = \eta_{\mu \nu} $. These coordinates are not unique, since any Lorentz transformation leaves the metric components unchanged and equal to $ \eta_{\mu \nu} $ (by definition of Lorentz transformation, recall \obsref{obs:vielbein-lorentz}). Lorentz invariance must then be imposed on the Riemann tensor: this means that it can only be expressed in terms of Lorentz invariant tensors, which uniquely are the Lorentz--Minkowski metric, the Kronecker delta and the Levi--Civita tensor. The only combination of these tensor which matches the symmetries of the Riemann tensor (\pref{prop:riemann-symmetries}) is:
    \begin{equation*}
      \rr_{\mu \nu \alpha \beta} \vert_p \propto \eta_{\mu \alpha} \eta_{\nu \beta} - \eta_{\mu \beta} \eta_{\nu \alpha}
    \end{equation*}
    Replacing $ \eta_{\mu \nu} $ with $ g_{\mu \nu} \vert_p = \eta_{\mu \nu} $ turns this into a tensorial equation, valid on all of $ \man $ by the arbitrariness of $ p $, hence the thesis.
  \end{proof}
\end{proofbox}

The constant $ \lambda \in \R $ has dimensions of $ [\lambda] = L^{-2} $. The Ricci tensor and scalar are consequently expressed by:
\begin{equation}
  \rr_{\mu \nu} = (d-1) \lambda g_{\mu \nu}
  \qquad \qquad
  \rr = d(d-1) \lambda
\end{equation}
Therefore, maximally symmetric spacetimes are Einstein manifolds, and as such have constant Ricci scalar curvature. An explicit construction of maximally symmetric solutions to \eref{eq:einstein-def} is given in \secref{ssec:maximally-symmetric-solutions}.

\subsubsection{Komar integrals}

Noether's theorem is not the only way to associate conserved quantities to Killing vector fields in matter-less spacetimes.

\begin{definition}{Komar form}{}
  Let $ (\man , g) $ be a spacetime, and let $ \xi \in \xm $ be a Killing vector field. The \bcdef{Komar charge} $ \ve{K}[\xi] \in \df{d-2} $ associated to $ \xi $ is defined as:
  \begin{equation}
    \ve{K}[\xi] \defeq - \frac{1}{16 \pi \gd} \star \dd \xi^\flat
    \label{eq:komar-form}
  \end{equation}
\end{definition}

The Killing $ 2 $-form $ \dd \xi^\flat $ is a gravitational generalization of the field-strength tensor of a Yang--Mills theory (see \secref{sec:maxwell} for the $ \Un{1} $ case). Indeed, $ \Gamma^\rho_{[\mu \nu]} = 0 $ for torsion-free connections, hence:
\begin{equation*}
  \dd \xi^\flat = \pa_\mu \xi_\nu \dd x^\mu \wedge \dd x^\nu = \na_{[\mu} \xi_{\nu]} \dd x^\mu \wedge \dd x^\nu
\end{equation*}

\begin{theorem}{Gravitational Maxwell equations}{}
  Let $ (\man , g) $ be a Ricci-flat spacetime, and let $ \xi \in \xm $ be a Killing vector field. Then:
  \begin{equation}
    \dd \ve{K}[\xi] = 0
  \end{equation}
\end{theorem}
...
\begin{proofbox}
  \begin{proof}
    To prove this result, it is convenient to work in a local frame, so that, by \pref{prop:maxwell-equation-forms}, the thesis is equivalent to $ \na^\mu X_{\mu \nu} = 0 $, where $ \dd \xi^\flat = \frac{1}{2} X_{\mu \nu} \dd x^\mu \wedge \dd x^\nu $.

    Contracting $ \mu , \nu $ in \eref{eq:killing-riemann} yields $ \na_\mu \na_\alpha \xi^\mu = \rr_{\alpha \beta} \xi^\beta $, so using the Killing equation $ \na_{\mu} \xi_\nu = - \na_\nu \xi_\mu $:
    \begin{equation*}
      \na^\mu X_{\mu \nu} = \na^\mu (\na_\mu \xi_\nu - \na_\nu \xi_\mu) = - 2 \na^\mu \na_\nu \xi_\mu = -2 \rr_{\nu \beta} \xi^\beta
    \end{equation*}
    For a Ricci-flat spacetime $ \rr_{\mu \nu} \equiv 0 $, hence the thesis.
  \end{proof}
\end{proofbox}

The fact that Komar forms are conserved (i.e. closed) in Ricci-flat spacetimes is often frased as the fact that they are ``on-shell closed", as they require the vacuum field equations to be satisfied in order to be closed.
Like other on-shell closed $ (d-2) $-forms, the integral of the Komar form $ \ve{K}[\xi] $ over a suitable spatial $ (d-2) $-hypersurface gives the (gravitational) charge associated with the Killing vector field $ \xi $.

\begin{definition}{Komar charge}{}
  Let $ (\man , g) $ be a spacetime, and let $ \xi \in \xm $ be a Killing vector field. Given a closed oriented spacelike $ (d-2) $-submanifold $ S \ssq \man $, the \bcdef{Komar charge} associated to $ \xi $ in $ S $ is define as:
  \begin{equation}
    Q_\text{K}[\xi ; S] \defeq \int_S \ve{K}[\xi]
    \label{eq:komar-charge-s}
  \end{equation}
\end{definition}

For asymptotically flat spacetimes, a $ (d-2) $-sphere at spatial infinity $ \Sn{d-2}_\infty $ is usually chosen, resulting in the so-called \emph{Komar integral} of $ \xi $:
\begin{equation}
  Q_\text{K}[\xi] \equiv \int_{\Sn{d-2}_\infty} \ve{K}[\xi] = - \frac{1}{16 \pi \gd} \int_{\Sn{d-2}_\infty} \star \dd \xi^\flat
  \label{eq:komar-charge}
\end{equation}

As for Noether integrals of a particle, Komar integrals of spacetime are interpreted based on the defining Killing vector. For example, if $ \xi $ is everywhere timelike, then its Komar integral can be identified with the energy (or the mass) of spacetime; if the Killing vector is related to rotations, instead, the conserved charge is the angular momentum of spacetime.

Note that the sign of \eref{eq:komar-form} and \eref{eq:komar-charge-s} is dictated by the convention used for the Hodge dual \eref{eq:hodge-alibi}: if $ \varepsilon_{j_{p+1} \dots j_n j_1 \dots j_p} $ is used in place of $ \varepsilon_{j_1 \dots j_p} $, as in \cite{ballesteros}, then the correct sign is $ (-1)^{d-1} $.

\subsection{Maximally symmetric solutions}
\label{ssec:maximally-symmetric-solutions}

% TODO: must rewiew

The Einstein equations in the absence of matter reduce to $ \rr_{\mu \nu} = \Lambda g_{\mu \nu} $, with $ \Lambda \in \R $. In the case of maximally symmetric solutions, $ \Lambda = (n-1) R $, and the possible solutions split into three classes: $ \Lambda = 0 $ (Minkowski), $ \Lambda > 0 $ (de Sitter) and $ \Lambda < 0 $ (anti-de Sitter).

\subsubsection{Minkowski space}

The simplest case is $ \Lambda = 0 $. The Einstein equations then reduce to $ \rr_{\mu \nu} = 0 $, with the condition of $ g_{\mu \nu} $ being non-degenerate, since it is a metric and the field equations require the existence of the inverse metric $ g^{\mu \nu} $. This restriction is physically unusual: it is not a holonomic constraint on the physical degrees of freedom, but an inequality $ \det g < 0 $ (with required signature $ (-,+,+,+) $) which is not found for other fields of the Standard Model.

% TODO: add a definition of holonomic constraint as a footnote

The simplest Ricci flat metric is the Lorentz--Minkowski metric $ \dd s^2 = -\dd t^2 + \dd \ve{x}^2 $ on $ \R^{1,d-1} $, but it is not the only one: another example is the Schwarzschild metric (\secref{sec:schwarz-sol}).

\subsubsection{de Sitter space}
\label{ssec:dS-space}

Consider now $ \Lambda > 0 $ and $ d = 4 $. A static spherically-symmetric ansatz is:
\begin{equation*}
...
First, one needs to compute the Ricci tensor for this metric: a simple way is using the curvature form. The non-coordinate 1-forms satisfy $ \dd s^2 = \eta_{ab} \theta^a \otimes \theta^b $, thus:
\begin{equation*}
  \theta^0 = f \dd t
  \qquad
  \theta^1 = f^{-1} \dd r
  \qquad
  \theta^2 = r \dd \vartheta
  \qquad
  \theta^3 = r \sin \vartheta \dd \varphi
\end{equation*}
\begin{equation*}
  \dd\theta^0 = f' \dd r \wedge \dd t
  \qquad
  \dd\theta^1 = 0
  \qquad
  \dd\theta^2 = \dd r \wedge \dd \vartheta
  \qquad
  \dd\theta^3 = \sin \vartheta \dd r \wedge \dd \varphi + r \cos \vartheta \dd \vartheta \wedge \dd \varphi
\end{equation*}
The first Cartan structure relation \eref{eq:first-cartan-stru-eq}, together with \eref{eq:connection-form-symm}, allow to determine the connection 1-form. For example, the first equation is $ \tensor{\omega}{^0_1} = f' f \dd t = f' \dd \theta^0 $, and then $ \tensor{\omega}{^1_0} = \omega_{10} = - \omega_{01} = \tensor{\omega}{^0_1} $. Using the other structure equation, the only non-vanishing components of the connection 1-form are:
\begin{equation*}
...
\end{equation*}
The curvature 2-form can be computed from the second Cartan structure relation \eref{eq:second-cartan-stru-eq}. For example, $ \tensor{\Omega}{^0_1} = \dd \tensor{\omega}{^0_1} + \tensor{\omega}{^0_c} \wedge \tensor{\omega}{^c_1} $, but $ \tensor{\omega}{^0_c} \wedge \tensor{\omega}{^c_1} = \tensor{\omega}{^0_1} \wedge \tensor{\omega}{^1_1} = 0 $, thus $ \tensor{\Omega}{^0_1} = \dd \tensor{\omega}{^0_1} = ( (f')^2 + f'' f) \dd r \wedge \dd t $. From the curvature 2-form, the Riemann tensor can be computed via \eref{eq:curvature-2-def} (recall the anti-symmetries of the Riemann tensor), finding the only non-vanishing components:
\begin{equation*}
  \rr_{0101} = f f'' + (f')^2
  \qquad
  \rr_{0202} = \rr_{0303} = \frac{f f'}{r}
  \qquad
  \rr_{1212} = \rr_{1313} = - \frac{f f'}{r}
  \qquad
  \rr_{2323} = \frac{1 - f^2}{r^2}
\end{equation*}
To convert back to $ x^\mu = (t,r,\vartheta,\varphi) $, use $ \rr_{\mu \nu \rho \sigma} = \tensor{e}{^a_\mu} \tensor{e}{^b_\nu} \tensor{e}{^c_\rho} \tensor{e}{^d_\sigma} \rr_{abcd} $, which is particularly easy given that the matrices $ \tensor{e}{_a^\mu} $ which define the non-coordiante 1-forms are diagonal:
\begin{equation*}
  \rr_{trtr} = f(r) f''(r) + f'(r)^2
  \qquad
  \rr_{t \vartheta t \vartheta} = f(r)^3 f'(r) r
  \qquad
  \rr_{t \varphi t \varphi} = f(r)^3 f'(r) r \sin^2 \vartheta
\end{equation*}
\begin{equation*}
  \rr_{r \vartheta r \vartheta} = - \frac{f'(r) r}{f(r)}
  \qquad
  \rr_{r \varphi r \varphi} = - \frac{f'(r) r}{f(r)} \sin^2 \vartheta
  \qquad
  \rr_{\vartheta \varphi \vartheta \varphi} = (1 - f(r)^2) r^2 \sin^2 \vartheta
\end{equation*}
...
\begin{equation*}
  \rr_{tt} = - f(r)^4 R_{rr} = f(r)^3 \left[ f''(r) + \frac{2f'(r)}{r} + \frac{f'(r)^2}{f(r)} \right]
\end{equation*}
\begin{equation*}
  \rr_{\varphi \varphi} = \sin^2 \vartheta \rr_{\vartheta \vartheta} = (1 - f(r)^2 - 2 f(r) f'(r) r) \sin^2 \vartheta
\end{equation*}
...
\paragraph{de Sitter geodesics}

To interpret a metric, it is useful to study its geodesics. First, note that a non-trivial $ g_{tt}(r) $ term means that a particle won't remain at rest at $ r \neq 0 $, but it will be pushed to smaller values of $ g_{tt}(r) $, i.e. larger values of $ r $. The action of a particle in the general $ f(r) $ metric, parametrized by its proper time $ \sigma $, is:
\begin{equation}
  \mathcal{S} = \int \dd \sigma \left[ - f(r)^2 \dot{t}^2 + f(r)^{-2} \dot{r}^2 + r^2 (\dot{\vartheta}^2 + \sin^2 \vartheta \dd \dot{\varphi}^2) \right]
  \label{eq:dS-action}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:dS-ang-mom}
\end{equation}
The $ \frac{1}{2} $ are due to the absence of the usual factor in the kinetic term. The equations of motion from the action \eref{eq:dS-action} should be supplemented with a constraint to distinguish whether a particle is massive or massless. For a massive particle, the trajectory must be timelike, so:
\begin{equation*}
...
\paragraph{de Sitter embeddings}

de Sitter space can be embedded as a submanifold of $ \R^{1,4} $ with metric:
\begin{equation}
...
Finally, consider $ \Lambda < 0 $. With the same ansatz as for de Sitter space, it is easy to find the metric for \bctxt{anti-de Sitter space} (AdS):
\begin{equation}
...
To solve for the trajectory, it is easier to work with $ r = R \sinh \rho $ and $ \ell = 0 $, so that:
\begin{equation*}
...
\paragraph{Anti-de Sitter embeddings}
...
\begin{equation*}
  \pa_\mu K_\nu + \pa_\nu K_\mu = 0
\end{equation*}
There are two forms of solution. The first one is:
\begin{equation*}
  K_\mu = c_\mu
\end{equation*}
...
\end{equation*}
These are 10 Killing vectors, 4 for translations and 6 for rotations and boosts, which obey the Poincaré algebra:
\begin{equation*}
  [P_\mu, P_\nu] = 0
  \qquad
...
Energy is defined as the Noether charge of a timelike Killing vector. In anti-de Sitter space there is no problem finding a timelike Killing vector, as the metric in global coordinates \eref{eq:ads-metric} is time-independent, so $ K = \pa_t $. However, this is not the case in de Sitter space.
...
\end{equation*}
On the static patch, this Killing vector is timelike and the energy \eref{eq:dS-energy} follows from it. The problem is that the static patch is only one-half of the hyperboloid: when considering the whole AdS, one must account for the case $ X^0 = 0, X^4 < 0 $, in which the Killing vector points in the past direction, or $ X^0 \neq 0, X^4 = 0 $, in which it is spacelike. Therefore, the Killing vector can be both timelike (future-directed and past-directed) or spacelike when spanning the whole manifold: for this reason, it is not possible to define a global positive conserved energy on the total de Sitter space. The same conclusion follows by noting that the metric in global coordinates \eref{eq:ds-global-coo} is time-dependent. 
...
\begin{observation}{Field theories in curved spacetime}{}
  The simplest way to describe matter is by fields governed by a Lagrangian. Consider a scalar field $ \phi(x) $ in a flat Minkowski spacetime $ \R^{1,d-1} $, whose action is:
  \begin{equation}
    \act[\phi] \defeq \int_\man \dd^d x \left[ - \frac{1}{2} \eta^{\mu \nu} \pa_\mu \phi \pa_\nu \phi - V(\phi) \right]
  \label{eq:scalar-action}
  \end{equation}
  The negative sign of the kinetic term follows from the signature choice $ (-,+,+,+) $. The generalization to curved spacetime is straightforward with the minimal-coupling prescription: $ \pa_\mu \mapsto \na_\mu $ and $ \dd^d x \mapsto \omega_g \equiv \dd^d x \sqg $. Curved spacetime also introduces the possibility to add new terms to the Lagrangian, for example:
  \begin{equation}
    \act[\phi] = \int_\man \dd^d x \sqg \left[ - \frac{1}{2} g^{\mu \nu} \na_\mu \phi \na_\nu \phi - V(\phi) - \frac{1}{2} \kappa \rr \phi^2 \right]
    \label{eq:scalar-action-r}
  \end{equation}
  for some $ \kappa \in \R $ (renormalizable $ \forall d \in \N $). This theory correctly reduces to \eref{eq:scalar-action} in flat spacetime, as $ \rr = 0 $. To derive the equation of motion, vary $ \act $ with respect to $ \phi $:
  \begin{equation*}
    \begin{split}
      \delta \act[\phi]
      &= \int_\man \dd^d x \sqg \left[ - g^{\mu \nu} \na_\mu \delta \phi \na_\nu \phi - \frac{\pa V}{\pa \phi} \delta \phi - \kappa \rr \phi \delta \phi \right] \\
      &= \int_\man \dd^d x \sqg \left[ \left( g^{\mu \nu} \na_\mu \na_\nu \phi - \frac{\pa V}{\pa \phi} - \kappa \rr \phi \right) \delta \phi - \na_\mu ( \delta \phi \na^\mu \phi ) \right]
    \end{split}
  \end{equation*}
  where integration by parts was possible by the metric compatibility of the Levi--Civita connection. The last term is, by divergence theorem, a boundary term, thus the equation of motion is:
  \begin{equation*}
    g^{\mu \nu} \na_\mu \na_\nu \phi - \frac{\pa V}{\pa \phi} - \kappa \rr \phi = 0
  \end{equation*}
  Now covariant derivatives are necessary, as $ \na_\mu \na_\nu \neq \pa_\mu \pa_\nu $. It is useful to introduce the \bcobs{d'Alembert operator} $ \Box \defeq \na^\mu \na_\mu $, so that the equation of motion for the scalar field $ \phi $ becomes:
  \begin{equation}
    \Box \phi - \frac{\pa V}{\pa \phi} - \kappa \rr \phi = 0
  \end{equation}
\end{observation}

\subsection{Energy-momentum tensor}

In classical (and quantum) field theory in a Minkowski background, given a Poincarè-invariant Lagrangian $ \lag = \lag(\Phi , \pa \Phi) $, i.e. a Lagrangian invariant under $ \ISO{1,d-1} $, it is possible to define the \emph{canonical energy-momentum tensor} (C-EMT) as the Noether current associated to spacetime translations (see \S 1.3.1 of \cite{leonardo} for additional details):
\begin{equation}
  \tensor{\mathcal{T}}{^\mu_\nu} = \frac{\pa \lag}{\pa (\pa_\mu \Phi_a)} \pa_\nu \Phi_a - \delta^\mu_\nu \lag
\end{equation}
Like all Noether currents, the C-EMT is on-shell conserved, i.e. $ \pa_\mu \tensor{\mathcal{T}}{^\mu_\nu} = 0 $ if the equations of motion are satisfied.
The main problem of the C-EMT, however, is that it is not symmetric; moreover, its generalization to a generic curved spacetime is not obvious. For this reason, it is convenient to define another energy-momentum tensor starting from the action $ \act[\Phi] = \int_\man \dd^d x \sqg \lag(\Phi , \pa \Phi , g) $:
\begin{equation}
  T_{\mu \nu} \defeq - \frac{2}{\sqg} \frac{\delta (\sqg \lag)}{\delta g^{\mu \nu}}
  \label{eq:em-tens-def}
\end{equation}
This is the so-called \bctxt{Belinfante--Rosenfeld energy-momentum tensor} (BR-EMT), which is manifestly a symmetric $ (0,2) $-tensor, as it inherits the symmetries of the metric. It can be shown that, in a flat background, the C-EMT and the BR-EMT only differ by a total derivative:
\begin{equation}
  T^{\mu \nu} \vert_{g = \eta} = \mathcal{T}^{\mu \nu} + \pa_\alpha B^{\alpha \mu \nu}
\end{equation}
where the form of the potential $ B^{\alpha \mu \nu} $ depends on the nature of the field $ \phi $, and in particular on its spin. Indeed, if the field $ \Phi $ transforms under a finite-dimensional representation of the Lorentz group $ \SOp{1,d-1} $, with generators $ \tensor{[\mathcal{J}^{\mu \nu}]}{^a_b} $ obeying the Lorentz algebra\footnotemark, then the Belinfante potential has a universal expression in terms of the \emph{spin current} (see \S 7.4 of \cite{weinberg}):
\begin{equation}
  B^{\alpha \mu \nu} = \frac{\img}{2} \left( S^{\alpha \mu \nu} + S^{\mu \nu \alpha} + S^{\nu \mu \alpha} \right)
  \qquad \qquad
  S^{\alpha \mu \nu} \equiv \frac{\pa \lag}{\pa (\pa_\alpha \Phi^a)} \tensor{[\mathcal{J}^{\mu \nu}]}{^a_b} \Phi^b
\end{equation}
It can be shown that the combination $ \mathcal{T}^{\mu \nu} + \pa_\alpha B^{\alpha \mu \nu} $ is indeed symmetric, and the manifest antisymmetry $ B^{\alpha \mu \nu} = - B^{\mu \alpha \nu} $ ensures that $ \pa_\mu T^{\mu \nu} = \pa_\mu \mathcal{T}^{\mu \nu} $, i.e. that the BR-EMT is on-shell conserved too. Moreover, the addition of a total derivative does not affect the Noether charge $ P^\mu \equiv \int_\Sigma \dd^{d-1}x \mathcal{T}^{0\mu} $, since it only determines a vanishing spatial boundary term, hence the C-EMT and the BR-EMT encode the same dynamical information: for this reason, in the following $ T^{\mu \nu} $ is simply called the energy-momentum tensor (EMT).

\footnotetext{The Lorentz algebra $ \lorad $ is composed of the generators of $ d - 1 $ boosts (hyperbolic rotations) and $ \frac{1}{2} (d - 1) (d - 2) $ spatial rotations, hence it has $ \frac{1}{2} d (d - 1) $ generators $ \mathcal{J}^{\mu \nu} = - \mathcal{J}^{\nu \mu} $ satisfying:
\begin{equation}
  [\mathcal{J}^{\mu \nu} , \mathcal{J}^{\rho \sigma}] = \img \left( \eta^{\mu \sigma} \mathcal{J}^{\nu \rho} - \eta^{\mu \rho} \mathcal{J}^{\nu \sigma} - \eta^{\nu \sigma} \mathcal{J}^{\mu \rho} + \eta^{\nu \rho} \mathcal{J}^{\mu \sigma} \right)
  \label{eq:lorentz-algebra}
\end{equation}}

\subsubsection{Geometrodynamics sourced by matter}

To understand how matter fields back-react on the spacetime metric, consider the combined action:
\begin{equation}
  \act[g,\Phi] = \frac{1}{16\pi \gd} \int_\man \dd^d x \sqg \left( \rr - (d - 2) \Lambda \right) + \act_\text{mat}[g,\Phi]
  \label{eq:gravity-matter-action}
\end{equation}
where $ \act_\text{m} $ is the combined action for all matter fields (here collectively denoted by $ \Phi $), which in general depends on both the fields and the metric.

\begin{proposition}{Einstein field equations}{}
  The equations of motion determined by the action \eref{eq:gravity-matter-action} are:
  \begin{equation}
    G_{\mu \nu} + \Lambda g_{\mu \nu} = 8\pi \gd T_{\mu \nu}
    \qquad \qquad
    \frac{\delta \act_\text{mat}}{\delta \Phi} = 0
    \label{eq:ef-eq}
  \end{equation}
\end{proposition}
...
\begin{proofbox}
  \begin{proof}
    Varying the full action and inserting \eref{eq:em-tens-def}:
    \begin{equation*}
      \delta \act = \frac{1}{16\pi \gd} \int_\man \dd^d x \sqg \left[ G_{\mu \nu} + \Lambda g_{\mu \nu} \right] \delta g^{\mu \nu} + \int_\man \dd^d x \sqg \left[ - \frac{1}{2} T_{\mu \nu} \delta g^{\mu \nu} + \frac{\delta \lag_\text{mat}}{\delta \Phi} \delta \Phi \right] = 0
    \end{equation*}
    where matter fields are assumed to vanish asymptotically.
  \end{proof}
\end{proofbox}

These are the full Einstein field equations, describing gravity coupled to matter. It is possible to rewrite them by observing that the cosmological constant can be absorbed in the EMT as an additive component:
\begin{equation}
  (T_\Lambda)_{\mu \nu} = - \frac{\Lambda}{8\pi G} g_{\mu \nu}
  \label{eq:lambda-emt}
\end{equation}
This is justified by the fact that matter fields often mimic a cosmological constant. Taking the trace then gives $ \frac{2 - d}{2} \rr = 8\pi \gd T $, where $ T \equiv g^{\mu \nu} T_{\mu \nu} $, hence:
\begin{equation}
  \rr_{\mu \nu} = 8\pi \gd \left( T_{\mu \nu} - \frac{1}{d-2} T g_{\mu \nu} \right)
  \label{eq:ef-eq-alt}
\end{equation}
with cosmological constant absorbed in the EMT.

\subsubsection{Symmetries}

The combined action \eref{eq:gravity-matter-action} must be diffeomorphism-invariant, thus, given $ Z \in \xm : \delta g = \ld_Z g $ and following the same argument which lead to the Bianchi identity $ \eref{eq:bianchi-id} $:
\begin{equation*}
  \delta \act = \int_\man \dd^dx \sqg \left[ \left( \frac{1}{8 \pi \gd} G_{\mu \nu} - T^{\mu \nu} \right) \na^{(\mu} Z^{\nu)} + \frac{\delta \lag_\text{mat}}{\delta \Phi} \delta_Z \Phi \right] = 0
\end{equation*}
Using the Bianchi identity \eref{eq:bianchi-id} to rewrite the first term:
\begin{equation*}
  G_{\mu \nu} \na^{(\mu} Z^{\nu)} = G_{\mu \nu} \na^\nu Z^\mu = \na^\nu (G_{\mu \nu} Z^\mu) \equiv \na^\nu q_\nu
\end{equation*}
So, by Gauss's theorem \tref{th:gauss} applied to $ q \in \xm $:
\begin{equation*}
  \int_\man \dd^dx \sqg \, \na^\nu q_\nu = \int_{\pa \man} \dd^{d-1}x \sqrt{\gamma} \, n_\nu q^\nu \equiv 0
\end{equation*}
which can be made vanishing by a suitable choice of boundary conditions. Then, using the same argument for the EMT:
\begin{equation*}
  T_{\mu \nu} \na^{(\mu} Z^{\nu)} = \na^\nu (T_{\mu \nu} Z^\mu) - Z^\mu \na^\nu T_{\mu \nu}
\end{equation*}
The first term produces a neglectable boundary term, as before, so the remaining variation of the action is:
\begin{equation*}
  \delta \act = \int_\man \dd^dx \sqg \left[ Z^\mu \na^\nu T_{\mu \nu} + \frac{\delta \lag_\text{mat}}{\delta \Phi} \delta_Z \Phi \right] = 0
\end{equation*}
For on-shell matter fields the second term vanishes by the Euler--Lagrange equations, hence the EMT is on-shell covariantly conserved:
\begin{equation}
  \na^\nu T_{\mu \nu} = 0
\end{equation}

\subsubsection{Examples of EMTs}

\paragraph{Scalar field theory}

It is straightforward to compute $ T_{\mu \nu} $ for a scalar field $ \phi(x) $. Recall \eref{eq:scalar-action-r} (with $ \xi = 0 $) and \lref{lemma:metric-variations}:
\begin{equation*}
  \delta \mathcal{S}_{\text{scalar}} = \int \dd^4 x \sqg \left[ \frac{1}{4} g_{\mu \nu} \na^\rho \phi \na_\rho \phi + \frac{1}{2} g_{\mu \nu} V(\phi) - \frac{1}{2} \na_\mu \phi \na_\nu \phi \right] \delta g^{\mu \nu}
\end{equation*}
...
A perfect fluid is described by its energy density $ \rho(\ve{x},t) $, pressure $ p(\ve{x},t) $ and velocity 4-vector field $ u^\mu(\ve{x},t) : u^\mu u_\mu = -1 $. Pressure and energy density are related by an \textit{equation of state} $ p = p(\rho) $: cosmological fluids have a simple equation of state of the form $ p = w \rho $, e.g. $ w = \frac{1}{3} $ for radiation and $ w = 0 $ for dust.
...
\label{ssec:energy-conservation}

Being the EMT a tensor current, its covariant conservation has more subtle implications than the covariant conservation of vector currents.

\begin{observation}{Covariant conservation of vector currents}{vec-curr-cons}
  Consider a conserved vector current $ J \in \xm : \na_\mu J^\mu = 0 $. By Gauss's theorem \tref{th:gauss}, given a $ d $-dimensional submanifold $ \mathcal{V} $:
  \begin{equation*}
    0 = \int_\mathcal{V} \dd^dx \sqg \, \na_\mu J^\mu = \int_{\pa \mathcal{V}} \dd^{d-1}x \sqrt{\gamma} \, n_\mu J^\mu
  \end{equation*}
  Now, assume that the boundary has a tubular shape like $ \pa \mathcal{V} = \Sigma_1 \cup \Sigma_2 \cup S $, where $ \Sigma_{1,2} $ are past and future spacelike hypersurfaces and $ S $ is a (lateral) timelike hypersurface. If no current flows in/out of $ \mathcal{V} $, i.e. if $ n_\mu J^\mu \vert_S = 0 $, then the above equation reduces to charge conservation $ Q_J(\Sigma_1) = Q_J(\Sigma_2) $, where the charge associated to $ J $ on a spacelike hypersurface $ \Sigma $ is defined as:
  \begin{equation}
    Q_J(\Sigma) \defeq \int_\Sigma \dd^{d-1}x \sqrt{\gamma} \, n_\mu J^\mu
  \end{equation}
  Therefore, for a vector current, covariant conservation is equivalent to charge conservation.
\end{observation}

The same argument as in \obsref{obs:vec-curr-cons} does not apply to the energy-momentum tensor: the problem arises from generalizing $ \na_\mu J^\mu = \frac{1}{\sqg} \pa_\mu (\sqg J^\mu) $ to higher-order tensor fields, which is necessary in order to have a divergence theorem like \eref{eq:gauss-theorem}. Indeed, for a $ (0,2) $-tensor field:
\begin{equation*}
  \na_\mu T^{\mu \nu} = \pa_\mu T^{\mu \nu} + \Gamma^\mu_{\mu \rho} T^{\rho \nu} + \Gamma^\nu_{\mu \rho} T^{\mu \rho} = \frac{1}{\sqg} \pa_\mu (\sqg\, T^{\mu \nu}) + \Gamma^\nu_{\mu \rho} T^{\mu \rho}
\end{equation*}
The last term does not allow to convert the integral of $ \na_\mu T^{\mu \nu} = 0 $ to a boundary term. Instead:
\begin{equation}
  \pa_\mu (\sqg\, T^{\mu \nu}) = -\sqg\, \Gamma^\nu_{\mu \rho} T^{\mu \rho}
  \label{eq:no-div-th}
\end{equation}
Thus, for higher-order tensors, covariant conservation is not equivalent to charge conservation.
For the EMT, however, it is possible to define a conserved vector current associated to a Killing vector field $ \xi \in \xm $:
\begin{equation}
  J_T^\nu[\xi] \defeq \xi_\mu T^{\mu \nu}
\end{equation}
...
\begin{equation}
  Q_T[\xi ; \Sigma] \defeq \int_\Sigma d^{d-1} x \sqrt{\gamma}\, n_\mu J_T^\mu
\end{equation}
The interpretation of this conserved charge depends on the nature of the associated Killing vector field, e.g. if $ \xi $ is globally timelike, then the charge can be interpreted as the energy of matter.

The problem of energy conservation becomes subtle when dealing with spacetimes which do not admit any globally timelike Killing vector field.
For example, a system comprised of two orbiting stars is modelled by a spacetime which does not have such a Killing vector field: however, the problem of energy conservation does not arise in this case, as the stars, while orbiting each other, emit gravitational waves, thus losing energy and eventually spiraling towards each other. Nonetheless, a meaningful question is that of energy conservation of the total system, i.e. the two stars and the gravitational field. A guess would be to consider a total energy-momentum tensor defined similarly to \eref{eq:em-tens-def}, but:
\begin{equation*}
  T^{\text{total}}_{\mu \nu} = - \frac{2}{\sqg} \left[ \frac{1}{16\pi G} \frac{\delta \mathcal{S}_{\text{EH}}}{\delta g^{\mu \nu}} + \frac{\delta \mathcal{S}_M}{\delta g^{\mu \nu}} \right] = - \frac{1}{8\pi G} G_{\mu \nu} + T_{\mu \nu} = 0
\end{equation*}
by the Einstein field equations. This equation has no physical significance other than expressing the subtlety of energy conservation in Genera Relativity.

Clearly, one could try to understand the energy carried by the gravitational field alone. Unfortunately, there are compelling arguments that there exists no tensor which can be thought as the local energy density of the gravitational field: roughly speaking, the energy density of the Newtonian gravitational field $ \Phi $ is proportional to $ (\na \Phi)^2 $, so the relativistic equivalent should be proportional to the first derivatives of the metric, which can be made locally vanishing by normal coordinates due to the equivalence principle, and a tensor which vanishes in one coordinate frame does so in all of them.

\subsection{Energy conditions}

To study the general properties of a spacetime without explicitly referencing the specific characteristics of matter, one needs to place certain restrictions on the kinds of EMTs which are considered physical: these are the \textit{energy conditions} and express the idea that energy should be positive.

\paragraph{Weak energy condition}

The weak energy condition states that, for any timelike vector field $ X \in \xm $:
\begin{equation*}
  T_{\mu \nu} X^\mu X^\nu \ge 0
\end{equation*}
...
\paragraph{Strong energy condition}
...
\end{equation}
This condition ensures that timelike geodesics converge, i.e. that gravity is attractive. Using \eref{eq:ef-eq-alt}, this condition can be rewritten as:
\begin{equation*}
  \left( T_{\mu \nu} - \frac{1}{d-2} T g_{\mu \nu} \right) X^\mu X^\nu \ge 0
\end{equation*}
...
\end{equation}
This condition is implied by both the weak and strong energy condition, but the converse is not true: it is weaker than both conditions.

\paragraph{Dominant energy condition}

Given a future-directed timelike vector field $ X \in \xm $, it is possible to define the energy density current measured by an observer moving along integral curves of $ X $ as:
\begin{equation*}
  J^\mu \equiv - T^{\mu \nu} X_\nu
\end{equation*}
The dominant energy condition requires that, in addition to the WEC \eref{eq:weak-en-cond}:
\begin{equation}
  J_\mu J^\mu \le 0
  \label{eq:dominant-en-cond}
\end{equation}
This means that the energy density current is either timelike or null, so requiring that energy does not flow faster than light.

\begin{example}{Fluids}{}
  Recall the energy-momentum tensor for a fluid \eref{eq:em-tens-fluid} and impose the weak energy condition (WLOG $ \braket{X , X}_g = -1 $):
  \begin{equation*}
  ...
  On the other hand, the strong energy condition for the energy-momentum tensor of a fluid \eref{eq:em-tens-fluid} in the rest-frame reads:
  \begin{equation*}
  ...
  \begin{equation*}
    ...
    \end{cases}
  \end{equation*}
  It is possible to show that $ \Lambda > 0 $ violates the strong energy condition: dS space is incompatible with it, as timelike geodesics are pulled apart by the expansion of space.

  Furthermore, it can be shown that the null energy condition implies $ \rho + p \geq 0 $, while the dominant energy condition imposes $ \rho^2 \geq p^2 $.
\end{example}

\begin{example}{Scalar field theories}{}
  ...
  \begin{equation*}
    ...
    V(\phi) \ge 0
  \end{equation*}
  The weak energy condition is clearly violated by any classical field theory with $ V(\phi) \le 0 $. On the contrary, any classical scalar field theory with $ V(\phi) \ge 0 $ violates the strong energy condition, while the null is satisfied by any classical scalar field theory.
\end{example}
...
The three special solution (Minkowski, dS, AdS) not only have different spacetime curvature and symmetries, but, more fundamentally, they have different behaviour at infinity. Their importance lies in the fact that, however complicated the metric might be, if fields are suitably localized, then they will asymptote to one of these three symmetric spaces.
...
\end{equation}
where $ d \equiv \dim_{\R}\mathcal{M} $. The Weyl tensor has all the symmetries of the Riemann tensor, with the additional property that it vanishes when contracting any pair of indices with the metric: it can be viewed as the trace-free part of the Riemann tensor.
...
\end{equation}
Conversely, assume that the Froebenius condition holds for the Killing field $ K $ of a stationary Lorentzian manifold $ (\man , g) $. Then, applying $ \ip_K $ to \eref{eq:froebenius-cond} and using \tref{th:cartan} yields:
\begin{equation*}
  0 = \ip_K (\omega \wedge \dd \omega) = (\ip_K \omega) \wedge \dd \omega - \omega \wedge (\ip_K \dd \omega) = g(K,K) \dd \omega - \omega \wedge (\ld_K \omega - \dd g(K , K))
\end{equation*}
...
\begin{proofbox}
  \begin{proof}
    First of all, spherical symmetry requires that the Lorentzian manifold considered has $ \SOn{3} $ as an isometry group, with the conditions that the group orbits\footnote{Given a group $ G $ which acts on a set $ X $, the orbit of $ x \in X $ is defined as $ G(x) \defeq \{g x \in X : g \in G\} \ssq X $.} are 2-dimensional spacelike hypersurfaces. Thus, the manifold splits into $ \man = \Sigma \times \Sn{2} $, where $ \Sigma $ is a 2-dimensional Lorentzian submanifold. Since the metric on a 2-dimensional pseudo-Riemannian manifold can be always diagonalized\footnote{This is achieved with the so-called \emph{isothermal coordinates}.}, the most general spherically-symmetric metric takes the form:
    \begin{equation*}
      \dd s^2 = - e^{2a(t,r)} \dd t^2 + e^{2b(t,r)} \dd r^2 + r^2 \left[ \dd \vartheta^2 + \sin^2 \varphi \, \dd \varphi^2 \right]
    \end{equation*}
    The $ \vartheta , \varphi $ are the standard coordinates on $ \Sn{2} $, $ t \in \R $ and the radial coordinate $ r $ is determined by the foliation of the spatial part of the manifold by invariant 2-spheres: in particular, each 2-sphere is characterized by a surface area $ A $, and $ r $ is defined by $ A = 4\pi r^2 $.
    ...
    \end{equation*}
    Now, the vacuum field equations read $ \tensor{G}{^\mu_\nu} = 0 $. From $ \tensor{G}{^1_0} = 0 $ it is clear that $ b = b(r) $; then, $ \tensor{G}{_1_1} + \tensor{G}{_0_0} = 0 $ implies the partial differential equation $ a' + b' = 0 $, which has the general solution $ a(t,r) = -b(r) + f(t) $. To determine $ b(r) $, consider $ \tensor{G}{^0_0} = 0 $:
    \begin{equation*}
...
\begin{equation}
  ...
  \label{eq:schwarz-effective-pot}
\end{equation}
This potential always has asymptotics $ V_\text{eff}(r) \ra -\infty $ for $ r \ra 0 $ and $ V_\text{eff}(r) \ra 1 $ for $ r \ra + \infty $, but the intermediate behaviour depends on the ratio $ \ell / m $: in particular, for $ \ell / m < 2 \sqrt{3} $ the potential has no critical points, hence any incoming particle falls towards the singularity at $ r = \sr $.
...
Consider a point-like source, emitting light in a constant and isotropic way in its comoving reference frame, which is radially falling from infinity. The emitted light is detected by an observer in a fixed position, along the same radial direction, at $ r = R \gg \sr $.

Since light propagates on null curves with $ \dd s^2 = 0 $, from \eref{eq:schwarz-sol}:
\begin{equation*}
  \dd t = \pm \frac{r}{r - \sr} \dd r
  \quad \implies \quad
  t = r + \sr \ln (r - \sr) + \text{const.}
\end{equation*}
where the positive solution has been chosen and the $ \Sn{2} $ component has been ignored, since the motion is radial (i.e. with fixed $ \vartheta $ and $ \varphi $).
Now, fix an emission event $ (t_\text{e} , r_\text{e}) $, which is detected at $ (t_\text{r},R) $. Clearly, by the above relation:
\begin{equation}
  t_\text{r} - R - \sr \ln (R - \sr) = t_\text{e} - r_\text{e} - \sr \ln (r_\text{e} - \sr)
  \label{eq:aux-1}
\end{equation}
It is possible to make the relation between $ t_\text{e} $ and $ r_\text{e} $ explicit. Recall \eref{eq:schwarz-effective-pot} and set $ \ell = 0 $ (radial motion), so that:
\begin{equation*}
  \dot{r}^2 = E^2 - 1 + \frac{\sr}{r} \equiv \frac{\sr}{r} - \frac{\sr}{r_0}
  \qquad \qquad
  r_0 \equiv \frac{\sr}{1 - E^2}
\end{equation*}
where $ r_0 $ is the initial radial coordinate of the geodesic. Note that this is the same equation which can be derived from energy conservation in Newtonian mechanics, with $ \Phi(r) = - GM m_\text{s} / r $. Since the source is falling from infinity, $ E = 1 $ (which justifies the choice of the positive solution for $ t = t(r) $), hence the geodesic is described by:
\begin{equation}
  r = \sr s^2
  \qquad \qquad
  \tau = - \frac{2}{3} \sr s^3
  \label{eq:aux-2}
\end{equation}
where $ s \in \R^+_{>1} $ is an auxiliary variable and the choice of the sign of $ \tau $ is dictated by the fact that the source is falling, so $ s $ decreases along the trajectory while $ \tau $ must always increase. Then, by \eref{eq:schwarz-energy}:
\begin{equation*}
  \frac{r - \sr}{r} \frac{\dd t}{\dd \tau} = 1
  \quad \implies \quad
  t = - \frac{2}{3} \sr s^3 - 2 \sr s + \sr \ln \frac{s + 1}{s - 1}
\end{equation*}
Inserting this into \eref{eq:aux-1} in place of $ (t_\text{e} , r_\text{e}) $:
\begin{equation*}
  t_\text{r} = - \frac{2}{3} \sr s^3 - \sr s^2 - 2 \sr s - 2 \sr \ln (s - 1) + R + \sr \ln \left( \frac{R}{\sr} - 1 \right)
\end{equation*}
In the limit $ s \ra 1 $, i.e. $ r \ra \sr $, this equation becomes:
\begin{equation*}
  t_\text{r} \ra - 2 \sr \ln (s - 1) + \text{const.}
  \quad \implies \quad
  s \ra 1 + a e^{- \frac{t_\text{r}}{2\sr}}
\end{equation*}
with $ a \equiv \exp (\text{const.} / 2\sr) $. Differentiating this equation and \eref{eq:aux-2}, then:
\begin{equation}
  \frac{\dd t_\text{r}}{\dd \tau} \ra a^{-1} e^{\frac{t_\text{r}}{2\sr}}
\end{equation}
Note that $ \tau $ is the proper time of the source, while $ t_\text{r} $ can be identified with the proper time of the observer, since $ R \gg \sr $. This means that the rate of reception of photons by the observer is exponentially damped, resulting in an exponential decrease of the observed intensity of the source as it approaches $ r = \sr $. Additionally, the light emitted by the source is exponentially redshifted\footnotemark, near the horizon: combining these effects, the source eventually becomes undetectable, but the radiation intensity never drops to zero, although the source crosses the horizon in a finite proper time (as per \eref{eq:aux-2}).

\footnotetext{Requires proof.}
% TODO: proof of exponential redshift
...
\begin{equation}
  u(\varphi) = \frac{m}{\ell^2} \left( 1 + e \cos \varphi \right) + \frac{3m^3}{\ell^4} \left[ 1 + \frac{e^2}{2} - \frac{e^2}{6} \cos 2 \varphi + e \varphi \sin \varphi \right]
\end{equation}
...
\begin{proposition}{Elapsed coordinate time}{}
  ...
  \begin{equation}
    \Delta t(r,r_0) \simeq \sqrt{r^2 - r_0^2} + \sr \ln \left[ \frac{r + \sqrt{r^2 - r_0^2}}{r_0} \right] + \frac{\sr}{2} \sqrt{\frac{r - r_0}{r + r_0}}
  \end{equation}
\end{proposition}
...
\begin{proofbox}
  \begin{proof}
    ...
    \end{equation*}
    If $ r_0 $ is the distance of closest approach, then $ \frac{\dd r}{\dd t} \big\vert_{r = r_0} = 0 $, so that:
    \begin{equation*}
...
\begin{figure}
  \centering
   \begin{tikzpicture}
    \begin{feynman}
      \vertex [dot] (s) {};
      \vertex [right = 20em of s, dot] (l) {};
      \vertex [right = 14em of l, dot] (rp) {};
      \vertex [above = 4em of rp, dot] (r) {};

      \vertex [above = 8em of l, dot] (q) {};
      \vertex [above = 4em of l, dot] (rpp) {};
      \vertex [above = 2.352941176em of l, dot] (qp) {};

      \vertex [right = 5em of q] (ap);
      \vertex [above = 2em of ap] (a);

      % labels
      \vertex [left = 1em of rpp] (lql) {$ b $};
      \vertex [left = 1em of s] (ls) {$ \text{S} $};
      \vertex [below = 1em of l] (ll) {$ \text{L} $};
      \vertex [right = 1em of r] (lr) {$ \text{R} $};
      \vertex [right = 1em of rp] (lrp) {$ \text{R}' $};
      \vertex [above = 1em of q] (lq) {$ \text{Q} $};

      \diagram* {
        (s) -- [edge label' = $ D_\text{s} $] (l) -- [edge label' = $ D_\text{r} $] (rp),
        (s) -- (q) -- (r),

        (r) -- [scalar, edge label = $ r $] (rp),
        (r) -- [scalar] (rpp),
        (r) -- [scalar] (s),
        (q) -- [scalar] (qp),
        (qp) -- [scalar, edge label = $ r' $] (l),

        (q) -- [scalar] (a),
      };
    \end{feynman}

    \path pic["$ \theta_\text{s} $", draw, angle radius = 3em, angle eccentricity = 1.2] {angle = l--s--q};
    \path pic["$ \theta_\text{r} $", draw, angle radius = 4em, angle eccentricity = 1.2] {angle = q--r--rpp};
    \path pic["$ \delta $", draw, angle radius = 2em, angle eccentricity = 1.2] {angle = r--q--a};
  \end{tikzpicture}
  \caption{Schematics of the lens geometry.}
  \label{fig:lensing}
\end{figure}

\paragraph{Optical schematics}

The optical scheme of a gravitational lens is composed of three points: the source $ \text{S} $, the lens $ \text{L} $ and the observer $ \text{R} $. Then, set $ \text{R}' $ the projection of $ \text{R} $ on the line $ \text{SL} $ and define the distances $ D_\text{s} \equiv \overline{\text{SL}} $, $ D_\text{r} \equiv \overline{\text{LR}'} $ and $ r \equiv \overline{\text{RR}'} $, as in Fig. \figref{fig:lensing}.

At first approximation, it is possible to identify the lightray emitted from the source and deflected by the lens with its two asymptotes, which are lines passing through a common point $ Q $ at a distance $ b $ from the lens and forming an angle $ \delta $ given by \eref{eq:light-deflection}, with $ \overline{\text{QL}} \perp \overline{\text{SL}} $: these assumptions only induce errors of order $ \smo(b^{-2}) $, where $ b,r \ll D_\text{s} , D_\text{r} $ is supposed.

\begin{proposition}{Lensed image}{}
  The image produced by the lens has dimension:
  \begin{equation}
    b_\pm = \frac{1}{2} \left[ r' \pm \sqrt{r'^2 + 4b_\text{E}^2} \right]
    \label{eq:grav-lensing}
  \end{equation}
  with:
  \begin{equation}
    b_\text{E} \equiv \sqrt{2\sr \frac{D_\text{s} D_\text{r}}{D_\text{s} + D_\text{r}}}
    \qquad \qquad
    r' \equiv \frac{D_\text{s}}{D_\text{s} + D_\text{r}} r
  \end{equation}
\end{proposition}

\begin{proofbox}
  \begin{proof}
    Geometrically, the following relations are clear (with $ b \ll \sr $):
    \begin{equation*}
      \theta_\text{s} + \theta_\text{r} = \delta
      \qquad \qquad
      D_\text{s} \theta_\text{s} = b
      \qquad \qquad
      D_\text{r} \theta_\text{r} = b - r
    \end{equation*}
    Substituting \eref{eq:light-deflection} and combining these equations, then:
    \begin{equation*}
      \frac{b}{D_\text{s}} + \frac{b - r}{D_\text{r}} = \frac{2\sr}{b}
      \quad \implies \quad
      b^2 - \frac{D_\text{s} r}{D_\text{s} + D_\text{r}} b = 2\sr \frac{D_\text{s} D_\text{r}}{D_\text{s} + D_\text{r}}
    \end{equation*}
    Solving this equation yields the thesis.
  \end{proof}
\end{proofbox}

Note that the two solutions correspond to the lightray passing either ``above" (like in \figref{fig:lensing}) or ``below" the lens: this does not mean that the lens forms two images, since, for an image to form, it is necessary that a small beam of light passes through the same point after being lensed, but this does not happen in gravitational lenses, which are highly astigmatic.

While $ r' $ has a clear geometrical meaning, $ b_\text{E} $ corresponds to the case of a perfect alignment, i.e. $ r = 0 $. In this case, the lighrays which get lensed are not only the two already analyzed, which confined on the plane determined by $ \text{S} $, $ \text{L} $ and $ \text{R} $, but are infinitely many, and they lie on a cone with axis $ \overline{\text{SR}} $ and semiangle $ \theta_\text{r} $. In this case, the lens does not produce two images, but a whole circumference of radius $ b_\text{E} $: an \bctxt{Einstein ring}.

Note that these Einstein rings are nearly impossible to observe for relatively close sources: with $ D_\text{s} = D_\text{r} = 1 \,\text{kpc} $ and $ \sr = 3 \,\text{km} $, the angular semiopening of the ring is $ \theta_\text{r} = b_\text{E} / D_\text{r} \simeq 0.002'' $. On the other hand, these rings are fairly common on cosmological scales, e.g. with a galaxy as lens, but on these scales it is not possible to apply a Euclidean analysis.

\paragraph{Microlensing}

In the conditions of gravitational microlensing, observing lensed images or Einstein rings is impossible, as already seen. However, an observable effect is the magnification of the source. Assuming that the source emits isotropically a luminosity $ L $, the power irradiated on an area $ \dd \sigma $ in the absence of any lens is:
\begin{equation*}
  P_0 = \frac{L}{4\pi} \frac{\dd \sigma}{(D_\text{s} + D_\text{r})^2}
\end{equation*}
On the other hand, if a lens is present, the power irradiated on $ \dd \sigma $ is given by the luminosity emitted in the solid angle $ \dd \Omega = \sin \theta_\text{s} \dd \theta_\text{s} \dd \varphi $, while $ \dd \sigma = r \, \dd r \dd \varphi $ by axial symmetry, so that:
\begin{equation*}
  P = L \frac{\dd \Omega}{4\pi}
\end{equation*}
The magnification $ A $ is the ratio between $ P $ and $ P_0 $, that is:
\begin{equation*}
  A = \frac{P}{P_0} = (D_\text{s} + D_\text{r})^2 \frac{\dd \Omega}{\dd \sigma} \simeq (D_\text{s} + D_\text{r})^2 \frac{\theta_\text{s} \dd \theta_\text{s}}{r \, \dd r} = \left( \frac{D_\text{s} + D_\text{r}}{D_\text{s}} \right)^2 \frac{b \, \dd b}{r \, \dd r} = \frac{b \, \dd b}{r' \dd r'}
\end{equation*}
Recalling \eref{eq:grav-lensing}, one must add the two contributions from each image, thus finding:
\begin{equation*}
  A = \frac{b_+^2}{r' \abs{2b_+ - r'}} + \frac{b_-^2}{r' \abs{2b_- - r'}}
\end{equation*}
where the absolute value is necessary in order to keep $ A $ positive. A trivial manipulation yields the final expression:
\begin{equation}
  A = \frac{r'^2 + 2 b_\text{E}^2}{2 r' \sqrt{r'^2 + 4 b_\text{E}^2}}
\end{equation}
This expression seams to have a pole for $ r' \ra 0 $: this is easily avoided by considering the source as an extended object, rather than a point.

Searching for these kinds of magnifications in the light of distant stars allows to study MACHOs: to distinguish the resulting luminous variability from variable stars, it is important to note that the microlensing effect is exactly symmetrical in time and non-dispersive, i.e. equal for all wavelengths.
