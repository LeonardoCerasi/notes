...
\end{equation}
where the negative sign makes it manifest that the metric has signature $ (-,+,+,+) $. Schematically, the Riemann tensor is $ R \sim \pa \Gamma + \Gamma \Gamma $, while $ \Gamma \sim \pa g $, thus the Einstein-Hilbert action is second order in derivatives of the metric, like most other actions (for bosonic fields) in physics.
...
In reality, \eref{eq:einstein-hilbert} is not the simplest action allowed by Differential Geometry: in fact, a constant term could be added to the Ricci scalar. The action then becomes:
\begin{equation}
...
Consider a diffeomorphism $ x^\mu \mapsto \tilde{x}^\mu = x^\mu + \delta x^\mu $: this can be viewed as an active diffeomorphism (alibi interpretation), where points in spacetime are mapped from one another, or as a passive diffeomorpshim (alias interpretation), in which only the coordinates of each point are affected, but the two interpretations are equivalent. This change of coordinates can be thought as generated by an infinitesimal smooth vector field $ Z \in \xm : \delta x^\mu = - Z^\mu (x) $.
...
\begin{lemma}{Transformation of tensors under infinitesimal diffeomorphisms}{tens-inf-diff}
  ...
  \end{equation}
  Then, if $ \tilde{x}^\mu = x^\mu - Z^\mu $, the pointwise variation of $ T $ is:
  \begin{equation}
...
\end{equation}
Thus, an infinitesimal diffeomorphism along $ Z \in \xm $ makes the metric change by an infinitesimal amount given by its Lie derivative along $ Z $, which can be viewed as the leading term in the Taylor expansion along $ Z $. These equations can be rewritten in a simpler form:
\begin{equation*}
  \delta g_{\mu \nu} = Z^\rho \pa_\rho g_{\mu \nu} + \pa_\nu Z_\mu - Z^\rho \pa_\nu g_{\mu \rho} + \pa_\mu Z_\nu - Z^\rho \pa_\mu g_{\nu \rho} = \pa_\mu Z_\nu + \pa_\nu Z_\mu + 2 g_{\rho \sigma} \Gamma^\sigma_{\mu \nu} Z^\rho
\end{equation*}
Therefore:
\begin{equation}
  \delta g_{\mu \nu} = \na_\mu Z_\nu + \na_\nu Z_\mu
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \frac{\dd Q}{\dd \tau} = \pa_\nu K_\mu \frac{\dd x^\nu}{\dd \tau} \frac{\dd x^\mu}{\dd \tau} + K_\mu \frac{\dd^2 x^\mu}{\dd \tau^2} = \pa_\nu K_\mu \frac{\dd x^\nu}{\dd \tau} \frac{\dd x^\mu}{\dd \tau} - K_\mu \Gamma^\mu_{\rho \sigma} \frac{\dd x^\rho}{\dd \tau} \frac{\dd x^\sigma}{\dd \tau} = \na_\nu K_\mu \frac{\dd x^\nu}{\dd \tau} \frac{\dd x^\mu}{\dd \tau} = 0
    \end{equation*}
