...
In these notes, the Lorentz--Minkowski metric $ \eta_{\mu \nu} $ has signature $ (-,+,+,+) $, except for the Cosmology part where a $ (+,-,-,-) $ signature is adopted, and Greek indices generally run over spacetime coordinates, while Latin indices are general $ \N_0 $-indices defined in each context. Repeated indices are generally summed over, unless otherwise specified. The $ n $-dimensional Levi--Civita symbol $ \epsilon^{i_1 \dots i_n} $ is defined with the convention $ \epsilon^{0 1 \dots n} = +1 $.
...
The imaginary unit is denoted by $ \img $ and the unit quaternions by $ \img , \jmg , \kmg $, so that $ \C(\R) = \lspan(1,\img) $ and $ \qt(\R) = \lspan(1,\img,\jmg,\kmg) $.

The $ n $-dimensional sphere is denoted by $ \Sn{n} $, while the $ n $-dimensional disk by $ \Dn{n} $, so that $ \pa \Dn{n} = \Sn{n-1} $, where in general $ \pa \Sigma $ denotes the boundary of $ \Sigma $. The measure on $ \Sn{n} $ is denoted by $ \dd \Omega_n $, while its line element as $ \dd \Omega_n^2 $.
...
Given a $ \K $-vector space $ V $, the space of all rank-$ (k,l) $ tensors is denoted by $ T^k_l(V) $, the space of exterior $ p $-forms by $ \lv{p}(V) $ and the Grassmann algebra of $ V $ by $ \ga(V) = \bigoplus_{k = 0}^n \lv{k}(V) $.

Given a smooth manifold $ \mathcal{M} $, the space of all smooth scalar functions on $ \mathcal{M} $ is denoted by $ \cm $, the space of all smooth vector fields on $ \mathcal{M} $ by $ \xm $, the space of all smooth $ (k,l) $-tensors on $ \man $ by $ \tns{k}{l}(\man) $, the space of all smooth $ p $-forms on $ \mathcal{M} $ by $ \df{p} $ and the smooth forms on $ \mathcal{M} $ by $ \dft \defeq \bigoplus_{k = 0}^n \df{k} $.

The exterior derivative is denoted by $ \dd $, the covariant exterior derivative by $ \dcov $, the partial derivative (and the ``boundary operator") by $ \pa $, the nabla operator by $ \grad $, the Laplace--Beltrami operator by $ \lap $ and the D'Alembert operator by $ \Box $.
...
Natural units are used, with $ c = \hbar = k_\text{B} \equiv 1 $, so that mass, energy, temperature, inverse length and inverse time all have the same units.

In the Cosmology part, the metric signature is inverted to $ (+ , - , - , -) $, and quantities with a subscript `0' are evaluated at the present coordinate time $ t_0 $.
