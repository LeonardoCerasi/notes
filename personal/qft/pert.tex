...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      ...
      \end{split}
    \end{equation*}
    Now, observe that $ \braket{0 | \normord\{\phi_I(x) \phi_I(y)\} | 0} = 0 $, as there is always either an annihilation operator acting on $ \ket{0} $ or a creation operator acting on $ \bra{0} $. On the other hand, $ \fpr(x-y) \in \C $, as $ [a_\ve{p} , a\dg_\ve{q}] \in \C $, therefore $ \braket{0 | D_\text{F}(x-y) | 0} = \fpr(x-y) \braket{0 | 0} = \fpr(x-y) $. There only remains to compute the commutators:
    \begin{equation*}
      \begin{split}
        \fpr(x-y)
...
\begin{equation*}
  \begin{split}
    & -\img \lambda \int \dd^4x \prod_{i = 1}^4 \dd^4x_i\, e^{\img ({p_1}_\mu {x_1}^\mu + {p_2}_\mu {x_2}^\mu - {p_3}_\mu {x_3}^\mu - {p_4}_\mu {x_4}^\mu)} \fpr(x_1 - x) \fpr(x_2 - x) \fpr(x_3 - x) D_\text{F}(x_4 - x) \\
...
\begin{equation}
  ...
  \quad \implies \quad
  \gprp(k) = \frac{-\img}{k^2 + \img \epsilon} \eta_{\mu \nu}
  \label{eq:phot-prop}
\end{equation}
