...
To see how $ \ve{\left( \frac{1}{2},\frac{1}{2} \right)} $ is the 4-vector representation, consider the space $ \mathcal{H}(2) \defeq \{\mt{H} \in \C^{2 \times 2} : \mt{H}\dg = \mt{H}\} $ of all $ \C^{2 \times 2} $ Hermitian matrices.
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      ...
      = V \in \mathcal{H}(2)
    \end{equation*}
    where the isomorphisms are $ V = v_\mu \sigma^\mu , v^\mu = \frac{1}{2} \tr \left( \bar{\sigma}^\mu V \right) $, with $ \sigma^\mu \equiv (\mt{I}_2,\bs{\sigma}) , \bar{\sigma}^\mu \equiv (\mt{I}_2,-\bs{\sigma}) $.
  \end{proof}
\end{proofbox}

As of \lref{lemma:mink-herm-iso}, given $ \Lambda \in \lorg $ acting on 4-vectors as $ v^\mu \mapsto \tensor{\Lambda}{^\mu _\nu} v^\nu $, it is isomorphic to the action of $ \Lambda_\text{s} \in \SL{2,\C} $ on Hermitian matrices as $ V \mapsto \Lambda_\text{s} V \Lambda_\text{s}\dg $: indeed, this transformation preserves Hermiticity and the Minkowski norm $ \det V = v_\mu v^\mu $ (as $ \det \Lambda_\text{s} = 1 $). This transformation can be explicited as:
\begin{equation}
  \tensor{\Lambda}{^\mu_\nu} \sigma^\nu = \Lambda_\text{s} \sigma^\mu \Lambda_\text{s}\dg
\end{equation}
By \eref{eq:sun-trace} and \pref{prop:lorentz-algebra}, $ \sla{2,\C} = \{\mt{X} \in \C^{2 \times 2} : \tr \mt{X} = 0\} $ and an $ \R $-basis is $ \{\frac{\bs{\sigma}}{2},\img\frac{\bs{\sigma}}{2}\} $, then:
\begin{equation*}
  \Lambda_\text{s} = \exp \left[ -\img \left( \bs{\theta} + \img \bs{\eta} \right) \cdot \frac{\bs{\sigma}}{2} \right] = \Lambda_\text{R}
  \quad \implies \quad
  \Lambda_\text{s}\dg = \exp \left[ \left( \img \bs{\theta} + \bs{\eta} \right) \cdot \frac{\bs{\sigma}}{2} \right] = \sigma^2 \Lambda_\text{L}^\intercal \sigma^2
\end{equation*}
