...
\begin{equation}
  \eta_n (t) = e^{\img \left( k n - \omega t \right)}
\end{equation}
...
\begin{equation}
  \eta_{n + N} = \eta_n
  \quad \implies \quad
  k_m = \frac{2\pi m}{N} \,,\, m = 0, 1, \dots, N - 1
\end{equation}
The normal-mode expansion can then be written as:
\begin{equation}
  \eta (t) = \sum_{m = 0}^{N - 1} \left[ \mathcal{A}_m e^{\img \left( k_m n - \omega_m t \right)} + \mathcal{A}^* e^{-\img \left( k_m n - \omega_m t \right)} \right]
  \label{eq:first-exp}
\end{equation}
...
\end{equation}
In quantum mechanics, $ \eta_n $ and $ \Pi_n $ become operators with canonical commutator $ [ \hat{\eta}_j, \hat{\pi}_k ] = \img \hbar \delta_{jk} $. Implementing time evolution with the Heisenberg picture\footnote{Recall that $ \hat{\mathcal{O}}(t) = e^{\frac{\img}{\hbar} \hat{H} t} \hat{\mathcal{O}}(0) e^{-\frac{\img}{\hbar} \hat{H} t} $ and $ \frac{\dd \hat{\mathcal{O}}}{\dd t} = \frac{\img}{\hbar} [ \hat{H}, \hat{\mathcal{O}} ] $.}:
\begin{equation}
  [ \hat{\eta}_j(t), \hat{\pi}_k(t) ] = \img \hbar \delta_{jk}
  \label{eq:first-comm}
\end{equation}
...
%
\footnotetext{For a harmonic oscillator $ \hat{H} = \frac{1}{2} \hat{p}^2 + \frac{1}{2} \omega^2 \hat{x}^2 $, so $ \frac{\dd \hat{x}}{\dd t} = \hat{p}(t) $ and $ \frac{\dd \hat{p}}{\dd t} = -\omega^2 \hat{x}(t) $ and the solution can be written as: $$ \hat{x}(t) = \sqrt{\frac{\hbar}{2\omega}} \left[ \hat{a}(t) + \hat{a}^\dagger(t) \right] \qquad \qquad \hat{p}(t) = -\img \omega \sqrt{\frac{\hbar}{2\omega}} \left[ \hat{a}(t) - \hat{a}^\dagger(t) \right] $$ Inverting these expressions one finds $ [\hat{a}(t), \hat{a}^\dagger(t)] = 1 $ and $ \hat{H} = \hbar \omega \left( \hat{a}^\dagger \hat{a} + \frac{1}{2} \right) $. The time evolution $ \hat{a}(t) = e^{-\img \omega t} \hat{a}(0) $ ensures that $ \hat{H} $ is times-independent.}
%
\begin{equation}
  \hat{\eta}_n(t) = \sum_{m = 0}^{N - 1} \sqrt{\frac{\hbar}{2 \omega_m}} \frac{1}{\sqrt{N}} \left[ e^{\img \left( k_m n - \omega_m t \right)} \hat{a}_m + e^{-\img \left( k_m n - \omega_m t \right)} \hat{a}_m^\dagger \right]
...
\begin{equation}
  \sum_{m = 0}^{N - 1} e^{\img k_m \left( n - n' \right)} = N \delta_{n n'}
  \label{eq:first-id}
\end{equation}
...
\end{equation}
This Hilbert space is called \bctxt{Fock space} and the excited states \bctxt{phonons}: these can be thought as ``particles" and $ n_m $ is the number of phonons in the $ m^{\mathrm{th}} $ normal mode.
...
\begin{proposition}{Dirac delta}{continuous-limit}
  In the continuous limit:
  \begin{equation*}
    \frac{\delta_{nn'}}{a} \longrightarrow \delta(x - x') = \int_\R \frac{\dd k}{2\pi} e^{\img k (x - x')}
  \end{equation*}
\end{proposition}
...
\begin{proofbox}
  ...
  \begin{equation*}
    \delta(x - x') \longleftarrow \frac{\delta_{nn'}}{a} = \frac{1}{Na} \sum_{m} e^{\img k_m (n - n')} \longrightarrow \int_\R \frac{\dd k}{2\pi} e^{\img k (x - x')}
  \end{equation*}
...
\begin{proposition}{}{}
  ...
  \begin{equation}
    \int_0^R \dd x\, e^{\img (k - k') x} = 2\pi \delta(k - k')
  \end{equation}
\end{proposition}

\begin{proofbox}
  \begin{proof}
    Denoting the Fourier transform by the operator $ \mathfrak{F} $, the thesis is a consequence of $ \mathfrak{F}^{-1} \mathfrak{F} = \id $.
  \end{proof}
\end{proofbox}
...
\begin{equation}
  \phi(x,t) = e^{\img \left( kx - \omega_k t \right)}
\end{equation}
...
\begin{equation}
  \hat\phi(x,t) = \int_\R \frac{\dd k}{2\pi} \sqrt{\frac{\hbar}{2\omega_k}} \left[ e^{\img \left( kx - \omega_k t \right)} \hat{a}_k + e^{-\img \left( kx - \omega_k t \right)} \hat{a}_k^\dagger \right]
\end{equation}
...
\end{equation}
\begin{equation}
  [\hat\phi(x,t), \hat\Pi(x',t)] = \img \hbar \delta(x - x')
\end{equation}
...
Consider the group of linear transformations $ x^\mu \mapsto \tensor{\Lambda}{^\mu_\nu} x^\nu $ on $ \R^{1,3} $ which leave invariant the quantity $ \eta_{\mu \nu} x^\mu x^\nu $, i.e. the orthogonal group $ \On{1,3} $. The condition that $ \tensor{\Lambda}{^\mu_\nu} $ must satisfy reads:
\begin{equation}
  \eta_{\rho \sigma} = \eta_{\mu \nu} \tensor{\Lambda}{^\mu_\rho} \tensor{\Lambda}{^\nu_\sigma}
  \label{eq:lor-def}
\end{equation}
This implies that $ \det \Lambda = \pm 1 $: a transformation with $ \det \Lambda = -1 $ can always be written as the product of a transformation with $ \det \Lambda = 1 $ and a discrete transformation which reverses the sign of an odd number of coordinates, hence one further defines the special orthogonal group $ \SOn{1,3} \defeq \{\Lambda \in \On{1,3} : \det \Lambda = 1\} $.
...
\begin{definition}{Lorentz group}{}
  The \bcdef{Lorentz group} $ \lorg $ is the orthochronous component of $ \SOn{1,3} $.
\end{definition}

The discrete transformations are factored out of the Lorentz group: these are \bctxt{parity} and \bctxt{time reversal}, represented as $ \tensor{\mathcal{P}}{^\mu_\nu} = \diag \left( +1,-1,-1,-1 \right) $ and $ \tensor{\mathcal{T}}{^\mu_\nu} = \diag \left( -1,+1,+1,+1 \right) $. Applying these discrete transformations in all combinations ($ \id $, $ \mathcal{P} $, $ \mathcal{T} $ and $ \mathcal{P} \mathcal{T} $) one gets the four disconnected components of $ \On{1,3} $, which are not simply connected. This means that Lorentz invariance does not include parity and time reversal invariance.

Considering an infinitesimal Lorentz transformation and applying \eref{eq:lor-def}:
\begin{equation*}
  \tensor{\Lambda}{^\mu_\nu} = \delta^\mu_\nu + \tensor{\omega}{^\mu_\nu}
  \quad \implies \quad
  \omega_{\mu \nu} = - \omega_{\nu \mu}
...
\begin{proofbox}
  \begin{proof}
    Spherical and hyperbolic rotations are continuous and differentiable functions respectively of angles and of rapidities, but while angles vary in $ [0, 2\pi) $, rapidities vary in $ \R $, hence the differentiable manifold associated to $ \lorg $ is not compact.
  \end{proof}
\end{proofbox}
...
\begin{equation}
  \Lambda = e^{- \frac{\img}{2} \omega_{\mu \nu} J^{\mu \nu}}
\end{equation}
The $ \frac{1}{2} $ factor arises from each generator being counted twice (product of two anti-symmetric objects). Given an $ n $-dimensional representation of $ \lorg $, then $ \tensor{[J^{\mu \nu}]}{^i_j} \in \C^{n \times n} $ and $ \tensor{[\Lambda]}{^i_j} \in \R^{n \times n} $: for example, the $ n = 1 $ representation acts on \bctxt{scalars}, which are invariant under Lorentz transformations, so $ J^{\mu \nu} \equiv 0 \,\,\forall \mu,\nu = 0, \dots, 3 $ and $ \Lambda \equiv 1 $.

\paragraph{4-vectors}

The $ n = 4 $ representation\footnotemark acts on \bctxt{contravariant 4-vectors} $ v^\mu $, which transform according to $ v^\mu \mapsto \tensor{\Lambda}{^\mu_\nu} v^\nu $, and \bctxt{covariant 4-vectors} $ v_\mu $, which transform according to $ v_\mu \mapsto \tensor{\Lambda}{_\mu^\nu} v_\nu $. In this representation, the generators are represented as $ \C^{4 \times 4} $ matrices:
\begin{equation}
  \tensor{[J^{\mu \nu}]}{^\rho_\sigma} = \img \left( \eta^{\mu \rho} \delta^\nu_\sigma - \eta^{\nu \rho} \delta^\mu_\sigma \right)
  \label{eq:lor-gen}
\end{equation}
This is an irreducible representation, and the associated Lie algebra $ \lora $, called the \bctxt{Lorentz algebra}, is:
\begin{equation}
  [J^{\mu \nu}, J^{\sigma \rho}] = \img \left( \eta^{\nu \rho} J^{\mu \sigma} - \eta^{\mu \rho} J^{\nu \sigma} - \eta^{\nu \sigma} J^{\mu \rho} + \eta^{\mu \rho} J^{\nu \sigma} \right)
  \label{eq:lor-comm}
\end{equation}
...
\end{equation}
The $ \lora $ algebra can then be rewritten as:
\begin{equation}
  [J^i, J^j] = \img \epsilon^{ijk} J^k
  \qquad \qquad
  [J^i, K^j] = \img \epsilon^{ijk} K^k
  \qquad \qquad
  [K^i, K^j] = - \img \epsilon^{ijk} J^k
\end{equation}
...
so that:
\begin{equation}
  \Lambda = \exp \left[ -\img \bs{\theta} \cdot \ve{J} + \img \bs{\eta} \cdot \ve{K} \right]
  \label{eq:lor-better-transf}
\end{equation}
where $ \bs{\theta} , \bs{\eta} \in \R^3 $ and $ \ve{J} , \ve{K} \in (\R^{4 \times 4})^3 $. This definition reflect the \textit{alias} interpretation: angles define counterclockwise rotations of vectors with respect to a fixed reference frame, while rapidities define boosts which increase velocities with respect to said frame.

\footnotetext{To be precise, contravariant $ 4 $-vectors are elements of $ V \equiv \R^{1,3} $, so they transform according $ v_\mu \mapsto \tilde{v}_\mu \equiv \tensor{[\Lambda]}{^\mu_\nu} v^\nu $, while covariant $ 4 $-vectors are elements of $ V^* $, hence they require the transposed endomorphism (see Def. 1.4.2 in \cite{leonardo}) and inversely transform according to $ \tilde{v}_\mu \mapsto v_\mu \equiv \tensor{[\Lambda\tsp]}{_\mu^\nu} \tilde{v}_\nu \defeq \tilde{v}_\nu \tensor{[\Lambda]}{^\nu_\mu} $. To invert this relation note that, from \eref{eq:lor-def}, $ \tensor{[\Lambda^{-1}]}{^\mu_\nu} = \tensor{[\Lambda]}{_\nu^\mu} $, thus the transformation law is $ v_\mu \mapsto \tilde{v}_\mu \equiv \tensor{[\Lambda]}{_\mu^\nu} v_\nu $. The identification $ \tensor{[\Lambda]}{^\mu_\nu} \equiv \tensor{\Lambda}{^\mu_\nu} $ is an abuse of notation.}

\subsubsection{Tensor Representations}

Given the action of $ \lorg $ on covariant and contravariant vectors, it is clear to see that a generic $ (p,q) $-tensor transforms as:
\begin{equation}
...
Restricting the action of $ \lorg $ to its subgroup $ \SOn{3} $, tensors can be decomposed according to irreducible representations of $ \SOn{3} $, which are labelled by the angular momentum $ j \in \N_0 $ and are of degree $ n = 2j + 1 $. Also recall the Clebsch--Gordan composition of angular momenta:
\begin{equation}
  \ve{j}_1 \otimes \ve{j}_2 = \bigoplus_{j = \abs{j_1 - j_2}}^{j_1 + j_2} \ve{j}
\end{equation}
A Lorentz scalar $ \alpha $ is a scalar under rotations too, so $ \alpha \in \ve{0} $ (abuse of notation to denote that $ \alpha $ transforms according to the $ \ve{0} $ representation). A 4-vector $ v^\mu $ is irreducible under the action of $ \lorg $, but under $ \SOn{3} $ it is decomposed into $ v^0 $ and $ \ve{v} $, so $ v^\mu \in \ve{0} \oplus \ve{1} $. A $ (2,0) $-tensor then is:
\begin{equation*}
  \begin{split}
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation}
      \ve{J}_{\pm} \defeq \frac{1}{2} \left( \ve{J} \pm \img \ve{K} \right)
      \quad \implies \quad
      [\ve{J}_{\pm}^i , \ve{J}_{\pm}^j] = \img \epsilon^{ijk} \ve{J}_{\pm}^k
      \qquad
      [\ve{J}_{\pm}^i , \ve{J}_{\mp}^j] = \ve{0}
      \label{eq:lora-su2-gen}
    \end{equation}
    These are two commuting $ \mathfrak{su}(2) $ algebras, thus proving the thesis\footnote{To be precise, $ \lora_\C \cong \sua{2}_\C \oplus \sua{2}_\C $, while $ \lora \cong \sla{2,\C} $ since complexification yields the following isomorphisms (with \eref{eq:lora-su2-gen}):
    \begin{equation*}
...
More formally, the $ (\ve{j}_-,\ve{j}_+) $ representation is $ \rho_{(j_-,j_+)} : \lora \rightarrow \mathfrak{gl}(V_{(j_-,j_+)}) $, where $ V_{(j_-,j_+)} $ is a $ (2j_- + 1)(2j_+ + 1) $-dimensional $ \C $-vector space, defined up to similarity as (inverting \eref{eq:lora-su2-gen}):
\begin{equation}
  \begin{gathered}
    \rho_{(j_-,j_+)}(J^i) = J^i_{(j_+)} \otimes \mt{I}_{(2j_- + 1)} + \mt{I}_{(2j_+ + 1)} \otimes J^i_{(j_-)} \\
    \rho_{(j_-,j_+)}(K^i) = -\img \left[ J^i_{(j_+)} \otimes \mt{I}_{(2j_ + 1)} - \mt{I}_{(2j_+ + 1)} \otimes J^i_{(j_-)} \right]
  \end{gathered}
  \label{eq:lorentz-su2-repr}
\end{equation}
where $ \ve{J}_{(j)} $ is the $ (2j + 1) $-dimensional irreducible representation of $ \mathfrak{su}(2) $ and $ \otimes $ is the tensor product (see \dref{def:tensor-product-applications}).
For example, the trivial representation is $ \ve{\left( 0,0 \right)} $, as both $ \ve{J}_{\pm} = \ve{0} $ and $ \ve{J} = \ve{K} = \ve{0} $: this is the representation which acts on scalars.

\paragraph{Weyl representations}

$ \ve{\left( \frac{1}{2},0 \right)} $ and $ \ve{\left( 0,\frac{1}{2} \right)} $ are 2-dimensional spinorial representations. These representations act on different spinors $ (\psi_\text{L})_\alpha , (\psi_\text{R})_\beta \in \C^2 $, with $ \alpha,\beta = 1,2 $, which are respectively called \bctxt{left-} and \bctxt{right-handed  Weyl spinors}.

In $ \ve{\left( \frac{1}{2},0 \right)} $ the generators are $ \ve{J}_- = \frac{\boldsymbol{\sigma}}{2} $ and $ \ve{J}_+ = \ve{0} $, while in $ \ve{\left( 0,\frac{1}{2} \right)} $ they are $ \ve{J}_- = \ve{0} $ and $ \ve{J}_+ = \frac{\boldsymbol{\sigma}}{2} $, thus, by \eref{eq:lorentz-su2-repr}, $ \ve{J}_\text{L} = \ve{J}_\text{R} = \frac{\boldsymbol{\sigma}}{2} $ and $ \ve{K}_\text{L} = - \ve{K}_\text{R} = \img \frac{\boldsymbol{\sigma}}{2} $, so that from \eref{eq:lor-better-transf}:
\begin{equation}
  \psi_\text{L} \mapsto \Lambda_\text{L} \psi_\text{L} = \exp \left[ \left( -\img \boldsymbol{\theta} - \boldsymbol{\eta} \right) \cdot \frac{\boldsymbol{\sigma}}{2} \right] \psi_\text{L}
  \label{eq:lor-left-hand-weyl}
\end{equation}
\begin{equation}
  \psi_\text{R} \mapsto \Lambda_\text{R} \psi_\text{R} = \exp \left[ \left( -\img \boldsymbol{\theta} + \boldsymbol{\eta} \right) \cdot \frac{\boldsymbol{\sigma}}{2} \right] \psi_\text{R}
...
\begin{proofbox}
  \begin{proof}
    ...
    \end{equation*}
    where $ \sigma^2 \sigma^2 = \mt{I}_2 $ was used. The proof for $ \sigma^2 \psi_\text{R}^* $ is analogous.
  \end{proof}
\end{proofbox}

This allows to define \bctxt{charge conjugation} on Weyl spinors:
\begin{equation}
  \psi_\text{L}^c \defeq \img \sigma^2 \psi_\text{L}^*
  \qquad \qquad
  \psi_\text{R}^c \defeq -\img \sigma^2 \psi_\text{R}^*
\end{equation}
As of \pref{prop:weyl-spin-par}, charge conjugation transforms a left-handed Weyl spinor into a right-handed one and vice versa. Moreover, the $ \img $ factor ensures that applying this operator twice yields the identity operator.
...
\begin{definition}{Charge conjugation on Dirac spinors}{}
  ...
  \begin{equation}
    \Psi^c \defeq
    \begin{pmatrix}
      - \img \sigma^2 \xi_\text{R}^* \\
      \img \sigma^2 \psi_\text{L}^*
    \end{pmatrix}
    = -\img
    \begin{bmatrix}
      0 & \sigma^2 \\ -\sigma^2 & 0
...
This definition reflect the fact that charge conjugation changes the representation of the Weyl spinors (\pref{prop:weyl-spin-par}), i.e. the chirality of the components of the Dirac spinor.

\begin{observation}{4-vector representation}{}
  The other $ 4 $-dimensional representation of the Lorentz group is $ \ve{\left( \frac{1}{2},\frac{1}{2} \right)} $. To see that this is, in fact, the 4-vector representation, first consider the double-covering $ \lorg \cong \SL{2,\C} / \Z_2 $.

\begin{lemma}[before upper = {\tcbtitle}]{Double-covering of Lorentz group}{lor-double-cov}
  \begin{equation}
    \lorg \cong \SL{2,\C} / \Z_2
  \end{equation}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    Define the set $ \mathcal{H}(2) \defeq \{\mt{H} \in \C^{2 \times 2} : \mt{H}\dg = \mt{H}\} $ of all $ \C^{2 \times 2} $ Hermitian matrices. Trivially $ \mathcal{H}(2) \cong \R^{1,3} $, since:
    \begin{equation*}
      ...
      = V \in \mathcal{H}(2)
    \end{equation*}
    where the isomorphisms are $ V = v_\mu \sigma^\mu , v^\mu = \frac{1}{2} \tr \left( \bar{\sigma}^\mu V \right) $, with $ \sigma^\mu \equiv (\mt{I}_2,\bs{\sigma}) , \bar{\sigma}^\mu \equiv (\mt{I}_2,-\bs{\sigma}) $, since $ \{\mt{I}_2 , \sigma^1 , \sigma^2 , \sigma^3\} $ is a basis of $ \mathcal{H}(2) $.

    Now, consider $ \mt{M} \in \SL{2,\C} $ and define its linear action on $ \mathcal{H}(2) $ as $ V \mapsto \mt{M} V \mt{M}\dg $: then, $ \SL{2,\C} $ is an endomorphism group of $ \mathcal{H}(2) $, since its action preserves Hermiticity, and $ \det V \mapsto \det V $ as $ \det \mt{M} = 1 $ by definition. But $ \det V = v_\mu v^\mu $, hence the action of $ \SL{2,\C} $ leaves the Minkowski norm invariant: it is thus possible to define a group homomorphism $ \Phi : \SL{2,\C} \ra \On{1,3} $. WTS $ \Phi(\SL{2,\C}) \ssq \lorg $: since $ \SL{2,\C} $ is simply connected (it is homeomorphic to $ \Sn{3} \times \R^3 $) and $ \mt{I}_2 \in \SL{2,\C} $, then $ \Phi(\SL{2,\C}) $ must be connected, as $ \Phi $ can be shown to be continuous, and it must contain the identity, but the only connected compontent of $ \On{1,3} $ which satisfies these conditions is $ \lorg $, hence $ \Phi(\SL{2,\C)} \ssq \lorg $. Then:
    \begin{itemize}
      \item surjectivity: to show that $ \Phi $ surjective, recall that $ \lorg $ is generated by spatial (trigonometric) rotations and boosts (i.e. hyperbolic rotations). The former are clearly realized by $ \Phi $, since $ \SUn{2} < \SL{2,\C} $ and $ \Phi \vert_{\SUn{2}} $ is the standard double-covering map for $ \SOn{3} \cong \SUn{2} / \Z_2 $. Then, by polar decomposition, any $ \mt{M} \in \SL{2,\C} $ can be written as $ \mt{M} = \mt{U} \mt{P} $, with $ \mt{U} \in \SUn{2} $ and $ \mt{P} = \exp \left( \ve{a} \cdot \bs{\sigma} \right) , \ve{a} \in \R^3 $: since $ \mt{P} $ represents\footnote{
          First, note that, given a versor $ \hat{\ve{n}} \in \R^3 $, it is always possible to find a rotation $ \mt{U}_n \in \SUn{2} $ such that $ \hat{\ve{n}} \cdot \bs{\sigma} = \mt{U}_n \sigma^3 \mt{U}_n\dg $, hence $ \mt{P} = \mt{U}_a \exp (a \sigma^3) \mt{U}_a\dg $, thus WTS $ \exp (a \sigma^3) = \diag (e^a , e^{-a}) $ is a boost along the $ z $-axis:
          \begin{equation*}
            \begin{bmatrix}
              e^a & 0 \\ 0 & e^{-a}
            \end{bmatrix}
            ...
            =
            \begin{pmatrix}
              e^{2a} (v^0 - v^3) & - v^1 + \img v^2 \\
              - v^1 - \img v^2 & e^{-2a} (v^0 + v^3)
            \end{pmatrix}
          \end{equation*}
          i.e. $ v^0 \pm v^3 \mapsto e^{\mp 2a} (v^0 \pm v^3) $ and $ (v^1 , v^2) \mapsto (v^1 , v^2) $, which is equivalent to:
          \begin{equation*}
            \begin{pmatrix}
              v^0 \\ v^3
            \end{pmatrix}
            ...
            \end{pmatrix}
          \end{equation*}
        which is exactly the Lorentz transformation realizing a boost along the $ z $-axis.
      } a boost along the direction $ \hat{\ve{a}} $ of rapidity $ 2a $, all generators of $ \lorg $ are represented by $ \Phi $, i.e. $ \im\{\Phi\} = \lorg $.
    \item $ \ker \Phi = \{\mt{I}_2 , -\mt{I}_2\} $: solving $ \Phi(\mt{M}) = \id_{\lorg} $ means that $ \mt{M} V \mt{M}\dg = V \,\, \forall V \in \mathcal{H}(2) $. $ V = \mt{I}_2 $ results in $ \mt{M} \in \SUn{2} < \SL{2,\C} $, hand $ V = \sigma_i $ that $ \mt{M} $ commutes with all the Pauli matrices: this means that $ \mt{M} $ commutes with all elements of $ \mathcal{H}(2) $, hence it is a multiple of $ \mt{I}_2 $ by Schur's Lemma (applicable since the Pauli matrices form the basis of an irreducible representation of $ \SUn{2} $), and $ \det \mt{M} = 1 \implies \mt{M} = \pm \mt{I}_2 $.
    \end{itemize}
    Since $ \{\mt{I}_2 , -\mt{I}_2\} \cong \Z_2 $, by the first isomorphism theorem $ \lorg \cong \SL{2,\C} / \Z_2 $.
  \end{proof}
\end{proofbox}

As of \lref{lemma:lor-double-cov}, given $ \Lambda \in \lorg $, it is possible to associate to it $ \Lambda_\text{s} \in \SL{2,\C} $ such that:
\begin{equation}
  \tensor{\Lambda}{^\mu_\nu} \sigma^\nu = \Lambda_\text{s} \sigma^\mu \Lambda_\text{s}\dg
\end{equation}
Recalling the proof of \pref{prop:lorentz-algebra} $ \sla{2,\C} \cong \sua{2}_\C \oplus \sua{2}_\C $, hence:
\begin{equation*}
  \Lambda_\text{s} = \exp \left[ \left( \ve{a} + \img \ve{b} \right) \cdot \frac{\bs{\sigma}}{2} \right]
  \quad \implies \quad
  \Lambda_\text{s}\dg = \exp \left[ \left( - \ve{a} + \img \ve{b} \right) \cdot \frac{\bs{\sigma}\tsp}{2} \right]
\end{equation*}
where $ \sigma^2 \bs{\sigma} \sigma^2 = - \bs{\sigma}^\intercal $ was used. Since $ \exp(\img \alpha \sigma^k) = \mt{I}_2 \cos \alpha - \img \sigma^k \sin \alpha $, the exponential is periodic of $ 2\pi $ in $ \ve{b} $, so identify $ \ve{a} \equiv - \bs{\eta} $ and $ \ve{b} \equiv - \bs{\theta} $, i.e. $ \Lambda_\text{s} \equiv \Lambda_\text{L} $ and $ \Lambda_\text{s}\dg = \sigma^2 \Lambda_\text{R}\tsp \sigma^2 $: it is then clear that the representation of $ \lorg $ acting on 4-vectors is $ \ve{\left( \frac{1}{2},0 \right)} \otimes \ve{ \left( 0,\frac{1}{2} \right)} $, with\footnote{To be precise, this is a representation equivalent to $ \ve{\left( \frac{1}{2},0 \right)} \otimes \ve{\left( 0,\frac{1}{2} \right)} $, which would act on $ [\psi \otimes \xi]_{(ij)} = \psi_i \xi_j $.}:

\begin{equation*}
  (\psi \otimes \xi) \equiv \psi \xi\tsp \sigma^2
  \quad \implies \quad
  \Lambda (\psi \otimes \xi) = \Lambda_\text{s} (\psi \otimes \xi) \Lambda_\text{s}\dg = (\Lambda_\text{L} \psi) \otimes (\Lambda_\text{R} \xi)
\end{equation*}
Note that the restriction of $ (\psi \otimes \xi) $ being Hermitian correctly reduces the real dimension of the basis space from $ 8 $ to $ 4 $ (i.e. the basis space from $ \C^2 \otimes \C^2 $ to $ \mathcal{H}(2) $). Now, given the canonical isomorphism $ V \otimes W \cong W \otimes V $ for generic $ \K $-vector spaces, the following equivalence relation holds\footnotemark:
\begin{equation*}
  \ve{\left( \tfrac{1}{2},0 \right)} \otimes \ve{\left( 0,\tfrac{1}{2} \right)}
  \cong
  \ve{\left( \tfrac{1}{2} \otimes 0 , 0 \otimes \tfrac{1}{2} \right)}
  \cong
  \ve{\left( \tfrac{1}{2} , \tfrac{1}{2} \right)}
\end{equation*}
where Clebsch--Gordan decomposition was used. This shows that $ \ve{\left( \frac{1}{2},\frac{1}{2} \right)} $ is the representation of $ \lorg $ acting on 4-vectors.

Moreover, by Clebsch--Gordan decomposition, $ \ve{\left( \frac{1}{2},\frac{1}{2} \right)} \cong \ve{0} \oplus \ve{1} $, which is exactly the representation acting on 4-vectors ($ v^0 \in \ve{0} $ and $ \ve{v} \in \ve{1} $).
\end{observation}

\footnotetext{In particular, a consequence is that, given two Lie algebras $ \mathfrak{g} $ and $ \mathfrak{h} $, and given $ \rho_1 : \mathfrak{g} \ra V_1 $, $ \rho_2 : \mathfrak{g} \ra V_2 $, $ \pi_1 : \mathfrak{h} \ra W_1 $ and $ \pi_2 : \mathfrak{h} \ra W_2 $ their representations on $ \C $-vector spaces, then $ (\rho_1 \otimes \pi_1) \otimes (\rho_2 \otimes \pi_2) : (\mathfrak{g} \times \mathfrak{h}) \times (\mathfrak{g} \times \mathfrak{h}) \ra \mathfrak{gl}((V_1 \otimes W_1) \otimes (V_2 \otimes W_2)) $ and $ (\rho_1 \otimes \rho_2) \otimes (\pi_1 \otimes \pi_2) : (\mathfrak{g} \times \mathfrak{g}) \times (\mathfrak{h} \times \mathfrak{h}) \ra \mathfrak{gl}((V_1 \otimes V_2) \otimes (W_1 \otimes W_2)) $ are equivalent.}
...
\begin{align*}
  & \mt{I}_4 & \text{1 matrix}\,\,\,\, \\
  & \gamma^\mu & \text{4 matrices} \\
  & \gamma^{\mu \nu} \equiv \frac{1}{2} [\gamma^\mu, \gamma^\nu] \equiv \gamma^{[\mu} \gamma^{\nu]} & \text{6 matrices} \\
  & \gamma^{\mu \nu \rho} \equiv \gamma^{[\mu} \gamma^\nu \gamma^{\rho]} = \img \epsilon^{\mu \nu \rho \sigma} \gamma_\sigma \gamma^5 & \text{4 matrices} \\
  & \gamma^{\mu \nu \rho \sigma} \equiv \gamma^{[\mu} \gamma^\nu \gamma^\rho \gamma^{\sigma]} = - \img \epsilon^{\mu \nu \rho \sigma} \gamma^5 & \text{1 matrix}\,\,\,\,
\end{align*}
where $ \gamma^5 \defeq \img \gamma^0 \gamma^1 \gamma^2 \gamma^3 $ is an additional gamma matrix. It is also convention to use $ \sigma^{\mu \nu} \equiv \img \gamma^{\mu \nu} $. Note that this matrix is not in the basis of $ \dira $. It has the following properties:
\begin{equation*}
  (\gamma^5)\dg = \gamma^5
  \qquad
  (\gamma^5)^2 = \mt{I}_4
...
Moreover, $ \mathfrak{cl}_{1,3}^{(2)}(\C) \cong \spin(1,3) $ (see \secref{subsubsec:spin-groups}). Given a spinor $ \Psi \in \C^4 $, this spin group acts as:
\begin{equation*}
  \Psi \mapsto \Lambda_{\frac{1}{2}} \Psi = \exp \left[ -\frac{i}{2} \omega_{\mu \nu} S^{\mu \nu} \right] \Psi
\end{equation*}
Using the property $ (\gamma^\mu)\dg = \gamma^0 \gamma^\mu \gamma^0 $, it is easy to see that $ (S^{\mu \nu})\dg = - \gamma^0 S^{\mu \nu} \gamma^0 $, so, defining the \bctxt{Dirac dual} $ \bar{\Psi} \defeq \Psi\dg \gamma^0 $, this transforms via the inverse transformation:
\begin{equation*}
...
\begin{theorem}{Lorentz index of gamma matrices}{gamma-lorentz}
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \tensor{[J^{\rho \sigma}]}{^\mu_\nu} \gamma^\nu = \img \left( \eta^{\rho \mu} \gamma^\sigma - \eta^{\sigma \mu} \gamma^\rho \right) = \frac{\img}{2} \left( \gamma^\sigma \gamma^\rho \gamma^\mu + \gamma^\sigma \gamma^\mu \gamma^\rho - \gamma^\rho \gamma^\sigma \gamma^\mu - \gamma^\rho \gamma^\mu \gamma^\sigma \right)
    \end{equation*}
    As $ [\gamma^\mu , \gamma^\nu] = \{\gamma^\mu , \gamma^\nu\} - 2 \gamma^\nu \gamma^\mu = 2 \left( \eta^{\mu \nu} - \gamma^\nu \gamma^\mu \right) $:
    \begin{equation*}
      \begin{split}
        [\gamma^\mu , S^{\rho \sigma}]
        &= \frac{\img}{4} [\gamma^\mu , \gamma^\rho \gamma^\sigma - \gamma^\sigma \gamma^\rho] = \frac{\img}{4} \left( \gamma^\rho [\gamma^\mu , \gamma^\sigma] + [\gamma^\mu , \gamma^\rho] \gamma^\sigma - [\gamma^\mu , \gamma^\sigma] \gamma^\rho - \gamma^\sigma [\gamma^\mu , \gamma^\rho] \right) \\
        &= \frac{\img}{2} \left( - \gamma^\rho \gamma^\sigma \gamma^\mu - \gamma^\rho \gamma^\mu \gamma^\sigma + \gamma^\sigma \gamma^\mu \gamma^\rho + \gamma^\sigma \gamma^\rho \gamma^\mu \right) = \tensor{[J^{\rho \sigma}]}{^\mu_\nu} \gamma^\nu
      \end{split}
    \end{equation*}
    Expanding the thesis at first order:
    \begin{equation*}
      \tensor{\left[ \mt{I}_4 - \frac{\img}{2} \omega_{\rho \sigma} J^{\rho \sigma} \right]}{^\mu_\nu} \gamma^\nu = \left[ \mt{I}_4 + \frac{\img}{2} \omega_{\rho \sigma} S^{\rho \sigma} \right] \gamma^\mu \left[ \mt{I}_4 - \frac{\img}{2} \omega_{\rho \sigma} S^{\rho \sigma} \right]
    \end{equation*}
    which becomes:
    \begin{equation*}
      - \tensor{[J^{\rho \sigma}]}{^\mu_\nu} \gamma^\nu = S^{\rho \sigma} \gamma^\mu - \gamma^\mu S^{\rho \sigma}
    \end{equation*}
...
\begin{lemma}{(Anti)-self-dual tensor}{}
  ...
  \begin{equation}
    A^{\mu \nu} = \pm \frac{\img}{2} \epsilon^{\mu \nu \rho \sigma} A_{\rho \sigma}
  \end{equation}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    Consider $ A^{\mu \nu} = \frac{\img}{2} \epsilon^{\mu \nu \rho \sigma} A_{\rho \sigma} $:
    \begin{equation*}
      A^{\mu \nu}_+ = \frac{1}{2} \left( A^{\mu \nu} + \frac{\img}{2} \epsilon^{\mu \nu \rho \sigma} A_{\rho \sigma} \right) = \frac{1}{2} \left( \img \epsilon^{\mu \nu \rho \sigma} A_{\rho \sigma} \right) = A^{\mu \nu}
      \qquad
...
\begin{example}{Gauge theories}{}
  In gauge theories (e.g. QED and QCD), the gauge potential $ A_\mu $ transforms according to the fundamental representation $ \ve{\left( \frac{1}{2},\frac{1}{2} \right)} $, while the field strength $ F_{\mu \nu} $ according to the adjoint representation $ \ve{\left( 1,0 \right)} \oplus \ve{\left( 0,1 \right)} $.
\end{example}
...
\begin{equation*}
  \delta_0 \phi = \phi'(x) - \phi(x) = \phi'(x' - \delta x) - \phi(x) = - \delta x^\rho \pa_\rho \phi = \frac{\img}{2} \omega_{\mu \nu} \tensor{\left[ J^{\mu \nu} \right]}{^\rho_\sigma} x^\sigma \pa_\rho \phi \equiv - \frac{\img}{2} \omega_{\mu \nu} L^{\mu \nu} \phi
\end{equation*}
Recalling \eref{eq:lor-gen}, the generators can be expressed as:
\begin{equation}
  L^{\mu \nu} \defeq \img \left( x^\mu \pa^\nu - x^\nu \pa^\mu \right)
  \label{eq:ang-mom-oper}
\end{equation}
This is an infinite-dimensional representation, as it acts on the space of scalar fields. As $ p^\mu = \img \pa^\mu $ (with signature $ (+,-,-,-) $), it is clear that $ L^i \equiv \frac{1}{2} \epsilon^{ijk} L^{jk} $ is the orbital angular momentum.
...
\end{equation}
with $ \Lambda_\text{L} $ defined in \eref{eq:lor-left-hand-weyl}, and similarly for $ \Lambda_\text{R} $. The infinite-dimensional representation of the Lorentz generators determined by Weyl spinors can be found as:
\begin{equation*}
  \begin{split}
    \delta_0 \psi_\text{L}
    &\equiv \psi'_\text{L}(x) - \psi_\text{L}(x) = \psi'_\text{L}(x' - \delta x) - \psi_\text{L}(x) \\
    &= \psi'_\text{L}(x') - \delta x^\rho \pa_\rho \psi_\text{L}(x) - \psi_\text{L}(x) = \left( \Lambda_\text{L} - \mt{I}_2 \right) \psi_\text{L}(x) - \delta x^\rho \pa_\rho \psi_\text{L}(x)
  \end{split}
\end{equation*}
The second term yields $ L^{\mu \nu} $, while the first can be further elaborated by writing:
\begin{equation}
  \Lambda_\text{L} = e^{-\frac{\img}{2} \omega_{\mu \nu} S^{\mu \nu}}
\end{equation}
Thus:
\begin{equation*}
  \delta_0 \psi_\text{L} = - \frac{\img}{2} \omega_{\mu \nu} J^{\mu \nu} \psi_\text{L}
\end{equation*}
where the angular momentum separates into the orbital and the spin components:
\begin{equation}
  J^{\mu \nu} = L^{\mu \nu} + S^{\mu \nu}
\end{equation}
This separation is general: $ L^{\mu \nu} $ is always expressed as in \eref{eq:ang-mom-oper}, while $ S^{\mu \nu} $ depends on the specific representation. In the scalar representation $ S^{\mu \nu} = 0 $, while in the left/right-handed Weyl representation $ S^{i0} = \pm \img \frac{\sigma^i}{2} $.
...
\begin{equation*}
  \delta_0 \Psi \equiv \Psi'(x) - \Psi(x) = \Psi'(x') - \delta x^\rho \pa_\rho \Psi(x) - \Psi(x) = \left( \Lambda_\text{D} - \mt{I}_4 \right) \Psi(x) - \frac{\img}{2} \omega_{\mu \nu} L^{\mu \nu} \Psi(x)
\end{equation*}
The first term can be rewritten defining:
\begin{equation*}
  \Lambda_\text{D} = e^{-\frac{\img}{2} \omega_{\mu \nu} S^{\mu \nu}}
  \qquad \qquad
  S^{0i} \equiv - \frac{i}{2}
  \begin{bmatrix}
    \sigma^i & 0 \\ 0 & -\sigma^i
...
\begin{equation}
  \delta_0 \Psi = - \frac{\img}{2} \omega_{\mu \nu} J^{\mu \nu} \Psi
...
\begin{equation}
  \begin{gathered}
    [P^\mu,P^\nu] = 0 \\
    [J^{\mu \nu}, J^{\sigma \rho}] = \img \left( \eta^{\nu \rho} J^{\mu \sigma} - \eta^{\mu \rho} J^{\nu \sigma} - \eta^{\nu \sigma} J^{\mu \rho} + \eta^{\mu \rho} J^{\nu \sigma} \right) \\
    [P^\mu,J^{\rho \sigma}] = \img \left( \eta^{\mu \rho} P^\sigma - \eta^{\mu \sigma} P^\rho \right)
  \end{gathered}
\end{equation}
It's easy to check that $ [K^i,P^0] = \img P^i $, while $ [J^i,P^0] = [P^i,P^0] = 0 $: given that $ P^0 $ generates time translations, linear and angular momentum are conserved quantities, while $ \ve{K} $ is not.
...
\begin{equation*}
  \begin{split}
    \delta_0 \phi \equiv \phi'(x) - \phi(x)
    &= \phi'(x' - \varepsilon) - \phi(x) = - \varepsilon^\mu \pa_\mu \phi(x) \\
    &= e^{-\img (-\varepsilon_\mu) P^\mu} \phi'(x') - \phi(x) = \left( e^{\img \varepsilon_\mu P^\mu} - \mt{I} \right) \phi(x) = \img \varepsilon_\mu P^\mu \phi(x)
  \end{split}
\end{equation*}
...
\begin{equation}
  P^\mu = +\img \pa^\mu
\end{equation}
Explicitly, $ P^0 = \img \pa_t $ and $ \ve{P} = - \img \grad $. It is trivial to check that these generators obey the Poincaré algebra.
...
\end{equation}
where $ \Box \equiv \pa_\mu \pa^\mu $. A plane wave $ e^{\pm \img p_\mu x^\mu} $ is a solution if $ p^2 = m^2 $, so the KG equation imposes the relativistic dispersion relation and $ m $ can be interpreted as the mass. As $ \phi $ must be real, the general solution is a superposition of waves:
\begin{equation}
  \phi(x) = \int \frac{\dd^3 p}{(2\pi)^3 \sqrt{2E_\ve{p}}} \left[ a_\ve{p} e^{-\img p_\mu x^\mu} + a^*_\ve{p} e^{\img p_\mu x^\mu} \right]_{p^0 = E_\ve{p}}
  \label{eq:scalar-field-exp}
\end{equation}
The positive energy solution has $ E_\ve{p} = + \sqrt{\ve{p}^2 + m^2} $, but it contains both \bctxt{positive} and \bctxt{negative frequency modes} $ e^{\mp \img p_\mu x^\mu} $, while the $ (2E_\ve{p})^{-1/2} $ factor is a convenient normalization of the $ a_\ve{p} $ coefficients. The overall normalization of $ \act[\phi] $ does not influence the equations of motion, however it is important for obtaining a positive-definite Hamiltonian. As the momentum conjugate to $ \phi $ is $ \Pi_\phi = \pa_0 \phi $, the Hamiltonian density is found as:
\begin{equation}
...
\begin{equation*}
  M^{ij} = \int \dd^3 x \left( x^i \theta^{0j} - x^j \theta^{0i} \right) = \int \dd^3 x\, \pa_0 \phi \left( x^i \pa^j - x^j \pa^i \right) \phi = \frac{\img}{2} \int \dd^3 x \left[ \phi L^{ij} (\pa_0 \phi) - (\pa_0 \phi) L^{ij} \phi \right]
\end{equation*}
...
\begin{definition}{Scalar product}{kg-scal-prod}
  ...
  \begin{equation}
    \braket{\phi_1 | \phi_2} \defeq \frac{\img}{2} \int \dd^3 x\, \phi_1 \smlra{\pa_0} \phi_2
  \end{equation}
...
\begin{example}{Four-momentum}{}
  Applying \tref{th:cons-charge-val} to four-momentum $ P^\mu = \braket{\phi | \img \pa^\mu | \phi} $; for example, the $ \mu = 0 $ component is:
  \begin{equation*}
    \begin{split}
      P^0 &= \braket{\phi | \img \pa^0 | \phi} = \braket{\phi | \img \pa_0 | \phi} = \frac{\img}{2} \int \dd^3 x \left[ \phi (\img \pa_0) \pa_0 \phi - (\pa_0 \phi) \img \pa_0 \phi \right] \\
...
Consider now two real scalar fields $ \phi_1, \phi_2 $ with the same mass $ m $ and combine them into a single complex scalar field $ \phi \equiv \frac{1}{\sqrt{2}}(\phi_1 + \img \phi_2) $. The KG action is the sum of the single actions and may be written as:
\begin{equation}
...
\begin{equation}
  \phi(x) = \int \frac{\dd^3p}{(2\pi)^3 \sqrt{2E_\ve{p}}} \left[ a_\ve{p} e^{-\img p_\mu x^\mu} + b^*_\ve{p} e^{\img p_\mu x^\mu} \right]_{p^0 = E_\ve{p}}
...
An interesting property of the complex KG field is the existence of a global $ \Un{1} $ symmetry of the action: $ \act[\phi] \mapsto \act[\phi] $ under $ \phi(x) \mapsto e^{\img \theta} \phi(x) $. The associated Noether current can be computed from \eref{eq:noether-current}, using $ \phi_i = (\phi, \phi^*) $, $ A^\nu_a = 0 $ and $ F_{i,a} = (\img,-\img) $:
\begin{equation}
  j_\mu = -\img \left( \phi \pa_\mu \phi^* - \phi^* \pa_\mu \phi \right) = \img \phi^* \smlra{\pa_\mu} \phi
  \label{eq:comp-scal-field-curr}
\end{equation}
The conserved $ \Un{1} $ charge is then $ Q_{\Un{1}} = \img \int \dd^3x\, \phi^* \smlra{\pa_0} \phi = \braket{\phi | \phi} $, which is consistent with \tref{th:cons-charge-val} as the generator of $ \Un{1} $ is the identity operator.

\subsection{Spinor fields}

\subsubsection{Weyl fields}

Consider the theory of a left/right-handed Weyl field: recalling that $ \psi_\text{L}^\dagger \sigma^\mu \psi_\text{L} $ and $ \psi_\text{R} \bar{\sigma}^\mu \psi_\text{R} $ are 4-vectors, with $ \sigma^\mu \equiv (1, \bs{\sigma}) $ and $ \bar{\sigma}^\mu \equiv (1, - \bs{\sigma}) $, the kinetic term of the Lorentz-invariant Lagrangian can be written as:
\begin{equation}
  \lag_\text{L} = \img \psi_\text{L}^\dagger \bar{\sigma}^\mu \pa_\mu \psi_\text{L}
  \qquad \qquad
  \lag_\text{R} = \img \psi_\text{R}^\dagger \sigma^\mu \pa_\mu \psi_\text{R}
\end{equation}
The $ \img $ factor ensures the reality of the Lagrangian, as $ \sigma $ matrices are hermitian. The Lagrangian does not depend on $ \pa_\mu \psi^* $, thus the Euler-Lagrange equations are:
\begin{equation}
  (\pa_0 - \sigma^i \pa_i) \psi_\text{L} = 0
  \qquad \qquad
  (\pa_0 + \sigma^i \pa_i) \psi_\text{R} = 0
\end{equation}
These are known as \bctxt{Weyl equations}: by $ \sigma^i \sigma^j = \delta^{ij} + \img \epsilon^{ijk} \sigma^k $, these equations imply two massless KG equations (assuming regular functions, so that $ \pa_i \pa_j = \pa_j \pa_i $). Considering positive-energy plane-wave solutions of the form $ \psi_\text{L}(x) = u_\text{L} \exp(-\img p_\mu x^\mu) = u_\text{L} \exp(-\img E t + \img \ve{p} \cdot \ve{x}) $, where $ u_\text{L} $ is a constant spinor, Weyl equations become:
\begin{equation*}
  \frac{\ve{p} \cdot \bs{\sigma}}{E} u_\text{L} = - u_\text{L}
  \qquad \qquad
  \frac{\ve{p} \cdot \bs{\sigma}}{E} u_\text{R} = u_\text{R}
\end{equation*}
As these are massless fields $ E = \abs{\ve{p}} $, and since for $ s = \frac{1}{2} $ fields $ \ve{J} = \frac{\bs{\sigma}}{2} $ these equations become:
\begin{equation*}
...
\begin{equation}
  \theta^{\mu \nu} = \img \psi_\text{L}^\dagger \bar{\sigma}^\mu \pa^\nu \psi_\text{L}
  \qquad \qquad
  \theta^{\mu \nu} = \img \psi_\text{R}^\dagger \sigma^\mu \pa^\nu \psi_\text{R}
\end{equation}
...
Consider a theory with both a left-handed and a right-handed Weyl spinor: one can construct two new Lorentz scalars, $ \psi_\text{L}^\dagger \psi_\text{R} $ and $ \psi_\text{R}^\dagger \psi_\text{L} $, as from \eeref{eq:lor-left-hand-weyl}{eq:lor-right-hand-weyl} it is easy to check that $ \Lambda_\text{L}^\dagger \Lambda_\text{R} = \Lambda_\text{R}^\dagger \Lambda_\text{L} = \id $. Two real combinations are $ \psi_\text{L}^\dagger \psi_\text{R} + \psi_\text{R}^\dagger \psi_\text{L} $ and $ \img (\psi_\text{L}^\dagger \psi_\text{R} - \psi_\text{R}^\dagger \psi_\text{L}) $: under parity $ \psi_\text{L} \leftrightarrow \psi_\text{R} $, so the former is a scalar and the latter a pseudoscalar. The Dirac Lagrangian then is:
\begin{equation}
  \lag_\text{D} = \img \psi_\text{L}^\dagger \bar{\sigma}^\mu \pa_\mu \psi_\text{L} + \img \psi_\text{R}^\dagger \sigma^\mu \pa_\mu \psi_\text{R} - m ( \psi_\text{L}^\dagger \psi_\text{R} + \psi_\text{R}^\dagger \psi_\text{L} )
\end{equation}
...
\begin{equation}
  \img \bar{\sigma}^\mu \pa_\mu \psi_\text{L} = m \psi_\text{R}
  \qquad \qquad
  \img \sigma^\mu \pa_\mu \psi_\text{R} = m \psi_\text{L}
\end{equation}
which is the \bctxt{Dirac equation} in terms of Weyl spinors.

\begin{proposition}{}{}
  The Dirac equation implies two massive Klein-Gordon equations.
\end{proposition}

\begin{proofbox}
  \begin{proof}
    Applying $ \img \sigma^\mu \pa_\mu $ to the first equation:
    \begin{equation*}
      - \sigma^\mu \bar{\sigma}^\nu \pa_\mu \pa_\nu \psi_\text{L} = m \img \sigma^\mu \pa_\mu \psi_\text{R} = m^2 \psi_\text{L}
    \end{equation*}
    Assuming that $ \Psi_\text{L} $ satisfies the hypotheses of Schwarz's Lemma, then $ \pa_\mu \pa_\nu = \pa_\nu \pa_\mu $, and $ \sigma^\mu \bar{\sigma}^\nu $ can be replaced by $ \frac{1}{2} (\sigma^\nu \bar{\sigma}^\mu + \sigma^\nu \bar{\sigma}^\mu) = \frac{1}{2} (2 \eta^{\mu \nu}) = \eta^{\mu \nu} $, yielding the thesis.
  \end{proof}
\end{proofbox}
...
\begin{equation}
  \gamma^0 = \sigma^1 \otimes \mt{I}_2
  \qquad \qquad
  \bs{\gamma} = \img \sigma^2 \otimes \bs{\sigma}
  \qquad \qquad
  \gamma^5 = - \sigma^3 \otimes \mt{I}_2
\end{equation}}
%
This allows to rewrite Dirac equation as:
\begin{equation}
  (\img \slashed{\pa} - m) \Psi = 0
\end{equation}
...
\begin{equation}
  \lag_\text{D} = \bar{\Psi} (\img \slashed{\pa} - m) \Psi
  \label{eq:dirac-lagrangian}
\end{equation}
The $ \gamma^5 $ matrix in the chiral representation allows to define the projectors on left- and right-handed Weyl spinors:
\begin{equation}
  \gamma^5 \equiv
  \begin{bmatrix}
    -\mt{I}_2 & 0 \\ 0 & \mt{I}_2
...
\begin{proposition}{Unitary equivalence}{dirac-unit-eq}
  Given a constant $ \mt{U} \in \C^{2 \times 2} : \mt{U}^\dagger = \mt{U}^{-1} $, then under $ \Psi \mapsto \mt{U} \Psi $ the Lagrangian is invariant for $ \gamma^\mu \mapsto \mt{U} \gamma^\mu \mt{U}^\dagger $.
\end{proposition}

\begin{proofbox}
  \begin{proof}
    Inserting $ \Psi = \mt{U}^\dagger \Psi' $ (and $ \bar{\Psi} = \Psi'^\dagger \mt{U} \gamma^0 $) in the Dirac Lagrangian:
    \begin{equation*}
      \lag_\text{D} = \Psi'^\dagger \mt{U} \gamma^0 (\img \gamma^\mu \pa_\mu - m) \mt{U}^\dagger \Psi' = \Psi'^\dagger \mt{U} \gamma^0 \mt{U}^\dagger (\img \mt{U} \gamma^\mu \mt{U}^\dagger \pa_\mu - m) \Psi' = \bar{\Psi}' (\img \gamma'^\mu \pa_\mu - m) \Psi'
    \end{equation*}
...
\begin{proposition}{Lorentz invariance}{}
  The Dirac equation is Lorentz invariant.
\end{proposition}
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \begin{split}
        \left( \img \gamma^\mu \pa_\mu - m \right) \Psi(x)
        &\mapsto \left( \img \gamma^\mu \tensor{[\Lambda^{-1}]}{^\nu_\mu} \pa_\nu - m \right) \Lambda_{\frac{1}{2}} \Psi(\Lambda^{-1}x) \\
        &= \Lambda_{\frac{1}{2}} \Lambda_{\frac{1}{2}}^{-1} \left( \img \gamma^\mu \tensor{[\Lambda^{-1}]}{^\nu_\mu} \pa_\nu - m \right) \Lambda_{\frac{1}{2}} \Psi(\Lambda^{-1}x) \\
        &= \Lambda_{\frac{1}{2}} \left( \img \Lambda_{\frac{1}{2}}^{-1} \gamma^\mu \Lambda_{\frac{1}{2}} \tensor{[\Lambda^{-1}]}{^\nu_\mu} \pa_\nu - m \right) \Psi(\Lambda^{-1}x) \\
        &= \Lambda_{\frac{1}{2}} \left( \img \tensor{\Lambda}{^\mu_\sigma} \gamma^\sigma \tensor{[\Lambda^{-1}]}{^\nu_\mu} \pa_\nu - m \right) \Psi(\Lambda^{-1}x) \\
        &= \Lambda_{\frac{1}{2}} \left( \img \gamma^\nu \pa_\nu - m \right) \Psi(\Lambda^{-1}x)
      \end{split}
    \end{equation*}
    Since $ 0 $ is a Lorentz scalar, this transformation law implies that $ \left( \img \slashed{\pa} - m \right) \Psi(y) = 0 $, where $ y = \Lambda^{-1}x $ are the coordinates in the new reference frame. This shows that the Dirac equation is Lorentz invariant.
  \end{proof}
\end{proofbox}

\paragraph{Solutions}

The general solution to the Dirac equation is a superposition of plane waves, both with positive and negative-frequency modes respectively written as $ \Psi(x) = u(p) e^{-\img p_\mu x^\mu} $ and $ \Psi(x) = v(p) e^{\img p_\mu x^\mu} $, where $ u(p) $ and $ v(p) $ are Dirac spinors which, in the chiral representation, have both a left-handed and a right-handed Weyl spinor component. The Dirac equation than becomes:
\begin{equation}
...
\begin{proofbox}
  \begin{proof}
    Using $ \sum_{s = 1,2} \xi^s \xi^{s \dagger} = \mt{I}_2 $:
    \begin{equation*}
      \begin{split}
...
\begin{equation*}
  \psi_\text{L} \mapsto e^{\img \theta_\text{L}} \psi_\text{L}
  \qquad \qquad
  \psi_\text{R} \mapsto e^{\img \theta_\text{R}} \psi_\text{R}
\end{equation*}
...
\begin{equation}
  \Psi \mapsto e^{\img \alpha} \Psi
  \label{eq:dirac-u1-symm}
\end{equation}
\begin{equation}
  \Psi \mapsto e^{\img \beta \gamma^5} \Psi
\end{equation}
where $ \alpha \equiv \theta_\text{L} = \theta_\text{R} $ and $ \beta \equiv - \theta_\text{L} = \theta_\text{R} $ respectively (to prove that the latter is a symmetry, from $ \{\gamma^5, \gamma^\mu\} = 0 $ it follows that $ \gamma^\mu e^{\img \beta \gamma^5} = e^{-\img \beta \gamma^5} \gamma^\mu $). The former symmetry is called the vector $ \Un{1}_\text{V} $ symmetry, with conserved Noether current the \bctxt{vector current} $ j^\mu_\text{V} \defeq \bar{\Psi} \gamma^\mu \Psi $, while the latter is called the axial $ \Un{1}_\text{A} $ symmetry, with conserved \bctxt{axial current} $ j^\mu_\text{A} \defeq \bar{\Psi} \gamma^\mu \gamma^5 \Psi $.\\
...
\begin{equation}
  \pa_\mu j^\mu_\text{A} = 2 \img m \bar{\Psi} \gamma^5 \Psi
\end{equation}
