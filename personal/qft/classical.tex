...
\end{equation}
This definition reflect the \textit{alias} interpretation: angles define counterclockwise rotations of vectors with respect to a fixed reference frame, while rapidities define boosts which increase velocities with respect to said frame.
...
Restricting the action of $ \lorg $ to its subgroup $ \SOn{3} $, tensors can be decomposed according to irreducible representations of $ \SOn{3} $, which are labelled by the angular momentum $ j \in \N_0 $ and are of degree $ n = 2j + 1 $. Also recall the Clebsh-Gordan composition of angular momenta:
\begin{equation}
...
\begin{proofbox}
  \begin{proof}
    \begin{equation*}
      ...
      = V \in \mathcal{H}(2)
    \end{equation*}
    where the isomorphisms are $ V = v_\mu \sigma^\mu , v^\mu = \frac{1}{2} \tr \left( \bar{\sigma}^\mu V \right) $, with $ \sigma^\mu \equiv (\mt{I}_2,\bs{\sigma}) , \bar{\sigma}^\mu = (\mt{I}_2,-\bs{\sigma}) $.
  \end{proof}
\end{proofbox}
