...
\end{equation}
In this case, the propagator already has a pole at $ k^2 = 0 $, hence it is sufficient to fix its pole to $ -\img \eta_{\mu \nu} + \left( k_\mu k_\nu \text{ terms} \right) $, i.e.:
\begin{equation}
  \Pi_\text{r}(0) = 0
\end{equation}
...
\end{equation}
This dependence of the physical coupling on the energy scale is known as the \bctxt{running of the coupling}, and it is effectively a prediction of the renormalization of thetheory (which has been verified experimentally).
...
\begin{equation*}
  \tilde{G}^{(k,n)}(\{p_i\} , \alpha , m , \Lambda) \equiv \braket{0 | \tempord\{A(p_1) \dots A(p_k) \Psi{p_{k + 1}} \dots \Psi_{p_{k + n}} \} | 0}
\end{equation*}
...
\end{equation*}
where the regulator $ \Lambda $ is substituted by the renormalization scale $ \mu $. Note that $ \tilde{G}_\text{r} $, $ \alpha_\text{r} $ and $ m_\text{r} $ are finite, while $ \tilde{G} $, $ \alpha $, $ m $, $ Z_2 $ and $ Z_3 $ are divergent with respect to the regulator (i.e. for $ \Lambda \ra \infty $ with a UV cut-off or $ \epsilon \ra 0 $ in dimensional regularization): in particular, $ \tilde{G} $, $ \alpha $ and $ m $ only depend on the regulator $ \Lambda $ and not on the renormalization point $ \mu $, hence taking the derivative with respect to $ \log \mu^2 $ yields:
\begin{equation*}
  0 = \frac{k}{2} Z_3^{k/2} \frac{1}{Z_3} \frac{\pa Z_3}{\pa \log \mu^2} Z_2^{n/2} \tilde{G}_\text{r}^{(k,n)} + Z_3^{k/2} \frac{n}{2} \frac{1}{Z_2} \frac{\pa Z_2}{\pa \log \mu^2} \tilde{G}_\text{r}^{(k,n)} + Z_3^{k/2} Z_2^{n/2} \frac{\dd \tilde{G}_\text{r}^{(k,n)}}{\dd \log \mu}
\end{equation*}
...
\end{equation}
where $ Z_i $ is the renormalization coefficient of the field strength $ \phi_i = Z_i \phi^\text{r}_i $. Note that the anomalous dimension is independent of the regulator, contrary to $ Z_i $, but only on the various couplings of the theory which affect $ \phi_i $.
