...
\begin{equation*}
  ...
  \end{tikzpicture}
  \quad \implies \quad
  \dd \sigma^\text{LO} = \frac{1}{\Phi} \frac{\dd^3p_3}{(2\pi)^3 2E_{p_3}} \frac{\dd^3p_4}{(2\pi)^3 2E_{p_4}} (2\pi)^4 \delta^{(4)}(p_1 + p_2 - p_3 - p_4) \lambda^2
\end{equation*}
In the CM frame:
\begin{equation*}
  p_1 = (E, 0, 0, p)
...
\begin{observation}{Wick rotations}{wick-rotations}
  Due to Lorentz invariance, the general form a $ d $-dimensional loop integral is:
  \begin{equation*}
    I = \int \dd^d k \, f(k^2)
  \end{equation*}
  However, these kinds of integrals cannot be treated with the standard techniques of Analysis, since the measure $ \dd^d k $ is defined on the Lorentzian manifold $ \R^{1,d-1} $ with Minkowskian metric $ \eta_{\mu \nu} = (+1 , -1 , \dots , -1) $. In order to treat them in a standard way, consider first the integral $ I_0 \equiv \int_\R \dd k^0 f(k^2) $ on the temporal component: by extending $ k^0 $ to the whole complex plane $ \C $, this integral turns into the integral of a meromorphic%
  \footnote{Given an open set $ D \subset \C $, then $ f : D \rightarrow \C $ is \textit{meromorphic} if it is holomorphic on $ D - P $, where $ P \subset D $ is a set of isolated points called \textit{poles}. Recall that a function $ f : D \rightarrow \C $ is \textit{holomorphic} on $ D $ if it is complex differentiable at every point in $ D $.}
  function $ f : \C \ra \C $ over the path $ \ell_\infty \equiv \R $ (identifying the curve with its support). In particular, consider the case of massive particles only, so that the poles of $ f(k^2) $ have non-vanishing real parts: due to the general form $ k^2 - m^2 + \img \epsilon $ of the denominators of propagators, the poles come in couples of the form $ k^0_\pm = \pm \sqrt{m^2 + \ve{k}^2} \mp \img \epsilon $. For example, the case of a single propagator is drawn as:
  \begin{equation*}
    \begin{tikzpicture}
      \def\bigradius{3}
      ...
      \draw [line width=1pt,
        ...
        postaction={decorate}]
        (\bigradius , 0) arc (0 : 90 : \bigradius);
        \node at (1.7 , 2.8) {$ \gamma_R $};

      \draw [line width=1pt,
        decoration={markings,
          mark=at position 0.5 with {\arrow[line width=1.2pt]{>}}},
        postaction={decorate}]
        (0 , -\bigradius) arc (270 : 180 : \bigradius);
        \node at (-2.8 , -1.7) {$ \gamma'_R $};
      ...
      \draw [line width=1pt,
        decoration={markings,
          ...
          mark=at position 0.75 with {\arrow[line width=1.2pt]{>}}},
        postaction={decorate}]
        (0 , \bigradius) -- (0 , -\bigradius);
        \node at (-0.27 , -1) {$ \ell'_R $};

      % poles
      \fill [black] (2 , -0.5) circle [radius = 2pt];
        \node at (2.35 , -0.8) {$ k^0_+ $};
      \fill [black] (-2 , 0.5) circle [radius = 2pt];
        \node at (-2.35 , 0.8) {$ k^0_- $};

      % label
      \draw [line width=0.2pt]
        (\bigradius , 1.25 * \bigradius) -- (\bigradius , \bigradius)
        -- (1.25 * \bigradius , \bigradius);
        \node at (1.15 * \bigradius , 1.15 * \bigradius) {$ k^0 $};
      \end{tikzpicture}
  \end{equation*}
  Considering the paths drawn in this figure, it is clear that, in the case of massive particle, the poles of $ f(k^2) $ will always be outside of the total closed path $ \gamma $; hence, by Cauchy's residue theorem:
  \begin{equation*}
    \oint_\gamma \dd k^0 f(k^2) = 0
  \end{equation*}
  The integrals over the arcs $ \gamma_R $ and $ \gamma'_R $ vanish for $ R \ra \infty $, hence this equation reduces to:
  \begin{equation*}
    \int_{-\infty}^\infty \dd k^0 f(k^2) = \int_{-\img\infty}^{\img\infty} \dd k^0 f(k^2)
  \end{equation*}
  Then, substituting $ k^0_\text{E} = \img k^0 , \ve{k}_\text{E} = \ve{k} $ in the second integral, i.e. the so-called \bcobs{Wick rotation}, the main result of this trick is found:
  \begin{equation*}
    \int_\R \dd k^0 f(k^2) = \img \int_\R \dd k^0_\text{E} f(-k_\text{E}^2)
    \quad \implies \quad
    \int \dd^d k \, f(k^2) = \img \int \dd^d k_\text{E} f(-k_\text{E}^2)
  \end{equation*}
  where the Euclidean momentum is now defined on the Euclidean manifold $ \R^d $ with Euclidean metric $ \delta_{\mu \nu} = (+1 , +1 , \dots , +1) $. The Wick rotation thus turns a Minkowskian integral into a Euclidean integral, which can be treated with the standard techniques of Analysis.
\end{observation}
...
The process of renormalization, which allows to obtain finite results from diverging amplitudes, can be implemented in various ways. That of introducing a UV cut-off $ \Lambda $, expressing the bare observables in terms of the physical observables at some scale $ \mu $ and then setting $ \Lambda \rightarrow \infty $ is rather inefficient at higher orders in perturbation theory: thus, a more efficient regularization scheme must be adopted. The main regulators which are typically used are:
\begin{itemize}
  \item cut-off regulator: a parameter $ \Lambda \in \R^+ $ is introduced to cut off the UV regime, i.e. imposing $ k_\text{E}^2 \leq \Lambda^2 $, so that the domain of UV divergences is removed entirely from the integration. However, this regulator effectively cuts off the infinite number of multi-particle states that can be produced at high energies (see \figref{fig:mass-2}), thus violating unitarity (and gauge symmetry, too);
  \item lattice regularization: to avoid the high-momentum regime, a discretization parameter $ a \in \R^+ $ is introduced, so that the position space is discretized as $ x^\mu = n^\mu a $, with $ n^\mu \in \Z^d $, thus reducing the momentum space to the first Brillouin zone $ \abs{k^\mu} \leq \frac{2\pi}{a} \,\, \forall \mu = 0, 1, \dots, d-1 $. Clearly, this regularization scheme is analogous to the UV cut-off, therefore it carries the same problems of violation of unitarity and gauge symmetry, with the addition of violation of Lorentz symmetry too (it is not possible to define trigonometric and hyperbolic rotations in a discretized space);
  \item convergent propagators: UV divergences can be cured by making propagators more convergent with additional higher-order terms in the denominator. For example, adding a term like $ + \frac{1}{\Lambda^2} \phi \dal \phi $ to the Lagrangian for a scalar field, the Feynman propagator becomes $ \img \fprp^{-1}(p) = p^2 + \frac{1}{\Lambda^2} p^4 - m^2 $, which is more convergent than the usual propagator. This higher-order terms, however, may break the symmetries of the original Lagrangians;
  \item Pauli-Villars regulator: the UV behavior of loop integrals is modulated by the introduction of auxiliary particles, which are then eliminated by taking their masses to infinity. This regulator preserver gauge invariance in abelian gauge theories, but it is not gauge covariant in non-abelian gauge theories;
  \item dimensional regularization: in this scheme, the regulator is the number of dimensions, and in fact it preserves both unitarity, gauge symmetry and Lorentz invariance.
\end{itemize}

\begin{observation}{Dimensional regularization}{}
  In general, the dimensional regularization scheme consists in the analytic continuation of loop momenta to $ d = 4 - 2 \epsilon $, with $ \epsilon \in \C : \Re \epsilon < 0 $. This procedure turns loop integrals into meromorphic functions of $ \epsilon \in \C $, allowing for the isolation of divergences as poles in $ \epsilon $.

  The dimensional regularization prescription leaves freedom in choosing the dimensionality of external momenta, as well as the number of polarizations of both external and internal particles, thus allowing for the definition of different regularization schemes: the \bcobs{conventional dimensional regularization} (CDR), in which all momenta and polarization are analytically continued to $ d $ dimensions, and the \bcobs{'t Hooft--Veltman scheme} (HV), in which only internal momenta and polarizations are.

  When considering non-chiral gauge theories like QED and QCD, CDR is the most natural choice, as the main difference between CDR and HV is the treatment of purely $ 4 $-dimensional objects, i.e. $ \gamma^5 $ and $ \epsilon^{\mu \nu \sigma \rho} $. In particular, in CDR both the Dirac algebra and Lorentz indices are analytically continued to $ d $ dimensions, leading to a mathematical inconsistency when $ d \notin \N $.
  In $ 4 $-dimensional Minkowski space $ \R^{1,3} $ with metric signature $ \eta = (+,-,-,-) $, the Dirac algebra is defined as $ \dira \cong \clar{1,3} \otimes \C $ and it admits a matrix representation with generators $ \{\gamma^\mu\}_{\mu = 0, 1, 2, 3} \subset \C^{4 \times 4} $ such that:
  \begin{equation}
    \{\gamma^\mu , \gamma^\nu\} = 2 \eta^{\mu \nu} \mt{I}_4
    \label{eq:dir-alg}
  \end{equation}
  ...
  To solve this issue, in CDR spinor indices remain $ 4 $-dimensional: this is realized considering a matrix representation generated by $ \{\gamma^\mu\}_{\mu = 0, \dots, d-1} \subset \C^{4 \times 4} $ and imposing the formal relations \eref{eq:dir-alg}. Consistency is achieved with the analytical continuation of trace identities: indeed, traces obtained by recursively applying \eref{eq:dir-alg} (like $ \tr\{\gamma^\mu \gamma^\nu\} = 4 \eta^{\mu \nu} $) are still valid in $ d $-dimensions, as the only dependence on dimension comes from contractions such as $ \tensor{\eta}{^\mu_\mu} = d $.
...
\begin{proposition}{Dimensionality in CDR}{}
  In $ d = 4 - 2\epsilon $:
  \begin{equation}
    [\phi] = [M]^{1 - \epsilon}
    \qquad \qquad
...
\begin{itemize}
  \item super-renormalizable theories: the coupling constant has strictly positive mass dimension, hence there is only a finite number of diverging diagrams.
...
\begin{example}{Fermi theory}{}
  Consider the Fermi theory for $ \beta^- $ decay, which describes the decay $ n^0 \ra p^+ + e^- + \bar{\nu}_e $ as a 4-point vertex with interaction Lagrangian:
  \begin{equation}
    \lag_\text{F,int} = - G_\text{F} \bar{\Psi}_n \Psi_p \bar{\Psi}_\nu \Psi_e
  \end{equation}
  Since $ [\Psi] = \frac{d - 1}{2} $, then $ [G_\text{F}] = 2 - d $, which makes the Fermi theory clearly non-renormalizable in $ d = 4 $. This, however, hints to the fact that this theory is only a low-energy approximation (i.e. an ``effective" field theory) of the theory of the weak interaction, whose gauge bosons ($ W^\pm $ and $ Z^0 $) are massive and therefore short-lived, making the interaction a short-range one (and indeed approximated by a 4-point vertex).

  Since the $ \beta^- $ decay is mediated by a $ W^- $ boson, the mass scale of the coupling is expected to be $ M_W $, which is exactly the case:
  \begin{equation*}
    G_\text{F} = \frac{\sqrt{2}}{8} \frac{g^2}{M_W^2} = 1.1663787(6) \cdot 10^{-5} \gev^{-2}
  \end{equation*}
  where $ g $ is the coupling constant of the weak interaction.
\end{example}
...
% TODO: write the explicit expressions for \delta_\lambda, \delta_Z and \delta_m found on OneNote in "Orale 1"

\newpage

\section{Renormalization of QED}

Recall the QED Lagrangian:
\begin{equation}
  \lag_\text{QED} = - \frac{1}{4} F_{\mu \nu} F^{\mu \nu} + \bar{\Psi} \left( \img \slashed{\pa} - m \right) \Psi - e \bar{\Psi} \slashed{A} \Psi
  \label{eq:qed-lagrangian}
\end{equation}
which is implicitly assumed to be normal-ordered. The interaction between the fermion field $ \Psi $ and the gauge field $ A_\mu $ is expressed by the interaction Lagrangian $ \lag_\text{int} = - e \bar{\Psi} \slashed{A} \Psi $, which depends on the coupling constant $ e $.

\begin{observation}{Renormalizablity of QED}{}
  Consider the QED Lagrangian \eref{eq:qed-lagrangian} in $ d $ dimensions. Then, imposing $ [\lag_\text{QED}] = d $ and $ [m] = [\slashed{\pa}] = 1 $, it is trivial to see that:
  \begin{equation}
    [A_\mu] = \frac{d - 2}{2}
    \qquad
    [\Psi] = \frac{d - 1}{2}
  ...
  \end{equation}
  Clearly, then, if $ d > 4 $ there is an infinite number of classes of diverging diagrams, since $ D $ increases as $ V $ increases. On the contrary, for $ d = 4 $ QED is renormalizable, as there are only finitely-many classes of diverging diagrams (those with $ N_\text{b} + \frac{3}{2} N_\text{f} \leq 4 $), while for $ d < 4 $ it is super-renormalizable, as there are only finitely-many diverging diagrams.
\end{observation}
...
\begin{example}{Corrections to the photon propagator}{}
  ...
  \begin{equation*}
    \gpr_{\mu \nu}(x - y) \hspace{-0.1em} = \hspace{-0.1em} \sum_{n = 0}^\infty \frac{(- \img e)^n}{n!} \hspace{-0.2em} \int \hspace{-0.2em} \dd x_1 \dots \dd x_n \braket{0 | \tempord\{ A_\mu(x) A_\nu(y) j^{\alpha_1} \hspace{-0.1em} (x_1) A_{\alpha_1} \hspace{-0.1em} (x_1) \dots j^{\alpha_n} \hspace{-0.1em} (x_n) A_{\alpha_n} \hspace{-0.1em} (x_n) \} | 0}
  \end{equation*}
  where the fermionic vector current is defined as $ j^\mu \defeq \bar{\Psi} \gamma^\mu \Psi $. Now, it is possible to apply Wick's theorem: e.g. up to $ \smo(e^2) $ order:
  \begin{equation*}
      \gpr_{\mu \nu}(x - y)
      =
      \begin{tikzpicture}[baseline = (r.base)]
      ...
      \quad + \quad \smo(e^4)
  \end{equation*}
  since, due to the form of the interaction vertex $ \lag_\text{int} \sim A_\mu $, only terms with $ n \in \N_0 $ even give non-vanishing contributions. Therefore, all power series in QED can be expressed either in terms of the coupling constant $ e $ or of the structure constant $ \alpha \equiv \frac{e^2}{4\pi} $: indeed, it is useful to define the following notation for $ n \in \N_0 $ (highest number of loops in the corrections):
  \begin{equation}
    \gpr_{\mu \nu}(x - y) \equiv \gpr_{\mu \nu} \od n (x - y) + \smo(\alpha^{n+1})
  \end{equation}
  ...
  \end{equation}
  Hence, each blob is itself a power series in $ \alpha $.
\end{example}
...
\begin{equation*}
    ...
    \ \sim \log \Lambda
\end{equation*}
These are respectively the corrections to the photon propagator (collectively denoted as vacuum polarization), to the electron propagator (denoted as electron self-energy) and to the electron-photon interaction vertex.

Therefore, since there are three classes of diverging diagram, the QED Lagrangian is expected to be renormalized with three counterterms.

\newpage

\subsection{Diverging amplitudes}

\subsubsection{Interaction vertex}

First, consider the electron-photon vertex, which at 1-loop order can be written as:
\begin{equation*}
    \begin{tikzpicture}[baseline = (r.base)]
      \begin{feynman}[inline = (r.base)]
        \vertex (a) {};
...
\end{equation*}
where the dependence on $ q $ only is to be proved. Note that vacuum polarization and electron self-energy diagrams have been omitted: these are indeed only corrections to the photon and electron propagators, and are not relevant to determine the vertex counterterm.
...
\begin{proofbox}
  \begin{proof}
    Denoting the incoming and outgoing fermionic momenta with $ p $ and $ p' $, in general $ \Gamma^\mu $ is expressed in terms of $ \gamma^\mu $, since $ \Gamma\od0_\mu = \gamma_\mu $, of the 4-vectors $ p $ and $ p' $ and of the constants $ e $ and $ m $. This is an exhaustive list, since no other objects appear in the Feynman rules for the diagrams involved\footnote{In principle $ \epsilon^{\mu \nu \rho \sigma} $ (or equivalently $ \gamma^5 $) could be present too, but it is forbidden by the fact that QED is a parity-conserving theory.}.
...
The vertex function, and in particular the form factors, contains all the information on how an electron is influenced by an electromagnetic field. In presence of an external classical field $ A^\text{cl}_\mu(x) $, the QED Lagrangian is modified with an additional interaction term $ - e A^\text{cl}_\mu j^\mu $, and the $ S $-matrix element for the scattering of an electron from this field is:
\begin{equation*}
...
\begin{equation}
  \mat = \braket{\ve{p}_\text{f} , s_\text{f} | V(\ve{x}) | \ve{p}_\text{i} , s_\text{i}} = \bar{u}^{s_\text{f}}(p_\text{f}) \int \dd^3 x \, e^{\img (\ve{p}_\text{f} - \ve{p}_\text{i}) \cdot \ve{x}} V(\ve{x}) u^{s_\text{i}}(p_\text{i}) = \bar{u}^{s_\text{f}}(p_\text{f}) \tilde{V}(\ve{q}) u^{s_\text{i}}(p_\text{i})
\end{equation}}
...
\begin{equation*}
  u(p) \approx \sqrt{m}
  \begin{pmatrix}
    \left( 1 - \frac{\ve{p} \cdot \bs{\sigma}}{2m} \right) \xi \\
    \left( 1 + \frac{\ve{p} \cdot \bs{\sigma}}{2m} \right) \xi \\
  \end{pmatrix}
  \quad \implies \quad
...
Note that, in addition to the UV divergence encapsulated in $ \epsilon $, the electric form factor also presents an IR divergence on the light-cone $ q^2 = 0 $, which is usually regulated introducing a small photon mass $ \mu $. Nonetheless, the UV-divergent part can be extracted with a Laurent series in $ \epsilon $:
\begin{equation}
  F_1\od1(q^2) = 1 + \frac{1}{\epsilon} \frac{\alpha}{4\pi} + \smo(\epsilon^0)
  \label{eq:electric-form-factor-div}
\end{equation}

On the other hand, the integral in the magnetic form factor is trivial\footnotemark on the light-cone $ q^2 = 0 $, and the resulting expression $ F_2\od1(0) = \frac{\alpha}{2\pi} $ determines the first-order correction to the Landé factor:
\begin{equation*}
...
\footnotetext{The magnetic form factor cannot develop divergences at any order, neither UV nor IR, since otherwise the coupling would have negative mass dimension and QED would be non-renormalizable.}

\subsubsection{Vacuum polarization}

Now, consider the corrections to the photon propagator, i.e. the geometric series \eref{eq:photon-propagator-series}.

\begin{lemma}{Exact photon propagator}{}
  The exact photon propagator can be written as:
  \begin{equation}
    \gprp_{\mu \nu}(k) = - \frac{\img \eta_{\mu \nu}}{k^2} \frac{1}{1 - \Pi(k)} + \frac{\img}{k^2} \frac{k_\mu k_\nu}{k^2} \frac{\Pi(k)}{1 - \Pi(k)}
    \label{eq:exact-photon-prop}
  \end{equation}
  where the corrections are given by the \bclemma{vacuum polarization}:
  \begin{equation}
    \begin{tikzpicture}[baseline = (r.base)]
      \begin{feynman}[inline = (r.base)]
        \vertex (a) {};
        \vertex (l1) at ($ (a) + (-0.2 , 0) $) {$ \mu $};

        \vertex [right = 1.2cm of a, blob] (v) {};

        \vertex [right = 1.2cm of v] (b) {};
        \vertex (l2) at ($ (b) + (0.2 , 0) $) {$ \nu $};
    ...
    \end{tikzpicture}
    \ \equiv \img \Pi^{\mu \nu}(k) = \img \left( \eta^{\mu \nu} k^2 - k^\mu k^\nu \right) \Pi(k^2)
    \label{eq:vac-pol-ward}
  \end{equation}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    \eref{eq:photon-propagator-series} can be written as:
    \begin{equation*}
      \begin{split}
        \gprp_{\mu \nu}
        & = \gprp_{\mu \nu}^{(0)} + \gprp_{\mu \alpha}^{(0)} \img \Pi^{\alpha \beta} \gprp_{\beta \nu}^{(0)} + \gprp_{\mu \alpha}^{(0)} \img \Pi^{\alpha \beta} \gprp_{\beta \gamma}^{(0)} \img \Pi^{\gamma \delta} \gprp_{\delta \nu}^{(0)} + \dots \\
        & \gprp_{\mu \nu}^{(0)} + \gprp_{\mu \alpha_1}^{(0)} \sum_{n \in \N} \img \Pi^{\alpha_1 \beta_1} \gprp_{\beta_1 \alpha_2}^{(0)} \img \Pi^{\alpha_2 \beta_2} \gprp_{\beta_2 \alpha_3}^{(0)} \dots \img \Pi^{\alpha_n \beta_n} \gprp_{\beta_n \nu}^{(0)}
      \end{split}
    \end{equation*}
    To see that this is, in fact, a geometric series, it is necessary to simplify the Lorentz-index structure:
    \begin{equation*}
      \img \Pi^{\alpha \beta} \gprp_{\beta \gamma}^{(0)} = \img \left( \eta^{\alpha \beta} k^2 - k^\alpha k^\beta \right) \Pi(k^2) \frac{- \img \eta_{\beta \gamma}}{k^2} = \left( \delta^\alpha_\gamma - \frac{k^\alpha k_\gamma}{k^2} \right) \Pi(k^2)
    \end{equation*}
    \begin{equation*}
      \implies \quad
      \img \Pi^{\alpha_1 \beta_1} \gprp_{\beta_1 \alpha_2}^{(0)} \dots \img \Pi^{\alpha_n \beta_n} \gprp_{\beta_n \nu}^{(0)} = \left( \delta^{\alpha_1}_{\nu} - \frac{k^{\alpha_1} k_\nu}{k^2} \right) [\Pi(k^2)]^n
    \end{equation*}
    The series is then trivially summed (as a formal power series) as:
    \begin{equation*}
      \gprp_{\mu \nu}(k) = \frac{- \img \eta_{\mu \nu}}{k^2} + \frac{- \img}{k^2} \left( \eta_{\mu \nu} - \frac{k_\mu k_\nu}{k^2} \right) \frac{\Pi(k^2)}{1 - \Pi(k^2)}
    \end{equation*}
    which is the thesis.
  \end{proof}
\end{proofbox}

Note that, as long as \eref{eq:vac-pol-ward} is valid (which is just the Ward identity $ k_\mu \Pi^{\mu \nu}(k) = 0 $), then $ \Pi(k^2) $ is regular on the light-cone $ k^2 = 0 $, which means that the exact photon propagator has a simple pole at $ k^2 = 0 $ and the photon remains massless at all orders in perturbation theory. On the contrary, if $ \Pi^{\mu \nu}(k) $ contained a term like $ M^2 \eta^{\mu \nu} $ without a compensating $ k^\mu k^\nu $ term, then the photon mass would be shifted to $ M $.

\begin{proposition}{Vacuum polarization}{}
  At 1-loop order in dimensional regularization, the \bcprop{vacuum polarization} is:
  \begin{equation}
    \Pi\od1(q^2) = - \frac{2\alpha}{\pi} (4\pi)^\epsilon \Gamma(\epsilon) \int_0^1 \dd x \, x (1 - x) \Delta^{-\epsilon}
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{equation*}
      \begin{split}
        \Pi\od1_{\mu \nu}(q^2)
        & = 4\img e^2 \int_0^1 \dd x \int \frac{\dd^4 \ell}{(2\pi)^4} \big[ 2 \ell_\mu \ell_\nu + (1 - 2x) (\ell_\mu q_\nu + \ell_\nu q_\mu) - 2x (1 - x) q^\mu q^\nu + \\
    ...
    \begin{equation*}
      \begin{split}
        \Pi\od1_{\mu \nu}(q^2)
        & = 4\img e^2 \int_0^1 \dd x \bigg[ \int \frac{\dd^4 \ell}{(2\pi^4)} \frac{2 \ell_\mu \ell_\nu - \eta_{\mu \nu} \ell^2}{(\ell^2 - \Delta + \img \epsilon)^2} + \\
    ...
    \begin{proofbox}
      \begin{proof}
        ...
        \begin{equation*}
          \int \frac{\dd^d k}{(2\pi)^d} \frac{k^{2m}}{(k^2 - \Delta + \img \epsilon)^n} = \img (-1)^{n + m} \int \frac{\dd^d \ell_\text{E}}{(2\pi)^d} \frac{\ell_\text{E}^{2m}}{(\ell_\text{E}^2 + \Delta)^n}
        \end{equation*}
    ...
    \begin{equation*}
      \begin{split}
        \Pi\od1_{\mu \nu}(q^2) = 8e^2 (q_\mu q_\nu - \eta_{\mu \nu} q^2) \int_0^1 \dd x \, x (1 - x) \frac{\Gamma(2 - \frac{d}{2})}{(4\pi)^{d/2}} \Delta^{d/2 - 2} \equiv \left( \eta_{\mu \nu} q^2 - q_\mu q_\nu \right) \Pi(q^2)
      \end{split}
    \end{equation*}
    where the transverse projector has been factored out. Then:
    \begin{equation*}
      \Pi\od1(q^2) = - 8e^2 \int_0^1 \dd x \, x (1 - x) \frac{\Gamma(2 - \frac{d}{2})}{(4\pi)^{d/2}} \Delta^{d/2 - 2}
    \end{equation*}
    Inserting $ d = 4 - 2 \epsilon $ yields the thesis.
  \end{proof}
\end{proofbox}

The Laurent series in $ \epsilon $ of the vacuum polarization then reads:
\begin{equation}
  \Pi\od1(q^2) = - \frac{\alpha}{3\pi} \frac{1}{\epsilon} + \smo(\epsilon^0)
  \label{eq:vacuum-polarization-div}
\end{equation}
This clearly shows that the pole term at 1-loop order does not depend on the energy scale $ q^2 $, which is a feature of all loop orders\footnotemark.

\footnotetext{This is to be expected: in fact, the divergences encapsulated in the $ \epsilon $-poles are UV divergences, and in the UV limit two energy scales $ q^2 $ and $ \mu^2 $ become equal, since they both $ q^2 , \mu^2 \ra \infty $.}
...
Finally, take the corrections to the electron propagator. They are analogous to those in \eref{eq:photon-propagator-series}, but in this case the structure of geometric series is manifest, and they can be resummed in the exact electron propagator as:
\begin{equation}
  \dprp(\slashed{p}) = \frac{i}{\slashed{p} - m - \Sigma(\slashed{p})}
  \label{eq:exact-self-energy}
\end{equation}

\begin{proposition}{Electron self-energy}{}
  At 1-loop order in dimensional regularization, the \bcprop{electron self-energy} is:
  \begin{equation}
    \Sigma\od1(\slashed{p}) = - \frac{\alpha}{2\pi} (4\pi)^\epsilon \Gamma(\epsilon) \int_0^1 \dd x \left( (1-x) \slashed{p} - 2m \right) \Delta^{-\epsilon}
  \end{equation}
  with $ \Delta \equiv x m^2 - x (1 - x) p^2 $.
\end{proposition}

\begin{proofbox}
  \begin{proof}
    At 1-loop order, the only correction to the electron propagator is:
    \begin{equation*}
      \begin{tikzpicture}[baseline = (r.base)]
        \begin{feynman}[inline = (r.base)]
          \vertex (a) {};
          \vertex [right = 1.5 cm of a, dot] (v1) {};
          \vertex [right = 1.5 cm of v1, dot] (v2) {};
          \vertex [right = 1.5 cm of v2] (b) {};

          \vertex [below = 0.1em of a] (r) {};

          \diagram* {
            a -- [fermion , momentum = $ p $] (v1) -- [fermion] (v2) -- [fermion , momentum = $ p $] (b),
            (v1) -- [photon , half left] (v2),
          };
        \end{feynman}
      \end{tikzpicture}
      \ \equiv - \img \Sigma\od1(\slashed{p})
    \end{equation*}
    With the Feynman rules for QED:
    \begin{equation*}
      \begin{split}
        - \img \Sigma\od1(\slashed{p})
        & = (-\img e)^2 \int \frac{\dd^4k}{(2\pi)^4} \frac{\gamma^\mu \img (\slashed{k} + \slashed{p} + m) \gamma_\mu (-\img)}{((k + p)^2 - m^2 + \img \epsilon) (k^2 + \img \epsilon)} \\
        & = 2 e^2 \int \frac{\dd^4k}{(2\pi)^4} \frac{\slashed{k} + \slashed{p} - 2m}{(k^2 + 2 k \cdot p + p^2 - m^2 + \img \epsilon) (k^2 + \img \epsilon)}
      \end{split}
    \end{equation*}
    Introducing Feynman parameters (\lref{lemma:feynman-parameters}):
    \begin{equation*}
      \Sigma\od1(\slashed{p}) = 2 \img e^2 \int_0^1 \dd x \int \frac{\dd^4 \ell}{(2\pi)^4} \frac{\slashed{\ell} + (1 - x) \slashed{p} - 2m}{(\ell^2 - \Delta + \img \epsilon)^2}
    \end{equation*}
    where the integration has been shifted to $ \ell \equiv k + x q $, with $ \Delta \equiv xm^2 - x(1 - x) p^2 $. Since both the integration measure and the denominator are even under $ \ell \ra -\ell $, the first term of the numerator vanishes, leaving only:
    \begin{equation*}
      \Sigma\od1(\slashed{p}) = 2 \img e^2 \int_0^1 \dd x \left( (1 - x) \slashed{p} - 2m \right) \int \frac{\dd^4 \ell}{(2\pi)^4} \frac{1}{(\ell^2 - \Delta + \img \epsilon)^2}
    \end{equation*}
    The loop integral can now be evaluated with $ \eref{eq:int-dn} $, and the thesis is trivially found.
  \end{proof}
\end{proofbox}

Like the electric form factor, the electron self-energy too presents an IR divergence on the light-cone $ k^2 = 0 $, which here is manifest as a divergence of the integral over the Feynman parameter $ x $, and it can again be regulated introducing a small photon mass $ \mu $.
The IR divergence, however, does not affect the UV-divergent part (as it is to be expected), which can be isolated with a Laurent series in $ \epsilon $:
\begin{equation}
  \Sigma\od1(\slashed{p}) = - \frac{1}{\epsilon} \frac{\alpha}{4\pi} (\slashed{p} - 4m) + \smo(\epsilon^0)
  \label{eq:electron-self-energy-div}
\end{equation}
This completes the computation of diverging diagrams in QED: the electron-photon vertex in \eref{eq:electric-form-factor-div}, the vacuum polarization in \eref{eq:vacuum-polarization-div} and the electron self-energy in \eref{eq:electron-self-energy-div}.

\subsection{Renormalization conditions}

At this point, the UV divergences of QED have been regularized, i.e. expressed in terms of a regulator (as poles in $ \epsilon $, in this case). There remains to fix renormalization conditions in order to absorb these divergences into unobservable bare parameters.
To do so, define the renormalized (i.e. physical) field-strengths, mass and charge as:
\begin{equation}
  \Psi = \sqrt{Z_2} \Psi_\text{r}
  \qquad \qquad
  A^\mu = \sqrt{Z_3} A^\mu_\text{r}
  \qquad \qquad
  m = Z_m m_\text{r}
  \qquad \qquad
  e = \frac{Z_1}{Z_2 \sqrt{Z_3}} e_\text{r}
  \label{eq:qed-renormalization}
\end{equation}
so that the bare Lagrangian \eref{eq:qed-lagrangian} can be rewritten as in terms of finite renormalized quantities and infinite renormalization coefficients:
\begin{equation}
  \lag_\text{QED} = - \frac{1}{4} Z_3 F^{\mu \nu}_\text{r} F^\text{r}_{\mu \nu} + Z_2 \bar{\Psi}_\text{r} \left( \img \slashed{\pa} - Z_m m_\text{r} \right) \Psi_\text{r} - Z_1 e_\text{r} \bar{\Psi}_\text{r} \slashed{A}_\text{r} \Psi_\text{r}
\end{equation}
It is convenient to work with renormalization counterterms, which are defined from the renormalization coefficients as:
\begin{equation}
  Z_1 = 1 + \delta_1
  \qquad \qquad
  Z_2 = 1 + \delta_2
  ...
  Z_3 = 1 + \delta_3
  \qquad \qquad
  Z_2 Z_m = 1 + \frac{\delta_m}{m_\text{r}}
  \label{eq:qed-counterterms}
\end{equation}
Writing the Lagrangian counterterms explicitely:
\begin{equation}
  \begin{split}
    \lag_\text{QED}
    & = - \frac{1}{4} F^{\mu \nu}_\text{r} F^\text{r}_{\mu \nu} + \bar{\Psi}_\text{r} \left( \img \slashed{\pa} - m_\text{r} \right) \Psi_\text{r} - e_\text{r} \bar{\Psi}_\text{r} \slashed{A}_\text{r} \Psi_\text{r} + \\
    & \qquad + \frac{1}{2} \delta_3 A^\mu_\text{r} \left( \eta_{\mu \nu} \Box - \pa_\mu \pa_\nu \right) A^\nu_\text{r} + \bar{\Psi}_\text{r} \left( \img \delta_2 \slashed{\pa} - \delta_m \right) \Psi_\text{r} - \delta_1 e_\text{r} \bar{\Psi}_\text{r} \slashed{A}_\text{r} \Psi_\text{r}
  \end{split}
\end{equation}
The Feynman rules for this Lagrangian are trivially found. Assuming always to work in Feynman gauge:
\begin{equation*}
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
  ...
  \begin{tikzpicture}[baseline = (r.base)]
    ...
    \end{feynman}
  \end{tikzpicture}
  \ = \frac{\img}{\slashed{p} - m_r + \img \epsilon}
  \qquad
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
      ...
      \diagram* {
        (a) -- [photon , momentum = $ k $] (v),
        (c) -- [fermion] (v) -- [fermion] (b),
      };
    \end{feynman}
  \end{tikzpicture}
  \ = - \img e_\text{r} \gamma^\mu
\end{equation*}
\begin{equation*}
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
      \vertex (l1) at ($ (a) + (-0.2 , 0) $) {$ \mu $};
      \vertex [right = 1cm of a, crossed dot] (v) {};
      \vertex [right = 1cm of v] (b) {};
      \vertex (l2) at ($ (b) + (0.2 , 0) $) {$ \nu $};
  ...
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      ...
      \diagram* {
        (a) -- [fermion, momentum = $ p $] (v),
        (v) -- [fermion] (b),
      };
    \end{feynman}
  \end{tikzpicture}
  \ = \img \left( \delta_2 \slashed{p} - \delta_m \right)
\end{equation*}
\begin{equation*}
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
      \vertex (l1) at ($ (a) + (-0.2 , 0) $) {$ \mu $};
      \vertex [right = 1.5cm of a, crossed dot] (v) {};
      \vertex (b) at ($ (v) + (1 , 1) $) {};
      \vertex (c) at ($ (v) + (1 , -1) $) {};
    ...
    \end{feynman}
  \end{tikzpicture}
  \ = - \img e_\text{r} \gamma^\mu \delta_1
\end{equation*}
Now, this counterterms must be chosen in order to cancel the divergences of the three classes of diverging diagrams, i.e. the photon and electron propagators and the interaction vertex.

\subsubsection{Electron propagator}

The geometric series to compute the exact electron propagator is still valid, however now the blob correction contains 1PI diagrams with both ``normal" vertices and ``counter"-vertices, i.e. it has the form:
\begin{equation*}
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
      \vertex [right = 1.5cm of a, crossed dot] (v) {};
      \vertex [right = 1.5cm of v] (b) {};
  ...
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
      \vertex [right = 1.5cm of a, blob] (v) {};
      \vertex [right = 1.5cm of v] (b) {};
  ...
  \end{tikzpicture}
\end{equation*}
Then, it is useful to generalize the electron self-energy introducing:
\begin{equation}
  -\img \Sigma_\text{r}(\slashed{p}) \equiv \img \left( \delta_2 \slashed{p} - \delta_m \right) - \img \Sigma(\slashed{p})
  \label{eq:sigma-r}
\end{equation}
so that the exact electron propagator is written as:
\begin{equation}
  \dprp(\slashed{p}) = \frac{\img}{\slashed{p} - m_\text{r} - \Sigma_\text{r}(\slashed{p})}
\end{equation}
Now, the relevant renormalization conditions imposed on the electron propagator are on its pole and the related residue:
\begin{equation}
  \dprp^{-1}(m_\text{p}) = 0
  \qquad \qquad
  \img \frac{\pa}{\pa \slashed{p}} \dprp^{-1}(m_\text{p}) = 1
\end{equation}
where the pole mass $ m_\text{p} $ is determined by the subtraction scheme adopted, i.e. by how the finite parts of the counteterms are defined.

\paragraph{On-shell scheme}

In the on-shell scheme, the pole mass is taken to be the physical mass of the electron, i.e. $ m_\text{p} = m_\text{r} $. Therefore, the above conditions translate to:
\begin{equation}
  \Sigma_\text{r}(m_\text{r}) = 0
  \qquad \qquad
  \Sigma'_\text{r}(m_\text{r}) = 0
  \label{eq:qed-mass-renorm}
\end{equation}
which allow to define both $ \delta_m $ and $ \delta_2 $. At 1-loop order, recalling \eref{eq:electron-self-energy-div}, the divergent part of the counterterms is found solving the above system:
\begin{equation}
  \delta_2\od1 = - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{4\pi} + \smo(\alpha_\text{r} \epsilon^0)
  \qquad \qquad
  \delta_m\od1 = - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{\pi} + \smo(\alpha_\text{r} \epsilon^0)
  \label{eq:qed-dz2}
\end{equation}
From \eref{eq:qed-counterterms}, noting that $ (1 + \delta_2)\od1 = 1 - \delta_2\od1 $, the renormalization coefficients are found to be:
\begin{equation}
  Z_2\od1 = 1 - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{4\pi} + \smo(\alpha_\text{r} \epsilon^0)
  \qquad \qquad
  Z_m\od1 = 1 - \frac{1}{\epsilon} \frac{3\alpha_\text{r}}{4\pi} + \smo(\alpha_\text{r} \epsilon^0)
  \label{eq:qed-z2}
\end{equation}

\paragraph{Minimal subtraction scheme}

In the MS scheme, the counterterms only contain a divergent part, hence, by \eref{eq:sigma-r}:
\begin{equation}
  \delta_2\od1 = - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{4\pi}
  \qquad \qquad
  \delta_m\od1 = - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{\pi}
\end{equation}
This explicitly shows that the divergent part of the counterterms is scheme-independent. The renormalization coefficients in the MS scheme thus differ from those in the on-shell scheme only by a finite quantity.
Recall that it is also possible to define a modified minimal subtraction scheme ($ \msb $), which additionally includes in the counterterms the universal factor $ \log (4\pi e^{-\gamma_\text{E}}) $ present in dimensionally-regularized calculations.

Note that in this case the pole mass $ m_\text{p} = m_\text{MS} $ is different from the physical mass $ m_r $ of the electron, but their difference is finite and given by the $ \smo(\alpha_\text{r} \epsilon^0) $ terms in \eref{eq:qed-dz2}.

\subsubsection{Photon propagator}

The exact photon propagator too is modified by the presence of counterterms. In particular, the blob correction in this case has the form:
\begin{equation*}
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
  ...
  \end{tikzpicture}
\end{equation*}
Defining a generalized vacuum polarization as:
\begin{equation}
  \img \Pi_\text{r}^{\mu \nu}(k) = - \img \left( \eta^{\mu \nu} k^2 + k^\mu k^\nu \right) \delta_3 + \img \Pi^{\mu \nu}(k)
  \qquad \qquad
  \Pi_\text{r}(k^2) = \Pi(k^2) - \delta_3
\end{equation}
the exact photon propagator can be written as:
\begin{equation}
  \gprp_{\mu \nu}(k) = \frac{- \img}{k^2} \eta_{\mu \nu} \frac{1}{1 - \Pi_\text{r}(k^2)} + \frac{\img}{k^2} \frac{k_\mu k_\nu}{k^2} \frac{\Pi_\text{r}(k^2)}{1 - \Pi_\text{r}(k^2)}
\end{equation}
In this case, the propagator already has a pole at $ k^2 = 0 $, hence it is sufficient to fix its residue to $ -\img \eta_{\mu \nu} $, i.e.:
\begin{equation}
  \Pi_\text{r}(0) = 0
\end{equation}
Thus, $ \delta_3 = \Pi(0) $, which at 1-loop can be computed from \eref{eq:vacuum-polarization}. The divergent part is:
\begin{equation}
  \delta_3\od1 = - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{3\pi} + \smo(\alpha_\text{r} \epsilon^0)
  \qquad \qquad
  Z_3\od1 = 1 - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{3\pi} + \smo(\alpha_\text{r} \epsilon^0)
\end{equation}

\subsubsection{Interaction vertex}

Given the definition of the renormalization coefficient for $ e_\text{r} $ in \eref{eq:qed-renormalization}, the corrections to the electron and the photon propagator are factored out of the vertex function, hence the corrections to the interaction vertex are only given by:
\begin{equation*}
  \begin{tikzpicture}[baseline = (r.base)]
    \begin{feynman}[inline = (r.base)]
      \vertex (a) {};
  ...
  \end{tikzpicture}
\end{equation*}
The resulting generalized vertex function is:
\begin{equation}
  \Gamma_\text{r}^\mu(k) = \Gamma^\mu(k) + \gamma^\mu \delta_1
\end{equation}
It is conventional to define $ e_\text{r} $ as the electron charge measured at great distance (i.e. at $ k \ra 0 $), hence the renormalization condition is:
\begin{equation}
  \Gamma_\text{r}^\mu(0) = \gamma^\mu
  \label{eq:qed-vertex-renorm}
\end{equation}
Recalling \eref{eq:vertex-function}, the counterterm is correctly determined by the electric form factor only (since the magnetic form factor is not divergent). At 1-loop order, from \eref{eq:electric-form-factor-div}:
\begin{equation}
  \delta_1\od1 = - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{4\pi} + \smo(\alpha_\text{r} \epsilon^0)
  \qquad \qquad
  Z_1 = 1 - \frac{1}{\epsilon} \frac{\alpha_\text{r}}{4\pi} + \smo(\alpha_\text{r} \epsilon^0)
  \label{eq:qed-z1}
\end{equation}

\subsubsection{Gauge symmetry and renormalization}

Comparing \eref{eq:qed-z2} and \eref{eq:qed-z1}, it may seem a coincidence that the divergent parts of $ Z_1 $ and $ Z_2 $ are equal. However, this is a consequence of the gauge symmetry which defines the QED, i.e. the $ \Un{1} $ vector symmetry.

\begin{lemma}{Ward--Takahashi identity}{}
  Defining $ \Gamma^\mu(p + k , p) \equiv \Gamma^\mu(k) $ in order to denote the incoming momentum in the fermionic current (not necessarily on-shell), the vertex function can be written as:
  \begin{equation}
    - \img k_\mu \Gamma^\mu(p + k , p) = \dprp^{-1}(\slashed{p} + \slashed{k}) - \dprp^{-1}(\slashed{p})
  \end{equation}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    See §7.5 of \cite{peskin}.
  \end{proof}
\end{proofbox}

Note that the Ward--Takahashi identity is a direct consequence of the conservation of the Noether current $ j^\mu = \bar{\Psi} \gamma^\mu \Psi $ associated to the gauge symmetry of QED, i.e. $ \pa_\mu j^\mu = 0 $.
...
\begin{proofbox}
  \begin{proof}
    Apply the Ward--Takahashi identity to the bare QED Lagrangian:
    \begin{equation*}
      k_\mu \Gamma^\mu(p + k , p) = \slashed{k} - \Sigma(\slashed{p} + \slashed{k}) + \Sigma(\slashed{p})
    \end{equation*}
    Taking the limit of all particles going on-shell, i.e. $ k \ra 0 $ and $ p \ra m_\text{r} $, and recalling that for on-shell electrons the vertex function is given by \eref{eq:vertex-function}, this equation becomes:
    \begin{equation*}
      F_1(0) = 1 - \Sigma'(m_\text{r})
    \end{equation*}
    However, from the renormalization conditions\footnotemark \ceref{eq:qed-mass-renorm}{eq:qed-vertex-renorm} it follows that, at all loop orders:
    \begin{equation*}
      F_1(0) = 1 - \delta_1
      \qquad \qquad
      \Sigma'(m_\text{r}) = \delta_2
    \end{equation*}
    Hence, $ \delta_1 = \delta_2 $ at all loop orders, which is the thesis.
  \end{proof}
\end{proofbox}

\footnotetext{Indeed, the definitions of the renormalization coefficients given in \eref{eq:qed-renormalization} are equivalent to defining:
\begin{equation}
  \Gamma^\mu(0) = Z_1^{-1} \gamma^\mu
  \qquad \qquad
  \Sigma(\slashed{p}) \sim \left( 1 - Z_2^{-1} \right) \slashed{p}
\end{equation}}

From this equivalence, it follows that the charge is truly renormalized as $ e = Z_3^{-1/2} e_\text{r} $. This has an important consequence when considering QED with multiple fermionic species (e.g. muons): the renormalized electric charge has the same universal value for all the species, since it only depends on the renormalization coefficient for the photon field.

\subsection{Renormalization group}

Consider the Coulomb scattering $ e^- e^- \ra e^- e^- $, which at tree-level has the matrix element (in the $ t $-channel):
\begin{equation*}
  \begin{tikzpicture}[baseline=(r.base)]
    \begin{feynman}[inline=(r.base)]
      \vertex (a1) {};
  ...
  \end{tikzpicture}
  \ \equiv
  \img \mat_0 (q^2) \sim \frac{e^2}{q^2}
\end{equation*}
where $ e $ is the bare coupling constant. Accounting for loop corrections to the photon propagator, i.e. inserting \eref{eq:exact-photon-prop}, results into the observed matrix element:
\begin{equation*}
  \img \mat(q^2) \sim \frac{e^2}{q^2} \frac{1}{1 - \Pi(q^2)}
\end{equation*}
This allows to define an observed ``effective" electron charge $ e_\text{eff}^2(q^2) $, which depends on the energy scale $ q^2 $ at which the experiment is performed:
\begin{equation}
  e_\text{eff}^2(q^2) \defeq \frac{e^2}{1 - \Pi(q^2)}
  \label{eq:eff-charge-def}
\end{equation}
Recalling that $ e_\text{r}^2 = Z_3 e^2 = e^2 / (1 - \Pi(0)) $, it is clear that the renormalized charge is defined as $ e_\text{r} \equiv e_\text{eff}(0) $, i.e. as the electron charge observed at great distance, as already noted when imposing the renormalization condition for the electron-photon vertex: this is the effect of vacuum polarization, where the physical charge of the electron $ e_\text{r} = 1.602176634 \cdot 10^{-19} \,\text{C} $ is affected by a progressively stronger screening effect when probed at smaller distances, i.e. at larger momenta. Indeed, at 1-loop order:
\begin{equation*}
  \alpha_\text{eff}\od1(q^2) = \alpha_\text{r} \frac{1 - \Pi\od1(0)}{1 - \Pi\od1(q^2)} = \frac{\alpha_\text{r}}{1 - \Pi\od1(q^2) + \Pi\od2(0)}
\end{equation*}
with $ \alpha_\text{r} = e_\text{r}^2 / 4\pi \simeq 1/137.036 $ and:
\begin{equation*}
  \Pi\od1(q^2) - \Pi\od1(0) = \frac{2\alpha\od1}{\pi} \int_0^1 \dd x \, x (1 - x) \log \left[ 1 - x (1 - x) \frac{q^2}{m^2} \right]
\end{equation*}
In the relativistic limit\footnotemark $ -q^2 \gg m^2 $ this expression becomes, using $ \alpha\od1 = \alpha_\text{r} $:
\begin{equation*}
  \Pi\od1(q^2) - \Pi\od1(0) = \frac{\alpha}{3\pi} \left[ \log \frac{- q^2}{m^2} - \frac{5}{3} + \smo \left( \frac{m^2}{q^2} \right) \right]
\end{equation*}
which clearly shows that $ \alpha_\text{eff}(q^2) $ becomes larger as $ \abs{q^2} $ increases, as predicted by the charge screening due to vacuum polarization.

\footnotetext{The two channels that contribute to the Coulomb scattering are the $ t $-channel and the $ u $-channel. In the CM frame, since $ p_1 = (E_\ve{p} , \ve{p}) $, $ p_2 = (E_\ve{p} , - \ve{p}) $, $ k_1 = (E_\ve{p} , \ve{p}') $ and $ k_2 = (E_\ve{p} , - \ve{p}') $, with $ \norm{\ve{p}'} = \norm{\ve{p}} $, defining the scattering angle $ \theta : \cos \theta = \hat{\ve{p}}' \cdot \hat{\ve{p}} $, the Mandelstam variables are:
\begin{equation*}
  t = - 2 \ve{p}^2 (1 - \cos \theta) \leq 0
  \qquad \qquad
  u = - 2 \ve{p}^2 (1 + \cos \theta) \leq 0
\end{equation*}
where $ t = 0 $ only for forward scattering ($ \theta = 0 $) and $ u = 0 $ only for backward scattering ($ \theta = \pi $). In the relativistic regime $ \ve{p}^2 \gg m^2 $, hence $ -t , -u \gg m^2 $.}

Moreover, note that $ \Pi(k^2) \propto e^2 $, hence, defining $ \pi(k^2) \equiv \Pi(k^2) / e^2 $, it is possible to invert this relation to find the bare electric charge in terms of the observed one:
\begin{equation}
  e^2 = \frac{e_\text{eff}^2(q^2)}{1 - e_\text{eff}^2(q^2) \pi(q^2)}
\end{equation}
Now, consider two measurements of $ e_\text{eff} $ at two different scales $ q^2 $ and $ \mu^2 $. From \eref{eq:eff-charge-def}:
\begin{equation*}
  \frac{1}{e_\text{eff}^2(q^2)} - \frac{1}{e_\text{eff}^2(\mu^2)} = \pi(\mu^2) - \pi(q^2)
\end{equation*}
Since the divergent part of the vacuum polarization is independent of the energy scale (recall the expansion \eref{eq:vacuum-polarization-div}), this expression is correctly finite, as it is to be expected from physically observable quantities.

\begin{lemma}{}{}
  In the relativistic regime $ - q^2 , - \mu^2 \gg m^2 $:
  \begin{equation*}
    \pi\od1(\mu^2) - \pi\od1(q^2) \simeq \frac{1}{12\pi^2} \log \frac{\mu^2}{q^2}
  \end{equation*}
\end{lemma}

\begin{proofbox}
  \begin{proof}
    From \eref{eq:vacuum-polarization}:
    \begin{equation*}
      \begin{split}
        \pi\od1(\mu^2) - \pi\od1(q^2)
        & = - \frac{1}{2\pi^2} \int_0^1 \dd x \, x (1 - x) \left[ - \log \Delta(\mu^2) + \log \Delta(q^2) + \smo(\epsilon) \right] \\
        & = \frac{1}{2\pi^2} \int_0^1 \dd x \, x (1 - x) \log \frac{1 - x (1 - x) \frac{\mu^2}{m^2}}{1 - x (1 - x) \frac{q^2}{m^2}} \simeq \frac{1}{2\pi^2} \int_0^1 \dd x \, x (1 - x) \log \frac{\mu^2}{q^2}
      \end{split}
    \end{equation*}
    Performing the integration yields the thesis.
  \end{proof}
\end{proofbox}

With this relation, it is possible to relate directly the measurements of the electric charge at different energy scales:
\begin{equation}
  \alpha_\text{eff}\od1(q^2) = \frac{\alpha_\text{eff}\od1(\mu^2)}{1 - \frac{\alpha_\text{eff}\od1(\mu^2)}{3\pi} \log \frac{q^2}{\mu^2}}
\end{equation}
This dependence of the physical coupling on the energy scale is known as the \bctxt{running of the coupling}, and it is effectively a prediction of the renormalization of the theory (which has been verified experimentally).

The above equation completely determines the dependence of the coupling on the energy scale (at 1-loop order). Switching the notation to $ q \mapsto \mu $ and $ \mu \mapsto \mu_0 $ and differentiating with respect to $ \log \mu^2 $:
\begin{equation*}
  \frac{\dd}{\dd \log \mu^2} \frac{1}{\alpha_\text{eff}\od1(\mu^2)} = - \frac{1}{3\pi}
  \quad \implies \quad
  \frac{\dd \alpha_\text{eff}\od1}{\dd \log \mu^2} = \alpha_\text{eff}\od1 \frac{4}{3} \frac{\alpha_\text{eff}\od1}{4\pi}
\end{equation*}
Generalizing to higher loop orders, the \bctxt{renormalization group} equation is found:
\begin{equation}
  \frac{\dd \alpha_\text{eff}(\mu^2)}{\dd \log \mu^2} = - \beta(\alpha_\text{eff}(\mu^2))
\end{equation}
where the \bctxt{Callan--Symanzik $ \beta $-function} is defined as:
\begin{equation}
  \beta(\alpha) \equiv \alpha \sum_{n \in N_0} \beta_n \left( \frac{\alpha}{4\pi} \right)^{n + 1}
\end{equation}
Clearly, for QED $ \beta_0 = - \frac{4}{3} $, which means that the coupling gets stronger as the energy increases. On the contrary, for theories with $ \beta_0 > 0 $ (like QCD), the coupling is weaker in the UV and stronger in the IR, a phenomenon known as \emph{asymptotic freedom}.

Furthermore, viewing masses as additional couplings, it is possible to define a massive $ \beta $-function, so that the scale-dependence of the observed mass can be written as:
\begin{equation}
  \frac{\dd m_\text{eff}(\mu^2)}{\dd \log \mu^2} = - \beta_m(\alpha_\text{eff}(\mu^2) , m_\text{eff}(\mu^2))
\end{equation}
Now, it is possible to study the renormalization of $ n $-point Green functions. In particular, consider a connected 1PI Green function with $ k $ photon lines and $ n $ electron lines:
\begin{equation*}
  \tilde{G}^{(k,n)}(\{p_i\} , \alpha , m , \Lambda) \equiv \braket{0 | \tempord\{A(p_1) \dots A(p_k) \Psi(p_{k + 1}) \dots \Psi(p_{k + n}) \} | 0}
\end{equation*}
where Lorentz indices and Dirac adjoints have been suppressed for brevity. Then, renormalizing the field-strengths:
\begin{equation*}
  \tilde{G}^{(k,n)}(\{p_i\} , \alpha , m , \Lambda) = Z_3^{k/2} Z_2^{n/2} \tilde{G}_\text{r}^{(k,n)}(\{p_i\} , \alpha_\text{r} , m_\text{r} , \mu)
\end{equation*}
where the regulator $ \Lambda $ is substituted by the renormalization point $ \mu $. Note that $ \tilde{G}_\text{r} $, $ \alpha_\text{r} $ and $ m_\text{r} $ are finite, while $ \tilde{G} $, $ \alpha $, $ m $, $ Z_2 $ and $ Z_3 $ are divergent with respect to the regulator (i.e. for $ \Lambda \ra \infty $ with a UV cut-off or $ \epsilon \ra 0 $ in dimensional regularization): in particular, $ \tilde{G} $, $ \alpha $ and $ m $ only depend on the regulator $ \Lambda $ and not on the renormalization point $ \mu $, hence taking the derivative with respect to $ \log \mu^2 $ yields:
\begin{equation*}
  0 = \frac{k}{2} Z_3^{k/2} \frac{1}{Z_3} \frac{\pa Z_3}{\pa \log \mu^2} Z_2^{n/2} \tilde{G}_\text{r}^{(k,n)} + Z_3^{k/2} \frac{n}{2} \frac{1}{Z_2} \frac{\pa Z_2}{\pa \log \mu^2} \tilde{G}_\text{r}^{(k,n)} + Z_3^{k/2} Z_2^{n/2} \frac{\dd \tilde{G}_\text{r}^{(k,n)}}{\dd \log \mu^2}
\end{equation*}
Rewriting the first two terms and expanding the total derivative, this equation becomes:
\begin{equation}
  \left[ \frac{\pa}{\pa \log \mu^2} - \beta(\alpha_\text{r}) \frac{\pa}{\pa \alpha_\text{r}} - \beta_m(\alpha_\text{r} , m_\text{r}) \frac{\pa}{\pa m_\text{r}} + k \gamma_\gamma(\alpha_\text{r}) + n \gamma_e(\alpha_\text{r} , m_\text{r}) \right] \tilde{G}_\text{r}^{(k,n)}(\{p_i\} , \alpha_\text{r} , m_\text{r} , \mu)
\end{equation}
which is known as the \bctxt{Callan--Symanzik equation}. The two factors $ \gamma_\gamma $ and $ \gamma_e $ are know respectively as the photon and the electron \emph{anomalous dimensions}; in general, the anomalous dimension of a field $ \phi_i $ is defined as:
\begin{equation}
  \gamma_i(\{\lambda_j\}) \defeq \frac{\pa \log \sqrt{Z_i}}{\pa \log \mu^2}
\end{equation}
where $ Z_i $ is the renormalization coefficient of the field strength $ \phi_i = Z_i \phi^\text{r}_i $. Note that the anomalous dimension is independent of the regulator, contrary to $ Z_i $, as it depends only on the various couplings of the theory which affect $ \phi_i $.

\begin{example}{Evolution of a massless renormalizable theory}{}
  Consider a quantum field theory with a single massless quantum field $ \phi $ and a single interaction vertex with dimensionless coupling $ \alpha $, and suppose to conduct two experiments to evaluate the $ n $-point Green function at two different energy scales, i.e. with momenta $ \{p_i\}_{i = 1 , \dots , n} $ and $ \{e^s p_i\}_{i = 1 , \dots , n} $, with $ s \in \R $ a rescaling parameter. Then, defining the dimension $ [\tilde{G}_\text{r}^{(n)}(\{p_i\} , \alpha_\text{r} , \mu)] \equiv d_n $, the $ n $-point Green function classically rescales as:
  \begin{equation*}
    \tilde{G}_\text{r}^{(n)}(\{e^s p_i\} , \alpha_\text{r} , \mu) = e^{d_n s} \tilde{G}_\text{r}^{(n)}(\{p_i\} , \alpha_\text{r} , e^{-s} \mu)
  \end{equation*}
  There remains to evaluate the scaling of the Green function due to the renormalization point. To do so, consider the Callan--Symanzik equation for this massless theory:
  \begin{equation*}
    \left[ \frac{\pa}{\pa \log \mu^2} - \beta(\alpha_\text{r}) \frac{\pa}{\pa \alpha_\text{r}} + n \gamma(\alpha_\text{r}) \right] \tilde{G}_\text{r}^{(n)}(\{p_i\} , \alpha_\text{r} , \mu) = 0
  \end{equation*}
  Now, parametrize\footnote{This is the \emph{method of characteristics} to solve quasi-linear first-order PDEs.} this equation in the $ (\mu , \alpha_\text{r}) $-plane so that:
  \begin{equation*}
    \frac{\dd \log \mu^2(s)}{\dd s} = 1
    \qquad \qquad
    \frac{\dd \alpha_\text{r}(s)}{\dd s} = - \beta(\alpha_\text{r}(s))
  \end{equation*}
  with initial conditions $ \mu(0) = \bar{\mu} , \alpha_\text{r}(0) = \bar{\alpha}_\text{r} $. The Callan--Symanzik equation then becomes:
  \begin{equation*}
    \frac{\dd}{\dd s} \tilde{G}_\text{r}^{(n)}(s) = - n \gamma(\alpha_\text{r}(s)) \tilde{G}_\text{r}^{(n)}(s)
  \end{equation*}
  with $ \tilde{G}_\text{r}^{(n)}(s) \equiv \tilde{G}_\text{r}^{(n)}(\{p_i\} , \alpha_\text{r}(s) , \mu(s)) $. This ODE is trivially solved as:
  \begin{equation*}
    \tilde{G}_\text{r}^{(n)}(s) = \tilde{G}_\text{r}^{(n)}(0) \exp \left[ -n \int_0^s \dd t \, \gamma(\alpha_\text{r}(t)) \right]
  \end{equation*}
  Since the characteristic equation for $ \mu(s) $ has solution $ \mu(s) = e^{s/2} \bar{\mu} $, the total scaling of the $ n $-point Green function is:
  \begin{equation}
    \tilde{G}_\text{r}^{(n)}(\{e^s p_i\} , \alpha_\text{r} , \mu) = \tilde{G}_\text{r}^{(n)}(\{p_i\} , \alpha_\text{r} , \mu) \exp \left[ d_n s - n \int_0^{-2s} \dd t \, \gamma(\alpha_\text{r}(t)) \right]
  \end{equation}
  This clarifies the nomenclature ``anomalous dimension".
\end{example}

Intuitively, the Callan--Symanzik equation expresses the fact that physical observables cannot depend on the renormalization point $ \mu $, which is an arbitrary energy scale introduced in the calculations to eliminate the explicit dependence on the regulator. Indeed, it constraints the dependence of renormalized quantities on $ \mu $ in such a way that when the physical observables (i.e. $ S $-matrix elements) are expressed in terms of them, if the renormalization point varies, the renormalized quantities vary too but the physical observables do not.
