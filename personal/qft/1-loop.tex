...
\begin{equation*}
  \begin{tikzpicture}[baseline=(r.base)]
    \begin{feynman}[inline=(r.base)]
      ...
      \vertex[right=4cm of a1, dot] (a2) {};
      \vertex (l2) at ($(a2)+(+0.4,0)$) {\(e^-\)};

      \vertex[below=4em of a1, dot] (b1) {};
      \vertex (l3) at ($(b1)+(-0.4,0)$) {\(e^-\)};

      \vertex[below=4em of a2, dot] (b2) {};
      \vertex (l4) at ($(b2)+(+0.4,0)$) {\(e^-\)};

      \vertex[below=2em of a1] (c1) {};

      \vertex[right=1cm of c1, dot] (c2) {};

      \vertex[right=2cm of c2, dot] (c3) {};
      ...
      \diagram* {
        ...
        (a2) -- [fermion, momentum' = $ p_2 $] (c3),
        (c3) -- [fermion, momentum' = \(k_2\)] (b2),
...
\begin{equation*}
  \mat = \mat_0 + \mat_0 \Pi \mat_0 + \mat_0 \Pi \mat_0 \Pi \mat_0 = \mat_0 \sum_{n = 0}^\infty (\Pi \mat_0)^n = \mat_0 (1 - \Pi \mat_0)^{-1} = (\mat_0 - \Pi)^{-1}
\end{equation*}
...
\begin{proofbox}
  \begin{proof}
    ...
    \begin{lemma}[before upper = {\tcbtitle}, borderline = {1pt}{0pt}{blue!35!black}]{}{generalized-d-int}
      \begin{equation}
        \int \frac{\dd^d k}{(2\pi)^d} \frac{k^{2m}}{(k^2 - \Delta + \img \epsilon)^n} = \frac{\img^m (-1)^n}{(4\pi)^{d/2}} \frac{\Gamma(n - \frac{d}{2} - m) \Gamma(\frac{d}{2} + m)}{\Gamma(\frac{d}{2}) \Gamma(n)} \Delta^{-n + \frac{d}{2} + m}
        \label{eq:int-dn-gen}
      \end{equation}
    \end{lemma}
    ...
    \begin{proofbox}
      \begin{proof}
        ...
        \begin{equation*}
          \int \frac{\dd^d k}{(2\pi)^d} \frac{k^{2m}}{(k^2 - \Delta + \img \epsilon)^n} = \img^m (-1)^n \int \frac{\dd^d \ell_\text{E}}{(2\pi)^d} \frac{\ell_\text{E}^{2m}}{(\ell_\text{E}^2 + \Delta - \img \epsilon)^n}
        \end{equation*}
        Parametrizing the Euclidean domain of integration $ \R^d $ with spherical coordinates and using \eref{eq:solid-angle}:
        \begin{equation*}
          \int \frac{\dd^d k}{(2\pi)^d} \frac{k^{2m}}{(k^2 - \Delta + \img \epsilon)^n} = \frac{2 \img^m (-1)^n}{(4\pi)^{d/2} \Gamma(\frac{d}{2})} \int_{\R^+} \dd \ell \frac{\ell^{2m + d - 1}}{(\ell^2 + \Delta)^n}
        \end{equation*}
