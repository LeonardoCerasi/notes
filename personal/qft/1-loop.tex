...
\begin{observation}{Wick rotations}{wick-rotations}
  ...
  function $ f : \C \ra \C $ over the path $ \ell_\infty \equiv \R $ (identifying the curve with its support). In particular, consider the case of massive particles only, so that the poles of $ f(k^2) $ have non-vanishing real parts: in particular, due to the general form $ k^2 - m^2 + \img \epsilon $ of the denominators of propagators, the poles come in couples of the form $ k^0_\pm = \pm \sqrt{m^2 + \ve{k}^2} \mp \img \epsilon $. For example, the case of a single propagator is drawn as:
  \begin{equation*}
    \begin{tikzpicture}
      \def\bigradius{3}
  ...
  \end{equation*}
  where the Euclidean momentum is now defined on the Euclidean manifold $ \R^d $ with Euclidean metric $ \delta_{\mu \nu} = (+1 , +1 , \dots , +1) $. The Wick rotation thus turns a Minkowskian integral into a Euclidean integral, which can therefor be treated with the standard techniques of Analysis.
\end{observation}
...
\begin{itemize}
  \item cut-off regulator: a parameter $ \Lambda \in \R^+ $ is introduced to cut off the UV regime, i.e. imposing $ k_\text{E}^2 \leq \Lambda^2 $, so that the domain of UV divergences is removed entirely from the itegration. However, this regulator effectively cuts off the infinite number of multi-particle states that can be produced at high energies (see \figref{fig:mass-2}), thus violating unitarity (and gauge symmetry, too);
...
\begin{observation}{Dimensional regularization}{}
  In general, the dimensional regularization scheme consists in the analytic continuation of loop momenta to $ d = 4 - 2 \epsilon $, with $ \epsilon \in \C : \Re \epsilon < 0 $. This procedure turnss loop integrals into meromorphic\footnotemark functions of $ \epsilon \in \C $, allowing for the isolation of divergences as poles in $ \epsilon $.
...
\end{equation*}
This allows to define an observed electron charge $ e_\text{obs}^2(q^2) $, which depends on the energy scale $ q^2 $ at which the experiment is performed:
\begin{equation}
  e_\text{obs}^2(q^2) \defeq \frac{e^2}{1 - \Pi(q^2)}
  \label{eq:obs-charge-def}
\end{equation}
Recalling that $ e_\text{r}^2 = Z_3 e^2 = e^2 / (1 - \Pi(0)) $, it is clear that the renormalized charge is defined as $ e_\text{r} \equiv e_\text{obs}(0) $, i.e. as the electron charge observed at great distance, as already noted when imposing the renormalization condition for the electron-photon vertex.
...
\begin{equation}
  e^2 = \frac{e_\text{obs}^2(q^2)}{1 - e_\text{obs}^2(q^2) \pi(q^2)}
\end{equation}
Now, consider two measurements of $ e_\text{obs} $ at two different scales $ q^2 $ and $ \mu^2 $. From \eref{eq:obs-charge-def}:
\begin{equation*}
  \frac{1}{e_\text{obs}^2(q^2)} - \frac{1}{e_\text{obs}^2(\mu^2)} = \pi(\mu^2) - \pi(q^2)
\end{equation*}
Since the divergent part of the vacuum polarization is independent of the energy scale (\eref{eq:vacuum-polarization-div}), this expression is correctly finite, as it is to expect from physically observable quantities.

\begin{lemma}{}{}
  In the relativistic regime\footnotemark $ - q^2 , - \mu^2 \gg m^2 $:
  \begin{equation*}
    \pi\od1(\mu^2) - \pi\od1(q^2) \simeq \frac{1}{12\pi^2} \log \frac{q^2}{\mu^2}
  \end{equation*}
\end{lemma}
  ...
  \begin{proof}
    ...
    \begin{equation*}
      \begin{split}
        ...
        & = \frac{1}{2\pi^2} \int_0^1 \dd x \, x (1 - x) \log \frac{1 - x (1 - x) \frac{q^2}{m^2}}{1 - x (1 - x) \frac{\mu^2}{m^2}} \simeq \frac{1}{2\pi^2} \int_0^1 \dd x \, x (1 - x) \log \frac{q^2}{\mu^2}
      \end{split}
    \end{equation*}
...
\begin{equation}
  \alpha_\text{obs}(q^2) = \frac{\alpha_\text{obs}(\mu^2)}{1 - \frac{\alpha_\text{obs}(\mu^2)}{3\pi} \log \frac{q^2}{\mu^2}}
\end{equation}
...
\begin{equation*}
  \frac{\dd}{\dd \log \mu^2} \alpha_\text{obs}^{-1}(\mu^2) = - \frac{4}{3} \frac{1}{4\pi}
  \quad \implies \quad
  \frac{\dd \alpha_\text{obs}}{\dd \log \mu^2} = \frac{4}{3} \frac{\alpha_\text{obs}^2}{4\pi}
\end{equation*}
Generalizing to higher loop orders, the \bctxt{renormalization group} equation is found:
\begin{equation}
  \frac{\dd \alpha_\text{obs}(\mu^2)}{\dd \log \mu^2} = - \beta(\alpha_\text{obs}(\mu^2))
\end{equation}
...
\begin{equation}
  \frac{\dd m_\text{obs}(\mu^2)}{\dd \log \mu^2} = - \beta_m(\alpha_\text{obs}(\mu^2) , m_\text{obs}(\mu^2))
\end{equation}
